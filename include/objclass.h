...
#define is_unwitherable(otmp)	(objects[otmp->otyp].oc_material == MT_BRICK || (otmp->oartifact == ART_MASSIVITY_CUTEBLOCK) || (otmp->oartifact == ART_VAELGROM_S_SPINE) || objects[otmp->otyp].oc_material == MT_FIRMAMENT || (objects[otmp->otyp].oc_material == MT_ADAMANTIUM && rn2(4)) )
#define hard_to_destruct(otmp)	((otmp->otyp == FAKE_AMULET_OF_YENDOR) || objects[otmp->otyp].oc_material == MT_ETERNIUM || objects[otmp->otyp].oc_material == MT_ALIMATHIUM || (otmp->oartifact == ART_JANA_S_MUD_OBSESSION) || (otmp->oartifact == ART_SUNA_S_CONTROVERSY_MATTER) || (otmp->oartifact == ART_VAELGROM_S_SPINE) || (otmp->oartifact == ART_PERMOSTAND) || (otmp->oartifact == ART_MASSIVITY_CUTEBLOCK) || (objects[otmp->otyp].oc_material == MT_BAMBOO && !rn2(4)) || (otmp->oartifact == ART_DAMN_HARD) || otmp->stckcurse || (uimplant && uimplant->oartifact == ART_HENRIETTA_S_TENACIOUSNESS) || (otmp->oartifact == ART_SOFT_COW_DUNG) || (otmp->oartifact == ART_SOFTSTEP) || (otmp->oartifact && otmp->cursed && (spec_ability(otmp, SPFX_EVIL)) ) )
...
#define is_filmitem(otmp)	(objects[otmp->otyp].oc_material == MT_FILM)
...
#define ARM_SUIT	0
	...
	unsigned /*short*/int	oc_weight;	/* encumbrance (1 cn = 0.1 lb.) */
	/*short*/int	oc_cost;		/* base cost in shops */
