...
#define IntShock_resistance	(HShock_resistance || (uarms && uarms->oartifact == ART_TEH_SHOCK) || ShockImmunity || resists_elec(&youmonst) || is_elec_resistant(youmonst.data) || (uarmc && uarmc->oartifact == ART_EXTRASHOCK) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FULLPOWER) || (uarm && uarm->oartifact == ART_EVERLASTING_GEMSTONE) || (powerfulsymbiosis() && is_elec_resistant(&mons[u.usymbiote.mnum])) )
...
#define IntPoison_resistance	(HPoison_resistance || (uball && uball->oartifact == ART_HIGH_AFFIX) || Race_if(PM_SLYER_ALIEN) || resists_poison(&youmonst) || is_poison_resistant(youmonst.data) || (uarmf && uarmf->oartifact == ART_POI_SEN_SUX) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_PULSE_NODE_OF_GERALT) || (powerfulsymbiosis() && is_poison_resistant(&mons[u.usymbiote.mnum])) )
...
#define FemtrapActiveJanet	(FemaleTrapJanet || u.uprops[FEMTRAP_JANET].extrinsic || have_femtrapjanet() || bmwride(ART_RHINE_CROSSING) || (uarmf && uarmf->oartifact == ART_ELIZAH_S_SINKER) )
...
#define ExtSee_invisible	(ESee_invisible || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (ublindf && ublindf->oartifact == ART_EYES_OF_THE_ORACLE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FLICKERING_OF_OZONE) || (uarmh && uarmh->oartifact == ART_ETERNAL_EYE) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (ublindf && ublindf->oartifact == ART_LENSES_OF_TRUTH) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uarmc && uarmc->oartifact == ART_OF_THE_ETHEREAL_EYE) || (uwep && uwep->oartifact == ART_MAUSER_C___CUSTOM) || (uwep && uwep->oartifact == ART_EORLINGAS) || (uwep && uwep->oartifact == ART_LUCKY_DAGGER) || (uwep && uwep->oartifact == ART_THIN_DAGGER) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmh && uarmh->oartifact == ART_SEENBC) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define ExtConf_resist	(EConf_resist || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == CONF_RES) ) || (uarmf && uarmf->oartifact == ART_BOOTS_OF_THE_MACHINE) || (uwep && uwep->oartifact == ART_LAUGHTERSNEE) || (uwep && uwep->oartifact == ART_OGRESMASHER____) || (uarmg && uarmg->oartifact == ART_EGASSO_S_GIBBERISH) || (uarmf && uarmf->oartifact == ART_KRAWASAKI_STEERER) || (uwep && uwep->oartifact == ART_BLOOD_MOON) || (uwep && uwep->oartifact == ART_BLOODY_MACE) || (uarmh && uarmh->oartifact == ART_EFGOURD) || (uarmh && uarmh->oartifact == ART_CAP_OF_THENGEL) || (uarmf && uarmf->oartifact == ART_CRASHING_YOUR_SISTER_S_WED) || (uarmf && uarmf->otyp == CATWALK_SANDALS) || (uarmh && uarmh->oartifact == ART_SEE_THE_TOTAL) || (uwep && uwep->oartifact == ART_SEA_ANGERANCHOR) || (uwep && uwep->oartifact == ART_FUNE_NO_IKARI) || (uarmc && uarmc->oartifact == ART_ORTED_SHAPE) || (uarmf && uarmf->oartifact == ART_PURPLE_SANDSTORM) || (uarmh && uarmh->oartifact == ART_CLAUDIA_S_DIRECTION_FINDER) || (uwep && uwep->oartifact == ART_SHINING_SILVER_KATANA) || (uwep && uwep->oartifact == ART_ZANTETSU) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || bmwride(ART_KERSTIN_S_CLEARSOUND) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uarmf && uarmf->oartifact == ART_SABRINA_S_FREEDOM) || (uarmh && uarmh->oartifact == ART_ENFORCED_MIND) || (uchain && uchain->oartifact == ART_HIMMELIE_S_STRAWBERRY && u.ualign.type == A_LAWFUL) || (uwep && uwep->oartifact == ART_HIMMELIE_S_STRAWBERRY && u.ualign.type == A_LAWFUL) || (uarm && uarm->oartifact == ART_GLARINGADERNADIR) || (uarmh && uarmh->oartifact == ART_ENSNARED_MIND) || (uamul && uamul->oartifact == ART_STAR_OF_HYPERNOTUS) || (uarmg && uarmg->oartifact == ART_TOLLEGAL_WRIGHT) || (uarmg && uarmg->oartifact == ART_GLOVES_OF_VESDA) || (uwep && uwep->oartifact == ART_SPEAR_OF_PEACE) || (uarmc && uarmc->oartifact == ART_BLOND_ANTICLICHE) || (uimplant && uimplant->oartifact == ART_SARREFREE) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_CLONE_) || (uarmf && uarmf->oartifact == ART_UNFELLABLE_TREE && u.burrowed) || (uarmc && uarmc->oartifact == ART_TEAM_BRIDE) || (uarmf && uarmf->oartifact == ART_SANDRA_S_BEAUTIFUL_FOOTWEA) || (uarmf && uarmf->oartifact == ART_SARAH_S_GRANNY_WEAR) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY) || (uarmf && uarmf->oartifact == ART_SARAH_S_SNEAKERS_OF_INSTAN) || (uarmf && uarmf->oartifact == ART_TOAD_S_CONTROL_BUTTON && Race_if(PM_RELEASIER)) || (uarmu && uarmu->oartifact == ART_TOTAL_CONTROL) || (uarmc && uarmc->oartifact == ART_FREQUENT_BUT_WEAK_STATUS) )
