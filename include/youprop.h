...
#define TrapCreationXtra	((have_trapcreationstone() == 2))
...
#define LimitationXtra	((have_limitationstone() == 2))
#define DropcurseXtra	((have_dropcursestone() == 2))
#define WallRegrowXtra	((have_wallstone() == 2))
...
#define WrapOverXtra	((have_wrapoverstone() == 2))
...
#define FalloutEffectXtra	((have_falloutstone() == 2))
...
#define MonNoiseXtra	((have_monnoisestone() == 2))
...
#define TechoutXtra	((have_techoutstone() == 2))
#define StatDecayXtra	((have_statdecaystone() == 2))
#define MoveMorkXtra	((have_movemorkstone() == 2))
...
#define FireImmunity		((uarm && uarm->oartifact == ART_COAL_PEER) || (uwep && uwep->oartifact == ART_HARDENED_IN_THE_FORGE) || (uarmh && uarmh->oartifact == ART_MAGMA_BRINGER) || (uwep && uwep->oartifact == ART_ONCHANGE_STAFF) || (uball && uball->oartifact == ART_HARDENED_IN_THE_FORGE) || (uarms && uarms->oartifact == ART_THATS_ENOUGH_RIGHT_THERE) || Race_if(PM_HYPOTHERMIC))

#define HCold_resistance	u.uprops[COLD_RES].intrinsic
#define ECold_resistance	u.uprops[COLD_RES].extrinsic
#define IntCold_resistance	(HCold_resistance || ColdImmunity || (uwep && uwep->oartifact == ART_CUTRELEASE) || resists_cold(&youmonst) || (uchain && uchain->oartifact == ART_HEI_THE_COLD) || (uwep && uwep->oartifact == ART_WINTERHOLD_S_KEEP) || (uwep && uwep->oartifact == ART_COOLSHIELDING) || (uwep && uwep->oartifact == ART_COLD_HAHA) || (uwep && uwep->oartifact == ART_LAINCHANTMENT) || is_cold_resistant(youmonst.data) || (powerfulsymbiosis() && is_cold_resistant(&mons[u.usymbiote.mnum])))
...
#define ColdImmunity		((uwep && uwep->oartifact == ART_CUTRELEASE) || (uchain && uchain->oartifact == ART_HEI_THE_COLD) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_BRIGHT_WHITE) || (uwep && uwep->oartifact == ART_COLD_HAHA) || (uwep && uwep->oartifact == ART_COOLSHIELDING) || (uwep && uwep->oartifact == ART_WINTERHOLD_S_KEEP))
...
#define NoDisint_resistance (!Race_if(PM_IMMUNIZER) && (u.uprops[DEAC_DISINT_RES].intrinsic || (uarmf && uarmf->oartifact == ART_SATAN_S_CHOPPING_BLOCK) || (uarmu && uarmu->oartifact == ART_TINY_THREAD) || (uarmh && uarmh->oartifact == ART_UBB_RUPTURE) || UHaveAids || (u.impossibleproperty == DISINT_RES) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0)))
...
#define ShockImmunity		(Race_if(PM_PLAYER_DYNAMO) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarms && uarms->oartifact == ART_TEH_SHOCK) || (uarms && uarms->oartifact == ART_GROUNDED_FOREVER && objects[uarms->otyp].oc_material == MT_WOOD) )
...
#define IntDrain_resistance	(HDrain_resistance || (uwep && uwep->oartifact == ART_VLADIMIR_BE_AFRAID) || (uarm && uarm->oartifact == ART_SEVEBREAKYOU__SEVEBREAK_) || resists_drli(&youmonst) || is_drain_resistant(youmonst.data) || (powerfulsymbiosis() && is_drain_resistant(&mons[u.usymbiote.mnum]) ) )
#define ExtDrain_resistance	(EDrain_resistance || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DRAIN_RES) ) || (uarmc && uarmc->oartifact == ART_ALL_IN_ONE_ASCENSION_KIT) || autismringcheck(ART_MAGICAL_SHOCK) || (uarmh && uarmh->oartifact == ART_LLLLLLLLLLLLLM) || (uarmf && uarmf->oartifact == ART_ULTRACURSED_VAMPIRIC_HIGH_) || (uarmf && uarmf->oartifact == ART_CUTESY_GIRL) || (uwep && uwep->oartifact == ART_CUBIC_BONE) || (uarmf && uarmf->oartifact == ART_MAILIE_S_CHALLENGE) || (uwep && uwep->oartifact == ART_AVENGER) || (uwep && uwep->oartifact == ART_VLADIMIR_BE_AFRAID) || (uarms && uarms->oartifact == ART_BARROWGRIM_S_ARMY) || (uarmh && uarmh->oartifact == ART_YOU_ARE_ALREADY_DEAD) || (uarm && uarm->oartifact == ART_ALUCART_MAIL) || (uwep && uwep->oartifact == ART_LIFEHUNT_SCYTHE) || (uarm && uarm->oartifact == ART_CHANGERING_ENVIROMENT) || (uwep && uwep->oartifact == ART_EXCALISHIELD) || (uarm && uarm->oartifact == ART_SEVEBREAKYOU__SEVEBREAK_) || (uarmf && uarmf->oartifact == ART_USE_THE_NORMALNESS_TURNS) || (uarm && uarm->oartifact == ART_DEMANDING_ENTRY) || (uarmg && uarmg->oartifact == ART_SPECTRATOR && (moves % 5 == 0) ) || (uarmh && uarmh->oartifact == ART_LOW_LOCAL_MEMORY) || (uarms && uarms->oartifact == ART_HEALTHIER_FEEL) || (uwep && uwep->oartifact == ART_SANDMAN_VOLUME__) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (uwep && uwep->oartifact == ART_ORTHODOX_MANIFEST) || (uarmf && uarmf->oartifact == ART_BARBED_HOOK_ZIPPER) || (uwep && uwep->oartifact == ART_UN_DEATH) || (uarmc && uarmc->oartifact == ART_VEIL_OF_LATONA) || (uwep && uwep->oartifact == ART_UNICORN_DRILL) || (uarmc && uarmc->oartifact == ART_VEIL_OF_MINISTRY) || (uwep && uwep->oartifact == ART_LANCE_OF_LONGINUS) || ((moves % 3 == 0) && uarmc && itemhasappearance(uarmc, APP_DEEP_CLOAK)) || (uarmc && itemhasappearance(uarmc, APP_VAMPIRIC_CLOAK) && (moves % 10 == 0) ) )
...
#define AcidImmunity		((uarm && uarm->oartifact == ART_NULARMOR) || (uarmf && uarmf->oartifact == ART_LISA_S_YELLOW_STRIPE) || (uball && uball->oartifact == ART_ACICORNER) || (uarmf && uarmf->oartifact == ART_POCAHONTAS_HOME))
...
#define FemtrapActiveLara	(FemaleTrapLara || u.uprops[FEMTRAP_LARA].extrinsic || have_femtraplara() || (uarms && uarms->oartifact == ART_CASTLE_CRUSH_GLITCH) )
#define FemtrapActiveAnastasia	(FemaleTrapAnastasia || u.uprops[FEMTRAP_ANASTASIA].extrinsic || have_femtrapanastasia() || (uarmf && uarmf->oartifact == ART_THAT_S_SUPER_UNFAIR) || (uarmf && uarmf->oartifact == ART_PLAYING_ANASTASIA) || (uarmf && uarmf->oartifact == ART_WHITE_LOAFERS) || (uarmf && uarmf->oartifact == ART_THICK_PLATFORM_CRAZE) || (uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) || (uarmf && uarmf->oartifact == ART_NANCY_S_ASSISTANCE) || (uarmf && uarmf->oartifact == ART_SHE_S_STILL_AN_ANASTASIA) || autismweaponcheck(ART_REAL_WALKING) || (autismweaponcheck(ART_DAMN_SKI_WEDGE) && !uarmf) || (uarmf && uarmf->oartifact == ART_FASTER_THAN_ALL_OTHERS_INT))
...
#define FemtrapActiveKristina	(FemaleTrapKristina || u.uprops[FEMTRAP_KRISTINA].extrinsic || have_femtrapkristina() || (uarmf && uarmf->oartifact == ART_JUEN_S_WEAKNESS) )
...
#define FemtrapActiveJette	(FemaleTrapJette || u.uprops[FEMTRAP_JETTE].extrinsic || have_femtrapjette() || autismweaponcheck(ART_HER_UNREACHABLE_BROOK) || (uarmu && uarmu->oartifact == ART_HEEEEELEEEEEN && !Role_if(PM_ACTIVISTOR)) )
...
#define FemtrapActiveGudrun	(FemaleTrapGudrun || u.uprops[FEMTRAP_GUDRUN].extrinsic || have_femtrapgudrun() || (autismweaponcheck(ART_OZYZEVPDWTVP) && u.homosexual == 1) )
#define FemtrapActiveElla	(FemaleTrapElla || u.uprops[FEMTRAP_ELLA].extrinsic || have_femtrapella() || (autismweaponcheck(ART_OZYZEVPDWTVP) && u.homosexual == 2) )
...
#define FemtrapActiveLisa	(FemaleTrapLisa || u.uprops[FEMTRAP_LISA].extrinsic || have_femtraplisa() || (uarmu && uarmu->oartifact == ART_LISE_S_UNDERWEAR) )
#define FemtrapActiveBridghitte	(FemaleTrapBridghitte || u.uprops[FEMTRAP_BRIDGHITTE].extrinsic || have_femtrapbridghitte() || (uarmf && uarmf->oartifact == ART_FIGHT_WITH_PLATFORM_INA) )
...
#define FemtrapActiveNicole	(FemaleTrapNicole || u.uprops[FEMTRAP_NICOLE].extrinsic || have_femtrapnicole() || (uarmc && uarmc->oartifact == ART_SHROUD) )
...
#define FemtrapActiveElise	(FemaleTrapElise || u.uprops[FEMTRAP_ELISE].extrinsic || have_femtrapelise() || (uarmu && uarmu->oartifact == ART_GREENTOP))
...
#define FemtrapActiveGreta	(FemaleTrapGreta || u.uprops[FEMTRAP_GRETA].extrinsic || have_femtrapgreta() || autismweaponcheck(ART_FREAK_VERSUS_BANGER) )
...
#define SuperFemtrapLudgera	( (have_femtrapludgera() == 2) )
...
#define SuperFemtrapPatricia	( (have_femtrappatricia() == 2) )
...
#define SuperFemtrapBridghitte	( (have_femtrapbridghitte() == 2) )
#define SuperFemtrapNicole	( (have_femtrapnicole() == 2) )
...
#define SuperFemtrapMariya	( (have_femtrapmariya() == 2) )
#define SuperFemtrapElise	( (have_femtrapelise() == 2) )
...
#define SuperFemtrapLuisa	( (have_femtrapluisa() == 2) )
#define SuperFemtrapLiselotte	( (have_femtrapliselotte() == 2) )
#define SuperFemtrapGreta	( (have_femtrapgreta() == 2) )
...
#define SuperFemtrapLara	( (have_femtraplara() == 2) )
...
#define ExtHalluc_resistance	(EHalluc_resistance || have_allisloststone() || (Race_if(PM_BATMAN) && uwep && uwep->oartifact == ART_BLACKSWANDIR))
...
#define ExtSee_invisible	(ESee_invisible || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define IntTelepat	(HTelepat || (uarmf && uarmf->oartifact == ART_GLITCH_IN_THE_MONITOR) || (uarmf && uarmf->oartifact == ART_SCAN_ME) || Race_if(PM_DEVELOPER) || telepathic(youmonst.data) || (Role_if(PM_TOPMODEL) && PlayerInHighHeels) )
...
#define IntInvis	(HInvis || (uarmc && uarmc->oartifact == ART_FIND_ME_NEVERTHELESS) || (uarmc && uarmc->oartifact == ART_DOEDOEDOEDOEDOEDOEDOE_TEST) || autismringcheck(ART_CAN_T_FIND_ME) || pm_invisible(youmonst.data))
...
#define IntStealth	(HStealth || autismringcheck(ART_XANATHAR_S_RING_OF_PROOF) || (uarmc && uarmc->oartifact == ART_STEALTH_OR_STEALING) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_RESTROOM_DRENCHING) || (Race_if(PM_MOON_ELF) && ((flags.moonphase >= 1 && flags.moonphase <= 3) || (uwep && uwep->oartifact == ART_SUPERSTAB) || (flags.moonphase >= 5 && flags.moonphase <= 7))) )
...
#define ExtFlying	(EFlying || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == FLYING) ) || (uwep && uwep->oartifact == ART_PENGUIN_S_THRUSTING_SWORD) || autismringcheck(ART_JANA_S_DIMINISHER) || (uarm && uarm->oartifact == ART_FLY_LIKE_AN_EAGLE) || (uwep && uwep->oartifact == ART_P_WING) || (uarmc && uarmc->oartifact == ART_FULL_WINGS) || (uarmc && uarmc->oartifact == ART_DARK_ANGELS) || (uarmc && uarmc->oartifact == ART_FAILSET_GAMBLE) || (uarms && uarms->oartifact == ART_I_IN_THE_SKY) || (uarmf && uarmf->oartifact == ART_MEPHISTO_S_BROGUES) || (uarmf && uarmf->oartifact == ART_FLOATOVER) || (uarmf && uarmf->oartifact == ART_CORINA_S_UNFAIR_SCRATCHER) || (uarmg && uarmg->oartifact == ART_GO_UP_AND_SOAR) || bmwride(ART_SADDLE_OF_BRYNHILDR) || (uwep && uwep->oartifact == ART_HELICOPTER_TWIRL) || (uwep && uwep->oartifact == ART_HELL_OR_HIGH_WATER) || (uarmf && uarmf->oartifact == ART_MARJI_JANA) || (uwep && uwep->oartifact == ART_OJOUSAMAHAMAJO) || (uarmf && itemhasappearance(uarmf, APP_WINGED_SANDALS) ) || (uwep && uwep->oartifact == ART_WIERD_ACCENT) || (uarmh && uarmh->oartifact == ART_SOARUP) || (ublindf && ublindf->oartifact == ART_FREEBOUND) || (ublindf && ublindf->oartifact == ART_MAGIC_CARPET) || (uwep && uwep->oartifact == ART_MILENA_S_MISGUIDING) || (uarmf && uarmf->oartifact == ART_DON_T_FALL_INTO_THE_ABYSS) || (uarmf && uarmf->oartifact == ART_FLYGEL_BOOTS) || (uwep && uwep->oartifact == ART_SPACEL_SWIM) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_FLYSKYHGH) || (uwep && uwep->oartifact == ART_ONE_HUNDRED_STARS) || flysaddle() || (uarmf && uarmf->oartifact == ART_FENG_HUO_LUN) || autismringcheck(ART_GOLDENIVY_S_RAGE) || (uwep && uwep->oartifact == ART_ALDEBARAN_FORM) || (uarmh && uarmh->oartifact == ART_GIRLFUL_FARTING_NOISES) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_JANA_S_MAKE_UP_PUTTY) || (uarmf && uarmf->oartifact == ART_NATURAL_SPRING) || (uarmc && uarmc->oartifact == ART_JANA_S_FAIRNESS_CUP) || autismringcheck(ART_GOLDENIVY_S_ENGAGEMENT_RIN) || (uarm && uarm->oartifact == ART_ASTRA_MAIL) || (uamul && uamul->oartifact == ART_STINGING_MEDALLION) || (uarmh && uarmh->oartifact == ART_FLYYYYY) || (uwep && uwep->oartifact == ART_SOURCE_CODES_OF_WORK_AVOID) || (uarmf && uarmf->oartifact == ART_RUTH_S_MORTAL_ENEMY) || (uamul && uamul->oartifact == ART_DIKKIN_S_DRAGON_TEETH) || (uarmg && uarmg->oartifact == ART_EXPERTENGAME_THE_ENTIRE_LE) || (uarmf && uarmf->oartifact == ART_EQUIPPED_FOR_TROUBLE) || (uarmg && uarmg->oartifact == ART_DIFFICULTY__) || (uarmf && uarmf->oartifact == ART_GRENEUVENIA_S_HUG) || (uarmh && uarmh->oartifact == ART_FLUE_FLUE_FLUEFLUE_FLUE) || (Role_if(PM_TOPMODEL) && PlayerInHighHeels) )
...
#define ExtSlow_digestion	(ESlow_digestion || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SLOW_DIGESTION) ) || (uwep && uwep->oartifact == ART_BREAD_FOR_THE_WORLD) || (uwep && uwep->oartifact == ART_HOLY_GRAIL) || (uarmh && uarmh->oartifact == ART_NEVEREATER) || autismringcheck(ART_SCHATZKI_S_RING) || (uarm && uarm->oartifact == ART_FULL_DOSE_LIFE) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_ENTEROGASTER_ALTO) || (uimplant && uimplant->oartifact == ART_BUCKET_HOUSE) || (uarmc && uarmc->oartifact == ART_FEMMY_FATALE) || (uarmh && uarmh->oartifact == ART_WOLF_KING) )
...
#define IntHalf_spell_damage	(HHalf_spell_damage || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CLEAN_ASCENSION_RUN && In_endgame(&u.uz)) || (uarms && uarms->oartifact == ART_ALTERNATE_SPELLWEAVE) || (Stoned_chiller && Stoned && !(u.stonedchilltimer)) || (Race_if(PM_BOVER) && u.usteed))
...
#define IntRegeneration	(HRegeneration || (uarmg && uarmg->oartifact == ART_GAUNTLETS_OF_THE_HEALING_H) || autismringcheck(ART_RING_OF_HYGIENE_S_DISCIPLE) || (uarmh && uarmh->oartifact == ART_HABIBA_S_MATRONAGE) || regenerates(youmonst.data))
...
#define ExtStun_resist	(EStun_resist || (uarmf && uarmf->oartifact == ART_KRISTIN_S_CHEATS) || (uwep && uwep->oartifact == ART_DEATHWRECKER) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == STUN_RES) ) || (uarmu && uarmu->oartifact == ART_TOTAL_CONTROL) || (uwep && uwep->oartifact == ART_LAUGHTERSNEE) || (uwep && uwep->oartifact == ART_OGRESMASHER____) || (uarm && uarm->oartifact == ART_PROTECTION_WITH_A_PRICE) || (PlayerInSexyFlats && uarmf && uarmf->oartifact == ART_FORMO____) || (uarm && uarm->oartifact == ART_DERANDERGRAAA) || (uarmh && uarmh->oartifact == ART_SEE_THE_TOTAL) || (uarmf && uarmf->oartifact == ART_RUBBER_LOVE) || (uwep && uwep->oartifact == ART_PENDULUM_OF_BALANCE) || (uarmf && uarmf->oartifact == ART_JENNY_SUPERSOFT) || (uwep && uwep->oartifact == ART_SPEAR_OF_PEACE) || (uwep && uwep->oartifact == ART_TURVANG_S_THUNDER) || (uarmf && uarmf->oartifact == ART_ENDLESS_DESEAMING) || autismringcheck(ART_RELIABLE_TRINSICS) || (uarmf && uarmf->oartifact == ART_CLONE_) || (uwep && uwep->oartifact == ART_JOUSTER_POWER) || (uarmc && uarmc->oartifact == ART_FREQUENT_BUT_WEAK_STATUS) || (uarmh && uarmh->oartifact == ART_CLAUDIA_S_DIRECTION_FINDER) || (uarmh && uarmh->oartifact == ART_UBB_RUPTURE) || (uarmh && uarmh->oartifact == ART_MIND_SHIELDING) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY) || (uamul && uamul->oartifact == ART_ONE_MOMENT_IN_TIME) || (uamul && uamul->oartifact == ART_PRECIOUS_UNOBTAINABLE_PROP) || (uamul && uamul->oartifact == ART_WOBBLESTEADY) || (uamul && uamul->oartifact == ART_RECOVERED_RELIC) )
...
#define ExtConf_resist	(EConf_resist || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == CONF_RES) ) || (uarmf && uarmf->oartifact == ART_BOOTS_OF_THE_MACHINE) || (uwep && uwep->oartifact == ART_LAUGHTERSNEE) || (uwep && uwep->oartifact == ART_OGRESMASHER____) || (uarmg && uarmg->oartifact == ART_EGASSO_S_GIBBERISH) || (uarmf && uarmf->oartifact == ART_KRAWASAKI_STEERER) || (uarmh && uarmh->oartifact == ART_EFGOURD) || (uarmf && uarmf->oartifact == ART_CRASHING_YOUR_SISTER_S_WED) || (uarmh && uarmh->oartifact == ART_SEE_THE_TOTAL) || (uarmh && uarmh->oartifact == ART_CLAUDIA_S_DIRECTION_FINDER) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (uarmh && uarmh->oartifact == ART_ENFORCED_MIND) || (uarm && uarm->oartifact == ART_GLARINGADERNADIR) || (uamul && uamul->oartifact == ART_STAR_OF_HYPERNOTUS) || (uwep && uwep->oartifact == ART_SPEAR_OF_PEACE) || (uimplant && uimplant->oartifact == ART_SARREFREE) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_CLONE_) || (uarmf && uarmf->oartifact == ART_UNFELLABLE_TREE && u.burrowed) || (uarmf && uarmf->oartifact == ART_SANDRA_S_BEAUTIFUL_FOOTWEA) || (uarmf && uarmf->oartifact == ART_SARAH_S_GRANNY_WEAR) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY) || (uarmf && uarmf->oartifact == ART_SARAH_S_SNEAKERS_OF_INSTAN) || (uarmu && uarmu->oartifact == ART_TOTAL_CONTROL) || (uarmc && uarmc->oartifact == ART_FREQUENT_BUT_WEAK_STATUS) )
...
#define ExtQuad_attack	(EQuad_attack || autismweaponcheck(ART_EXTREME_BITCH) || (uarmc && uarmc->oartifact == ART_SUPERMAN_S_SUPER_SUIT) || (uimplant && uimplant->oartifact == ART_IME_SPEW))
...
#define IntDiscount_action	(HDiscount_action || (uarmc && uarmc->oartifact == ART_STRIPED_SHIRT_OF_THE_FALSE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CLEAN_ASCENSION_RUN && In_endgame(&u.uz)) )
#define ExtDiscount_action	(EDiscount_action || (uarmf && uarmf->oartifact == ART_LISA_S_BUCKLED_BEAUTIES) || (uwep && uwep->oartifact == ART_HARMONY_VIRGIN) || (uarmf && uarmf->oartifact == ART_CRASHING_YOUR_SISTER_S_WED) || (uarmf && uarmf->oartifact == ART_ELEVECULT) || (uimplant && uimplant->oartifact == ART_CLEAN_ASCENSION_RUN && In_endgame(&u.uz)) || (Role_if(PM_SINGSLAVE) && uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) || (uarm && uarm->oartifact == ART_JOHN_CENA_SPLIT_MAIL) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_GOOSCH_HIDARI) || (flags.female && uarmf && uarmf->oartifact == ART_BS_____) || (!player_wears_metal() && uarmc && uarmc->oartifact == ART_NO_METAL_ALLOWED) || (uamul && uamul->oartifact == ART_PROTECTED_MODE_RUN_TIME) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CAN_T_BRING_US_DOWN) || (uarmc && uarmc->oartifact == ART_STRIPED_SHIRT_OF_THE_FALSE) || (uarmf && uarmf->oartifact == ART_NANCY_S_ASSISTANCE) || (uwep && uwep->oartifact == ART_HELF_ME_NEVERTHELESS) || (uarmf && uarmf->oartifact == ART_NADINE_S_CUTENESS) || (uwep && uwep->oartifact == ART_TRUE_MASTERPIECE) || (uarmf && uarmf->oartifact == ART_IMPOSSIBLE_CATWALK) || (uarmf && uarmf->oartifact == ART_FULL_PROGRAM) || (uarm && uarm->oartifact == ART_LYNN_S_EVERVIGILANCE) || (uwep && uwep->oartifact == ART_NOOBY_BONUS_STYLE && bimanual(uwep)) || (uwep && uwep->oartifact == ART_SMOKING_SQUIRREL) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_BUCKET_HOUSE) || (uarmu && uarmu->oartifact == ART_PEDIATRIC_GAWKING_GANGS) || (uwep && uwep->oartifact == ART_COUNTERSTRIKE_CESTUS) || (uarmu && uarmu->oartifact == ART_KING_OF_PORN) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DISCOUNT_ACTION) ) )
...
#define ExtScentView	(EScentView || (uarm && uarm->oartifact == ART_FARTHER_INTO_THE_JUNGLE) || (uarmf && uarmf->oartifact == ART_CAMELIC_SCENT) || (uarmh && uarmh->oartifact == ART_HEARING_EAR) || (uarmh && uarmh->oartifact == ART_COMPLETE_SIGHT) || (uwep && uwep->oartifact == ART_INSECTMASHER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_THEY_RE_REALLY_AFTER_ME) || (uwep && uwep->oartifact == ART_HELIOPOLIS_MISTAKE) || (uarm && uarm->oartifact == ART_CRYPT_SMELL && in_rooms(u.ux, u.uy, CRYPTROOM)) || (uarmf && uarmf->oartifact == ART_SAFARI_ROCKZ) || (ublindf && ublindf->oartifact == ART_SCENTFOLD) )
...
#define IntControlMagic	(HControlMagic || (uarm && uarm->oartifact == ART_SPECIAL_ROBUNG) )
...
#define ExtDefusing	(EDefusing || (uarm && uarm->oartifact == ART_ARMS_OVERSIGHT) || (uwep && uwep->oartifact == ART_GO_FULLY_OFF) || (uarm && uarm->oartifact == ART_ENCHANTED__OF_DEFUSING) || (uarm && uarm->oartifact == ART_BEEPA_DEVICE) || (uwep && uwep->oartifact == ART_HALF_SUPER_ENCHANTMENT) || (ublindf && ublindf->otyp == DEFUSING_BOX) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DEFUSING) ) )
...
#define IntSpellboost	(HSpellboost || (uwep && uwep->oartifact == ART_IRON_REINFORCEMENT) || (uarmg && uarmg->oartifact == ART_WAND_INTO_SPELL) || (uarm && uarm->oartifact == ART_SPECIAL_ROBUNG) )
#define ExtSpellboost	(ESpellboost || (uwep && uwep->oartifact == ART_IRON_REINFORCEMENT) || (uarmg && uarmg->oartifact == ART_WAND_INTO_SPELL) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uwep && uwep->oartifact == ART_CASTOR_BOOST) || (uwep && uwep->oartifact == ART_HALT_SHAMAN) || (uwep && uwep->oartifact == ART_SAMA_KATA) || (uwep && uwep->oartifact == ART_SCHWI_SCHWI) || (uarmh && uarmh->oartifact == ART_HEAD_W) || (uarms && uarms->oartifact == ART_GONGDE_ERTAL) || (uwep && uwep->oartifact == ART_FOOTPRINTS_IN_THE_LABYRINT) || (uwep && uwep->oartifact == ART_CALLANDOR) || (uarmu && uarmu->oartifact == ART_ALL_IN_ONE_EFF) || (uarmh && uarmh->oartifact == ART_CAAAAAST) || (uarm && uarm->oartifact == ART_SPECIAL_ROBUNG) || (uarm && uarm->oartifact == ART_HEART_INFACT) || (uarm && uarm->oartifact == ART_CHIP_INFACT) || (uarm && uarm->oartifact == ART_ETERNAL_BAMMELING) || (uarmh && uarmh->oartifact == ART_DAN_THE_MAGE) || (uwep && uwep->otyp == CASTER_STAFF) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SPELLBOOST) ) )
