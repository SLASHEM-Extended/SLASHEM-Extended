...
#define FemtrapActiveDora	(FemaleTrapDora || u.uprops[FEMTRAP_DORA].extrinsic || have_femtrapdora() || autismweaponcheck(ART_DESERT_MAID) || (uarmf && uarmf->oartifact == ART_SHE_REALLY_LIKES_IT) || (uarm && uarm->oartifact == ART_TIMONA_S_INNER_BICKER) )
...
#define IntResistancePiercing	(HResistancePiercing || autismringcheck(ART_DEBBLE_TREBBLE) || (uarmu && uarmu->oartifact == ART_HA_HA_HA_HA___) )
...
#define ExtWildWeatherEffect	(EWildWeatherEffect || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || (uamul && uamul->oartifact == ART_______SCORE) )
