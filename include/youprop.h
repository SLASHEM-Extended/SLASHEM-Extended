...
#define PlayerUninformation	(UninformationProblem || u.uprops[UNINFORMATION].extrinsic || have_uninformationstone() || (uimplant && uimplant->oartifact == ART_BREAHNA_S_GARBLE) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY))
...
#define ColdImmunity		((uwep && uwep->oartifact == ART_CUTRELEASE) || (uchain && uchain->oartifact == ART_HEI_THE_COLD) || (uarmf && uarmf->oartifact == ART_FLEEC_LIE) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_LOIS_S_CHILL) || (uarmf && uarmf->oartifact == ART_BRIGHT_WHITE) || (uwep && uwep->oartifact == ART_COLD_HAHA) || (uarmg && uarmg->oartifact == ART_MERYN_S_PEAK) || autismringcheck(ART_ARCTIC_EXPEDITE) || (uarmh && uarmh->oartifact == ART_CHARLENE_S_CORONA) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_BUGGEDNESS_OF_THE_COMPANY) || (uamul && uamul->oartifact == ART_CURSING_PENDANT && uamul->cursed) || (uarmc && uarmc->oartifact == ART_FLEECY_CORN) || (uwep && uwep->oartifact == ART_GOOTE_UTE) || u.cryopancaketurns || (uwep && uwep->oartifact == ART_COOLSHIELDING) || (uwep && uwep->oartifact == ART_WINTERHOLD_S_KEEP))
...
#define IntShock_resistance	(HShock_resistance || (uarms && uarms->oartifact == ART_TEH_SHOCK) || ShockImmunity || resists_elec(&youmonst) || is_elec_resistant(youmonst.data) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FULLPOWER) || (uarm && uarm->oartifact == ART_EVERLASTING_GEMSTONE) || (powerfulsymbiosis() && is_elec_resistant(&mons[u.usymbiote.mnum])) )
...
#define SuperFemtrapKerstin	( (have_femtrapkerstin() == 2) || (uarm && uarm->oartifact == ART_MOTA_ROLA) )
...
#define IntSee_invisible	(HSee_invisible || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FLICKERING_OF_OZONE) || autismringcheck(ART_FUBBLE) || perceives(youmonst.data))
#define ExtSee_invisible	(ESee_invisible || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (ublindf && ublindf->oartifact == ART_EYES_OF_THE_ORACLE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FLICKERING_OF_OZONE) || (uarmh && uarmh->oartifact == ART_ETERNAL_EYE) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (ublindf && ublindf->oartifact == ART_LENSES_OF_TRUTH) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uarmc && uarmc->oartifact == ART_OF_THE_ETHEREAL_EYE) || (uwep && uwep->oartifact == ART_MAUSER_C___CUSTOM) || (uwep && uwep->oartifact == ART_EORLINGAS) || (uwep && uwep->oartifact == ART_LUCKY_DAGGER) || (uwep && uwep->oartifact == ART_THIN_DAGGER) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define IntTelepat	(HTelepat || autismringcheck(ART_SET_DEFPARM) || (uarmf && uarmf->oartifact == ART_GLITCH_IN_THE_MONITOR) || (uarmf && uarmf->oartifact == ART_SCAN_ME) || (uarmh && uarmh->oartifact == ART_SUPERPATHY) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_JANE_S_BODY_SIZE) || Race_if(PM_DEVELOPER) || telepathic(youmonst.data) || (Role_if(PM_TOPMODEL) && PlayerInHighHeels) )
#define ExtTelepat	(ETelepat || (uarmf && uarmf->oartifact == ART_SCAN_ME) || (uamul && uamul->oartifact == ART_AMULET_OF_SPLENDOR) || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_JANE_S_BODY_SIZE) || (uchain && uchain->oartifact == ART_IZQUXXISQUID_S_RESEARCH) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || Race_if(PM_DEVELOPER))
...
#define IntStun_resist	(HStun_resist || (powerfulimplants() && uimplant && uimplant->oartifact == ART_LIVE_AT_READING) || Race_if(PM_EROSATOR) || tech_inuse(T_STAT_RESIST))
