...
#define ColdImmunity		((uwep && uwep->oartifact == ART_CUTRELEASE) || (uchain && uchain->oartifact == ART_HEI_THE_COLD) || (uarmf && uarmf->oartifact == ART_FLEEC_LIE) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_LOIS_S_CHILL) || (uarmf && uarmf->oartifact == ART_BRIGHT_WHITE) || (uwep && uwep->oartifact == ART_COLD_HAHA) || autismringcheck(ART_ARCTIC_EXPEDITE) || (uarmh && uarmh->oartifact == ART_CHARLENE_S_CORONA) || (uamul && uamul->oartifact == ART_CURSING_PENDANT && uamul->cursed) || (uarmc && uarmc->oartifact == ART_FLEECY_CORN) || (uwep && uwep->oartifact == ART_GOOTE_UTE) || u.cryopancaketurns || (uwep && uwep->oartifact == ART_COOLSHIELDING) || (uwep && uwep->oartifact == ART_WINTERHOLD_S_KEEP))
...
#define EPrem_death	(u.uprops[PREMDEATH].extrinsic || (uimplant && uimplant->oartifact == ART_THROW_MY_LIFE_AWAY) || (flags.female && uwep && autismweaponcheck(ART_TOMORROW_WENDY_S_CHOICENA)) || (uarms && uarms->oartifact == ART_YOU_GOTTA_SEPARATE) || (uarmc && uarmc->oartifact == ART_KYS_YOURSELF) )
...
#define FemtrapActiveEveline	(FemaleTrapEveline || u.uprops[FEMTRAP_EVELINE].extrinsic || have_femtrapeveline() || (uarmh && uarmh->oartifact == ART_JOLENE_S_ZEAL) )
...
#define FemtrapActiveSing	(FemaleTrapSing || u.uprops[FEMTRAP_SING].extrinsic || have_femtrapsing() || autismweaponcheck(ART_VAMPIRE_SING_S_BLOODRUSH) || autismweaponcheck(ART_TONA_S_GAMES) || (Role_if(PM_SINGSLAVE)) || (uarm && uarm->oartifact == ART_SING_S_STUNNING_BEAUTY) || (uarmf && uarmf->oartifact == ART_SINGHETA) )
...
#define FemtrapActiveVerena	(FemaleTrapVerena || u.uprops[FEMTRAP_VERENA].extrinsic || have_femtrapverena() || (uarm && uarm->oartifact == ART_ALEIT_GIVESITNOT) || (uarmh && uarmh->oartifact == ART_IGNORANCE_IS_BLISS))
...
#define FemtrapActiveKerstin	(FemaleTrapKerstin || u.uprops[FEMTRAP_KERSTIN].extrinsic || have_femtrapkerstin() || (uarmf && uarmf->oartifact == ART_KERSTIN_S_LITTLE_HELP) || (uarmh && uarmh->oartifact == ART_WHO_CAN_DRIVE_BEST_) || bmwride(ART_KERSTIN_S_COWBOY_BOOST) || autismweaponcheck(ART_GONDOLIN_S_HIDDEN_PASSAGE) )
...
#define FemtrapActiveMarleen	(FemaleTrapMarleen || u.uprops[FEMTRAP_MARLEEN].extrinsic || have_femtrapmarleen() || (uarmf && uarmf->oartifact == ART_MARLEEN_S_SOFTNESS) || (uarmf && uarmf->oartifact == ART_EROTIC_STAT_TRAIN) )
...
#define FunnyHallu		(Role_if(PM_GOFF) || (uamul && uamul->oartifact == ART_NARCOTIC_TRADE) || (uarmc && uarmc->oartifact == ART_BE_HI_WITHOUT_DRUGS) || RngeFunnyHallu || ishallucinator || HHallucination || u.uprops[MULTISHOES].extrinsic || u.funnyhallutimer || autismweaponcheck(ART_FADED_USELESSNESS) || autismweaponcheck(ART_LANCE_OF_LONGINUS) || (uamul && uamul->oartifact == ART_FUN_ALL) || have_leohighlantern() || autismweaponcheck(ART_SCHWANZUS_LANGUS) || HeavyHallu || EHallucination || u.uprops[SENSORY_DEPRIVATION].extrinsic || (u.funnyhalluroll != 9999999 && u.usanity > u.funnyhalluroll) || flags.hippie || ( (u.uprops[DEHYDRATION].extrinsic || Dehydration || have_dehydratingstone()) && ((u.dehydrationtime - moves) < 1) )  )
...
#define ExtSee_invisible	(ESee_invisible || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (ublindf && ublindf->oartifact == ART_EYES_OF_THE_ORACLE) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (ublindf && ublindf->oartifact == ART_LENSES_OF_TRUTH) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uwep && uwep->oartifact == ART_MAUSER_C___CUSTOM) || (uwep && uwep->oartifact == ART_EORLINGAS) || (uwep && uwep->oartifact == ART_LUCKY_DAGGER) || (uwep && uwep->oartifact == ART_THIN_DAGGER) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define IntRegeneration	(HRegeneration || (uarmg && uarmg->oartifact == ART_GAUNTLETS_OF_THE_HEALING_H) || autismringcheck(ART_RING_OF_HYGIENE_S_DISCIPLE) || (uwep && uwep->oartifact == ART_MAEVE_S_OPULENCE) || (uarm && uarm->oartifact == ART_BASTLE_BACK_UP) || (uarmh && uarmh->oartifact == ART_HABIBA_S_MATRONAGE) || regenerates(youmonst.data))
...
#define Protection_from_shape_changers \
				(HProtection_from_shape_changers || \
				 EProtection_from_shape_changers || (uarm && uarm->oartifact == ART_FIGGER_FOUR) || (uamul && uamul->oartifact == ART_VARIANT_GUARD) )
...
#define NoFast	(!Race_if(PM_IMMUNIZER) && (u.uprops[DEAC_FAST].intrinsic || (uarmf && uarmf->oartifact == ART_END_OF_LEWDNESS) || autismringcheck(ART_FUW_TENK) || UHaveAids || (FemtrapActiveNaomi && !PlayerInHighHeels) || (uarm && uarm->oartifact == ART_FUCK_UGGHH_THAT_S_HEAVY_) || (u.impossibleproperty == FAST) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0)))
...
#define ExtControlMagic	(EControlMagic || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == CONTROL_MAGIC) ) || autismringcheck(ART_HAVE_FULL_SWAY) || (uarmf && uarmf->oartifact == ART_BLUE_BLOSSOM) || (uamul && uamul->oartifact == ART_CONTRO_MOVE) || (uarmf && uarmf->oartifact == ART_ARABELLA_S_GIRL_KICK) || (uarmf && uarmf->oartifact == ART_CRASHING_YOUR_SISTER_S_WED) || (uwep && uwep->oartifact == ART_LETS_MAKE_IT_OFFICIAL && u.kliuskill >= 1280) || (uwep && uwep->oartifact == ART_CONTROL_HELP) || (uwep && uwep->oartifact == ART_COUNTERSTRIKE_CESTUS) || (uwep && uwep->otyp == CONTRO_STAFF))
...
#define ExtExpBoost	(EExpBoost || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == EXP_BOOST) ) || (uarmf && uarmf->oartifact == ART_NEXT_LEVEL) || have_minimejewel() || autismringcheck(ART_SET_DEFPARM) || (uwep && uwep->oartifact == ART_DOUBLE_MYSTERY) || autismringcheck(ART_RING_OF_FAST_LIVING))
...
#define ExtPainSense	(EPainSense || (uarmf && uarmf->oartifact == ART_CUTE_JEANS) || have_feeldasag() || (uwep && uwep->oartifact == ART_TANKFRIEND) || (uamul && objects[uamul->otyp].oc_color == CLR_BLUE) || (uleft && objects[uleft->otyp].oc_color == CLR_BLUE) || (uright && objects[uright->otyp].oc_color == CLR_BLUE) || (uimplant && objects[uimplant->otyp].oc_color == CLR_BLUE) || (ublindf && objects[ublindf->otyp].oc_color == CLR_BLUE) || (uarmf && objects[uarmf->otyp].oc_color == CLR_BLUE) || (uarmh && objects[uarmh->otyp].oc_color == CLR_BLUE) || (uamul && uamul->oartifact == ART_______SCORE) || (uarmg && objects[uarmg->otyp].oc_color == CLR_BLUE) || (uarmc && objects[uarmc->otyp].oc_color == CLR_BLUE) || (uarmu && objects[uarmu->otyp].oc_color == CLR_BLUE) || autismringcheck(ART_PAYN_HU) || (uarms && objects[uarms->otyp].oc_color == CLR_BLUE) || (uarm && objects[uarm->otyp].oc_color == CLR_BLUE) || (uwep && objects[uwep->otyp].oc_color == CLR_BLUE) )
...
#define ExtWildWeatherEffect	(EWildWeatherEffect || (uarmh && uarmh->oartifact == ART_SPRING_BREEZE) || (uarmc && uarmc->oartifact == ART_OUT_OF_THE_RAIN) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || (uamul && uamul->oartifact == ART_______SCORE) )
