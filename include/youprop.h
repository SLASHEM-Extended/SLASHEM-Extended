...
#define CanHaveBrownSpells (BrownSpells || u.uprops[BROWN_SPELLS].extrinsic || have_brownspellstone() || (uarmf && uarmf->oartifact == ART_ARABELLA_S_GIRL_KICK) || (uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) || u.arti_carry_bluedaypaper)
...
#define FluctuatingSpeedXtra	((have_fluctuatingspeedstone() == 2) || carryingarti(ART_ANALEAH_S_CONTINUITY) )
...
#define SpellCooldownXtra	((have_spellcooldownstone() == 2) || carryingarti(ART_ALBIE_S_WHITEOUT) )
...
#define Sleep_resistance	( !(YouAreThirsty && (u.uhunger > 2000)) && !(uarmf && uarmf->otyp == SLEEP_INDUCTION_BOOTS) && !Race_if(PM_KOBOLT) && !Race_if(PM_DEEP_ELF) && !carrying(SLEEPSTONE) && !NoSleep_resistance && !hybridragontype(AD_SLEE) && ((IntSleep_resistance && u.nonintrinsicproperty != SLEEP_RES) || (ExtSleep_resistance && u.nonextrinsicproperty != SLEEP_RES)) )
...
#define IntShock_resistance	(HShock_resistance || (uarms && uarms->oartifact == ART_TEH_SHOCK) || ShockImmunity || resists_elec(&youmonst) || is_elec_resistant(youmonst.data) || autismringcheck(ART_DEVINE_S_PRAYER) || (uarmc && uarmc->oartifact == ART_EXTRASHOCK) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FULLPOWER) || carryingarti(ART_EARTHSPARK) || (uarm && uarm->oartifact == ART_EVERLASTING_GEMSTONE) || (powerfulsymbiosis() && is_elec_resistant(&mons[u.usymbiote.mnum])) )
...
#define IntPoison_resistance	(HPoison_resistance || (uball && uball->oartifact == ART_HIGH_AFFIX) || Race_if(PM_SLYER_ALIEN) || resists_poison(&youmonst) || is_poison_resistant(youmonst.data) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarm && uarm->oartifact == ART_GREENHUE) || u.arti_carry_fragrantgirls || (uwep && uwep->oartifact == ART_SYLVIE_S_EASY_MODE) || (uarmf && uarmf->oartifact == ART_POI_SEN_SUX) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_PULSE_NODE_OF_GERALT) || (powerfulsymbiosis() && is_poison_resistant(&mons[u.usymbiote.mnum])) )
...
#define ExtAntimagic	(EAntimagic || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == ANTIMAGIC) ) || (uarmc && uarmc->oartifact == ART_ALL_IN_ONE_ASCENSION_KIT) || (uarmf && uarmf->oartifact == ART_FIND_THE_SECRET_COURSE) || (uarmh && uarmh->oartifact == ART_YOU_ARE_ALREADY_DEAD) || (uwep && uwep->oartifact == ART_ORB_OF_FLEECE) || (uarmf && uarmf->oartifact == ART_NOW_YOU_ARE_HOWEVER_TO) || (carryingarti(ART_ONE_IN_FOUR) && (moves % 4 == 1) ) || (uwep && uwep->oartifact == ART_ARK_OF_THE_COVENANT) || carryingarti(ART_HAVE_IT_ALL_BUT_NOT_GET) || (uchain && uchain->oartifact == ART_TSCHEND_FOR_ETERNITY) || (uarmg && uarmg->oartifact == ART_SPECTRATOR && (moves % 5 == 0) ) || carryingarti(ART_IT_S_MAGIC___) || autismringcheck(ART_CORGON_S_RING) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FULLPOWER) || (uarm && itemhasappearance(uarm, APP_MAGICPROT_ROBE)) || carryingarti(ART_HUNGER_FOR_THE_RES) || (uarm && uarm->oartifact == ART_CHANGERING_ENVIROMENT) || (irissetbonus() >= 3) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarm && uarm->oartifact == ART_WEB_OF_LOLTH) || (uarm && uarm->oartifact == ART_WEB_OF_HOHOTH) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_SLEX_WANTS_YOU_TO_DIE_A_PA) || (uarm && uarm->oartifact == ART_ROFLCOPTER_WEB) || carrying(STONE_OF_MAGIC_RESISTANCE) )
...
#define AcidImmunity		((uarm && uarm->oartifact == ART_NULARMOR) || (uarmf && uarmf->oartifact == ART_LISA_S_YELLOW_STRIPE) || (uarmf && uarmf->oartifact == ART_HEIKE_S_BIGOTRY) || carryingarti(ART_POISONOUS_IVE) || (uwep && uwep->oartifact == ART_PIPPA_S_URGE) || (uarm && uarm->oartifact == ART_PRENTY_TWETEEN) || (uball && uball->oartifact == ART_ACICORNER) || carryingarti(ART_GTWCTTWW) || (uamul && uamul->oartifact == ART_CURSING_SCARAB && uamul->cursed) || (uarmf && uarmf->oartifact == ART_POCAHONTAS_HOME))
...
#define EPrem_death	(u.uprops[PREMDEATH].extrinsic || (uimplant && uimplant->oartifact == ART_THROW_MY_LIFE_AWAY) || (flags.female && uwep && autismweaponcheck(ART_TOMORROW_WENDY_S_CHOICENA)) || carryingarti(ART_RUGGELUU_) || autismringcheck(ART_DUEWUEPI_HAHAHA) || (uarms && uarms->oartifact == ART_YOU_GOTTA_SEPARATE) || (uarmc && uarmc->oartifact == ART_KYS_YOURSELF) )
...
#define FemtrapActiveAnja	(FemaleTrapAnja || u.uprops[FEMTRAP_ANJA].extrinsic || have_femtrapanja() || u.arti_carry_fragrantgirls )
...
#define FemtrapActiveLara	(FemaleTrapLara || u.uprops[FEMTRAP_LARA].extrinsic || have_femtraplara() || (uamul && uamul->oartifact == ART_LARA_S_LAVAWALK) || (uarms && uarms->oartifact == ART_CASTLE_CRUSH_GLITCH) || carryingarti(ART_CLIMXIAN_WORSHIP) || (uarmg && uarmg->oartifact == ART_ELARA_S_AGILITY) )
#define FemtrapActiveAnastasia	(FemaleTrapAnastasia || u.uprops[FEMTRAP_ANASTASIA].extrinsic || have_femtrapanastasia() || (uarmf && uarmf->oartifact == ART_THAT_S_SUPER_UNFAIR) || (uarmf && uarmf->oartifact == ART_SUSCHEL_KUNA) || (uarmf && uarmf->oartifact == ART_ANASTASIA_S_SURPRISE) || (uarmf && uarmf->oartifact == ART_SUNA_S_CONTROVERSY_MATTER) || (uarmf && uarmf->oartifact == ART_PLAYING_ANASTASIA) || (uarmc && uarmc->oartifact == ART_EDNA_S_CALM) || (uarmf && uarmf->oartifact == ART_WHITE_LOAFERS) || (uarmf && uarmf->oartifact == ART_SOFT_COW_DUNG) || (uarmf && uarmf->oartifact == ART_THICK_PLATFORM_CRAZE) || (uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) || (uarmf && uarmf->oartifact == ART_NANCY_S_ASSISTANCE) || (uarmf && uarmf->oartifact == ART_SHE_S_STILL_AN_ANASTASIA) || autismweaponcheck(ART_REAL_WALKING) || (autismweaponcheck(ART_DAMN_SKI_WEDGE) && !uarmf) || (uarmf && uarmf->oartifact == ART_FASTER_THAN_ALL_OTHERS_INT))
#define FemtrapActiveJessica	(FemaleTrapJessica || u.uprops[FEMTRAP_JESSICA].extrinsic || have_femtrapjessica() || (uarmf && uarmf->oartifact == ART_LEGENDARY_JESSICA) || (uarmf && uarmf->oartifact == ART_JESSICA_S_UNFAIR_MODE) || (uarmf && uarmf->oartifact == ART_WEDDING_CHALLENGE))
...
#define FemtrapActiveElena	(FemaleTrapElena || u.uprops[FEMTRAP_ELENA].extrinsic || have_femtrapelena() || carryingarti(ART_ELENA_S_CELEBRATION) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) )
...
#define FemtrapActiveYvonne	(FemaleTrapYvonne || u.uprops[FEMTRAP_YVONNE].extrinsic || have_femtrapyvonne() || carryingarti(ART_COMPLEX_BUILDED) )
...
#define FemtrapActiveMeltem	(FemaleTrapMeltem || u.uprops[FEMTRAP_MELTEM].extrinsic || have_femtrapmeltem() || carryingarti(ART_MAGICRYSTAL) )
...
#define FemtrapActiveDora	(FemaleTrapDora || u.uprops[FEMTRAP_DORA].extrinsic || have_femtrapdora() || (uarmh && uarmh->oartifact == ART_ARABELLA_S_BEAUTY_BIRD) || autismweaponcheck(ART_DESERT_MAID) || (uarmf && uarmf->oartifact == ART_DAISY_S_PRINCESS_BOOTS) || (uarmf && uarmf->oartifact == ART_ALMUT_S_NOCARE) || (uarmf && uarmf->oartifact == ART_SHE_REALLY_LIKES_IT) || (uarm && uarm->oartifact == ART_TIMONA_S_INNER_BICKER) )
...
#define FemtrapActiveVerena	(FemaleTrapVerena || u.uprops[FEMTRAP_VERENA].extrinsic || have_femtrapverena() || carryingarti(ART_PRETTYBLOND) || (uarm && uarm->oartifact == ART_ALEIT_GIVESITNOT) || (uarmh && uarmh->oartifact == ART_IGNORANCE_IS_BLISS))
...
#define FemtrapActiveNora	(FemaleTrapNora || u.uprops[FEMTRAP_NORA].extrinsic || have_femtrapnora() || carryingarti(ART_IT_S_HER_DISEASE) || (youmonst.data->msound == MS_BULIMIA) )
...
#define FemtrapActiveUte	(FemaleTrapUte || u.uprops[FEMTRAP_UTE].extrinsic || have_femtrapute() || autismringcheck(ART_DEVINE_S_PRAYER) || carryingarti(ART_CRASH_RECOVERY) || autismweaponcheck(ART_DE_SID))
#define FemtrapActiveJasieen	(FemaleTrapJasieen || u.uprops[FEMTRAP_JASIEEN].extrinsic || have_femtrapjasieen())
#define FemtrapActiveYasaman	(FemaleTrapYasaman || u.uprops[FEMTRAP_YASAMAN].extrinsic || have_femtrapyasaman() || bmwride(ART_KERSIE_PERSIE) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || u.arti_carry_fragrantgirls || (uarms && uarms->oartifact == ART_FROM_MASHHAD_TO_AHVAZ) )
...
#define ExtHalluc_resistance	(EHalluc_resistance || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == HALLUC_RES) ) || u.arti_carry_allislost || (Race_if(PM_BATMAN) && uwep && uwep->oartifact == ART_BLACKSWANDIR))
...
#define FunnyHallu		(Role_if(PM_GOFF) || (uamul && uamul->oartifact == ART_NARCOTIC_TRADE) || (uarmc && uarmc->oartifact == ART_BE_HI_WITHOUT_DRUGS) || RngeFunnyHallu || ishallucinator || HHallucination || (uarmf && uarmf->oartifact == ART_LUSTY_GIGGLE) || u.uprops[MULTISHOES].extrinsic || u.funnyhallutimer || autismweaponcheck(ART_FADED_USELESSNESS) || autismweaponcheck(ART_LANCE_OF_LONGINUS) || (uamul && uamul->oartifact == ART_FUN_ALL) || u.arti_carry_leoishigh || autismweaponcheck(ART_SCHWANZUS_LANGUS) || HeavyHallu || EHallucination || u.uprops[SENSORY_DEPRIVATION].extrinsic || (u.funnyhalluroll != 9999999 && u.usanity > u.funnyhalluroll) || flags.hippie || ( (u.uprops[DEHYDRATION].extrinsic || Dehydration || have_dehydratingstone()) && ((u.dehydrationtime - moves) < 1) )  )
...
#define IntSee_invisible	(HSee_invisible || (uarms && uarms->oartifact == ART_EDIVA_S_SEE_THROUGH) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FLICKERING_OF_OZONE) || autismringcheck(ART_FUBBLE) || carryingarti(ART_NIXBRINGING) || perceives(youmonst.data))
#define ExtSee_invisible	(ESee_invisible || (uarms && uarms->oartifact == ART_EDIVA_S_SEE_THROUGH) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (ublindf && ublindf->oartifact == ART_EYES_OF_THE_ORACLE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_FLICKERING_OF_OZONE) || (uarmh && uarmh->oartifact == ART_ETERNAL_EYE) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (ublindf && ublindf->oartifact == ART_LENSES_OF_TRUTH) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || carryingarti(ART_SYLVIE_S_SEE) || (uarmc && uarmc->oartifact == ART_OF_THE_ETHEREAL_EYE) || (uwep && uwep->oartifact == ART_MAUSER_C___CUSTOM) || (uwep && uwep->oartifact == ART_EORLINGAS) || carryingarti(ART_SMILEYSEE) || (uarm && uarm->oartifact == ART_AVELINE_S_ROBE_OF_THE_FIRS) || (uwep && uwep->oartifact == ART_LUCKY_DAGGER) || (uwep && uwep->oartifact == ART_THIN_DAGGER) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmh && uarmh->oartifact == ART_SEENBC) || carryingarti(ART_NIXBRINGING) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmh && uarmh->oartifact == ART_GREGOR_S_SENTINEL_HELM) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define ExtWonderlegs	(EWonderlegs || carryingarti(ART_GRIFTED_CAPABILITY) || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || (uwep && uwep->oartifact == ART_PLANTAR_NO) || (uwep && uwep->oartifact == ART_DREAMOULE_ELEYELE) || (uarmf && uarmf->oartifact == ART_STAR_SOLES))
...
#define IntWarning	(HWarning || u.arti_carry_redbuttonflash || autismringcheck(ART_CLOTOBBLE) )
...
#define IntDisplaced	(HDisplaced || carryingarti(ART_WHERESIT) || (uarmh && uarmh->oartifact == ART_NIAMH_PENH && isok(u.ux, u.uy) && IS_TREE(levl[u.ux][u.uy].typ)) )
#define ExtDisplaced	(EDisplaced || carryingarti(ART_WHERESIT) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DISPLACED) ) || (uarmc && uarmc->oartifact == ART_IMAGE_PROJECTOR) || (uwep && uwep->oartifact == ART_SKY_RENDER) || (uarmh && uarmh->oartifact == ART_NIAMH_PENH && isok(u.ux, u.uy) && IS_TREE(levl[u.ux][u.uy].typ)) || (uarm && uarm->oartifact == ART_VERY_INVISIBLE) || (uarmf && uarmf->oartifact == ART_COVER_UP) || (uwep && uwep->oartifact == ART_LAUGHTERSNEE) || (uwep && uwep->oartifact == ART_STAFF_OF_TWELVE_MIRRORS) || (uarm && uarm->oartifact == ART_NINJUTSU) || (uwep && uwep->oartifact == ART_CHRIS_S_SPIGGL) || (uarmg && uarmg->oartifact == ART_STORMING_FISTS) || carryingarti(ART_ARE_YOU_MAN_ENOUGH_) || (uarmf && uarmf->oartifact == ART_LIORA_S_BOOTS_OF_MIRAGE_ST) || (uarmc && uarmc->oartifact == ART_DISS_PLACE) || (uarmc && uarmc->oartifact == ART_MESSEN_PLES) || autismringcheck(ART_WINGEL_AROUND) || (uwep && uwep->oartifact == ART_SESDERIRO) || (uarmc && uarmc->oartifact == ART_MATTHIAS__CLOAK_OF_TEN_SHA) || (uarmf && uarmf->oartifact == ART_WATER_FLOWERS) || (uwep && uwep->oartifact == ART_BELTHRONDING) || bmwride(ART_JOCK_S_SPECPLACE) || (uarms && uarms->oartifact == ART_EXTERIOUS_PELT) || (uarms && uarms->oartifact == ART_MS__PLACE) || (uarm && uarm->oartifact == ART_ROBE_OF_THE_ARCHMAGI) || (uarmh && uarmh->oartifact == ART_MARLENA_S_SONG) || (uarm && uarm->oartifact == ART_SHIVANHUNTER_S_UNUSED_PRIZ) || (uamul && uamul->oartifact == ART_MISPLACED_GLEAM) || (uarmc && itemhasappearance(uarmc, APP_SHROUDED_CLOAK) && (moves % 10 == 0) ) || ((moves % 3 == 0) && uarmc && itemhasappearance(uarmc, APP_DEEP_CLOAK) ) || (uarmf && itemhasappearance(uarmf, APP_KOREAN_SANDALS) && (moves % 3 == 0) ) )
...
#define IntTeleport_control	(HTeleport_control || carryingarti(ART_SKRIIIIIIIIIIIIIE_) || autismringcheck(ART_TO_JUDITHBIS) || (u.martialstyle == MARTIALSTYLE_BOOYAKASHA && uwep && uwep->otyp >= BRASS_KNUCKLES && uwep->otyp <= ELITE_KNUCKLES) || (uleft && objects[uleft->otyp].oc_material == MT_MENGETIUM) || control_teleport(youmonst.data))
...
#define ExtLevitation	(ELevitation || carryingarti(ART_FLIWITEERER) || (uarmf && uarmf->oartifact == ART_FERGIE_S_GLACIER && uwep && (weapon_type(uwep) == P_QUARTERSTAFF) ) || (uarmf && uarmf->oartifact == ART_ULTRA_P) || autismringcheck(ART_FLOAT_EYELER_S_CONDITION) || autismweaponcheck(ART_IRON_BALL_OF_LEVITATION) || autismweaponcheck(ART_XIUHCOATL) || u.uprops[MULTISHOES].extrinsic)
...
#define Lev_at_will		(!Race_if(PM_LEVITATOR) && !carryingarti(ART_FLIWITEERER) && ((HLevitation & I_SPECIAL) != 0L || \
...
#define ExtEnergy_regeneration	(EEnergy_regeneration || carryingarti(ART_COMPLEX_BUILDED) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CORE_HALO_OF_AURELION_THE_) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_NEURAL_THREAD_OF_GALADRIEL) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == ENERGY_REGENERATION) ) )
...
#define NoPolymorph_control	( (!Race_if(PM_IMMUNIZER) && u.uprops[DEAC_POLYMORPH_CONTROL].intrinsic) || UHaveAids || (uarmc && uarmc->oartifact == ART_PERMANENTITIS) || (uarmf && uarmf->oartifact == ART_CLONE_) || carryingarti(ART_US_STAYS_NO_SELECTION) || (flags.female && (u.genitalhealth_f < 1)) || (uarmc && uarmc->oartifact == ART_SHAPETAKE_NUMBER_FIVE) || (u.impossibleproperty == POLYMORPH_CONTROL) || (uarmh && uarmh->oartifact == ART_RANDOMNESS_PREVAILS) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0) )
...
#define IntFree_action	(HFree_action || (tech_inuse(T_SEXY_STAND) && PlayerInBlockHeels) || (uwep && uwep->oartifact == ART_YES_OCCIFER) || FemtrapActiveNatalje || (uarmg && uarmg->oartifact == ART_SIRINE_S_MELLOW_LOOK) || carryingarti(ART_LEGENDARIUM_PERCENTAGE) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CLEAN_ASCENSION_RUN && In_endgame(&u.uz)) )
...
#define IntPsi_resist	(HPsi_resist || (uarmh && uarmh->oartifact == ART_SING_AND_JULIETTA_S_JOINT_) || (uarmh && uarmh->oartifact == ART_ALIEN_MINDWAVE) || (uarmh && objects[uarmh->otyp].oc_material == MT_MENGETIUM) || carryingarti(ART_CASTLEPSYCHE) || (uarmh && uarmh->oartifact == ART_SPIREHEAD) || (Upolyd && attackdamagetype(youmonst.data, AT_BREA, AD_SPC2) ))
...
#define IntUseTheForce	(HUseTheForce || carryingarti(ART_LUKE_S_BLUE_LUMEN) )
#define ExtUseTheForce	(EUseTheForce || carryingarti(ART_LUKE_S_BLUE_LUMEN) || (uarm && uarm->oartifact == ART_SUSA_MAIL) || (uwep && uwep->oartifact == ART_THAIFORCE) || (uwep && uwep->otyp == DOUBLE_FORCE_BLADE) || (uwep && uwep->otyp == FORCE_WHIP) || (uarmg && uarmg->oartifact == ART_LONGHAND) || (uarm && uarm->oartifact == ART_HOW_DO_YOU_COME_UP_WITH___) || carryingarti(ART_BLACKTHRUST) || (uwep && uwep->oartifact == ART_HENRIETTENFORCE) || autismringcheck(ART_LONGHAND_JEDI) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == THE_FORCE) ) || (uarm && uarm->oartifact == ART_JEDE_BLADE_HOLD) || (uarm && uarm->oartifact == ART_LUKE_S_JEDI_POWER))
...
#define IntDefusing	(HDefusing || carryingarti(ART_I_LL_MURDER_EVERYONE_) || (uarmc && uarmc->oartifact == ART_ARABELLA_S_SAFETY_EQUIPMEN) || (uarmh && uarmh->oartifact == ART_SEEKDESTRUCT) || (uarmg && objects[uarmg->otyp].oc_material == MT_MENGETIUM) )
#define ExtDefusing	(EDefusing || (uarmh && uarmh->oartifact == ART_SEEKDESTRUCT) || (uarmf && uarmf->oartifact == ART_GHETTO_ROCKZ) || (uarm && uarm->oartifact == ART_ARMS_OVERSIGHT) || (uwep && uwep->oartifact == ART_GO_FULLY_OFF) || (uarm && uarm->oartifact == ART_ENCHANTED__OF_DEFUSING) || (uarm && uarm->oartifact == ART_BEEPA_DEVICE) || carryingarti(ART_I_LL_MURDER_EVERYONE_) || (uamul && uamul->oartifact == ART_SOIL_MAINTENANCE) || (uwep && uwep->oartifact == ART_HALF_SUPER_ENCHANTMENT) || (uarmc && uarmc->oartifact == ART_WIDERGO) || (uarmg && uarmg->oartifact == ART_SABRINA_S_UTILITY_GLOVES) || (uarmg && uarmg->oartifact == ART_BRIGITTE_S_SAFETY_MITTENS) || (uarmc && uarmc->oartifact == ART_ARABELLA_S_SAFETY_EQUIPMEN) || carryingarti(ART_BACKHISTORY_REMEMBERANCE) || (uarm && uarm->oartifact == ART_METALPULL) || (uamul && uamul->oartifact == ART_______SCORE) || (uamul && uamul->oartifact == ART_ARVIAT_S_LOAD) || (ublindf && ublindf->otyp == DEFUSING_BOX) || (uarmf && uarmf->oartifact == ART_MARC_S_SOMEWHAT_EQUIP) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DEFUSING) ) )
...
#define ExtBurdenedState	(EBurdenedState || (uimplant && uimplant->oartifact == ART_LUNG_INFUSER_OF_LINK) || (uarm && uarm->oartifact == ART_FUCK_UGGHH_THAT_S_HEAVY_) || (uarms && uarms->oartifact == ART_ITS_NO_GOOD) || carryingarti(ART_COMPLEX_BUILDED) || autismweaponcheck(ART_INDOBURDENIA_VIRII) || (uamul && uamul->oartifact == ART_EVERY_OTHER_AMULET_IS_THIS) || (uarmg && uarmg->oartifact == ART_SUPERHEAVYKLONK) )
...
#define ExtMagicVacuum	(EMagicVacuum || (uarm && uarm->oartifact == ART_FINICKYMAG) || autismringcheck(ART_SPEECHBREAK) || autismweaponcheck(ART_SHADOWBLADE_BASED_ON_STORM) || autismringcheck(ART_ARABELLA_S_NASTYGUARD) || carryingarti(ART_COMPLEX_BUILDED) || u.tempspelllossmagvac || (uarmc && uarmc->oartifact == ART_FEND_FOR_YOURSELF) || autismweaponcheck(ART_MCCAULEY_S_ARGUMENT) || (uarmc && uarmc->oartifact == ART_OLD_PERSON_TALK) || autismweaponcheck(ART_ARABELLA_S_THINNER) || (uwep && uwep->otyp == VACUUM_STAFF) || (u.twoweap && uswapwep && uswapwep->otyp == VACUUM_STAFF) )
