...
#define NoCold_resistance	( (!Race_if(PM_IMMUNIZER) && u.uprops[DEAC_COLD_RES].intrinsic) || UHaveAids || (u.impossibleproperty == COLD_RES) || autismweaponcheck(ART_FLARE_FLAKE) || (uarms && uarms->oartifact == ART_HEATEM_UP) || (uarmc && uarmc->oartifact == ART_WOAH_FREEZE_) || autismringcheck(ART_DEATHLY_COLD) || (uamul && uamul->oartifact == ART_CURSING_PENDANT && !uamul->cursed) || autismringcheck(ART_BERNCELD) || Race_if(PM_PLAYER_SALAMANDER) || Race_if(PM_GAVIL) || Race_if(PM_JAVA) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0) )

#define ColdImmunity		((uwep && uwep->oartifact == ART_CUTRELEASE) || (uchain && uchain->oartifact == ART_HEI_THE_COLD) || (uarmf && uarmf->oartifact == ART_FLEEC_LIE) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_LOIS_S_CHILL) || (uarmf && uarmf->oartifact == ART_BRIGHT_WHITE) || (uwep && uwep->oartifact == ART_COLD_HAHA) || (uarmg && uarmg->oartifact == ART_MERYN_S_PEAK) || autismringcheck(ART_ARCTIC_EXPEDITE) || (uarmh && uarmh->oartifact == ART_CHARLENE_S_CORONA) || (uamul && uamul->oartifact == ART_CURSING_PENDANT && uamul->cursed) || (uarmc && uarmc->oartifact == ART_FLEECY_CORN) || (uwep && uwep->oartifact == ART_GOOTE_UTE) || u.cryopancaketurns || (uwep && uwep->oartifact == ART_COOLSHIELDING) || (uwep && uwep->oartifact == ART_WINTERHOLD_S_KEEP))

#define HSleep_resistance	u.uprops[SLEEP_RES].intrinsic
#define ESleep_resistance	u.uprops[SLEEP_RES].extrinsic
#define IntSleep_resistance	(HSleep_resistance || (uarm && uarm->oartifact == ART_FUCK_UGGHH_THAT_S_HEAVY_ && (u.uhp < (u.uhpmax / 10)) ) || FemtrapActiveNatalje || resists_sleep(&youmonst) || is_sleep_resistant(youmonst.data) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CLEAN_ASCENSION_RUN && In_endgame(&u.uz)) || (powerfulsymbiosis() && is_sleep_resistant(&mons[u.usymbiote.mnum])) )
...
#define IntShock_resistance	(HShock_resistance || (uarms && uarms->oartifact == ART_TEH_SHOCK) || ShockImmunity || resists_elec(&youmonst) || is_elec_resistant(youmonst.data) || (uarm && uarm->oartifact == ART_EVERLASTING_GEMSTONE) || (powerfulsymbiosis() && is_elec_resistant(&mons[u.usymbiote.mnum])) )
...
#define ShockImmunity		(Race_if(PM_PLAYER_DYNAMO) || (uamul && uamul->oartifact == ART_CURSING_BROOCH && uamul->cursed) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarm && uarm->oartifact == ART_EVERLASTING_GEMSTONE && uarm->otyp == DIAMOND_DRAGON_SCALES) || autismringcheck(ART_SOIL_REALITY) || (uarms && uarms->oartifact == ART_TEH_SHOCK) || (uarmg && uarmg->oartifact == ART_ENNUI_STOICISM) || (uarms && uarms->oartifact == ART_GROUNDED_FOREVER && objects[uarms->otyp].oc_material == MT_WOOD) )

#define HPoison_resistance	u.uprops[POISON_RES].intrinsic
#define EPoison_resistance	u.uprops[POISON_RES].extrinsic
#define IntPoison_resistance	(HPoison_resistance || (uball && uball->oartifact == ART_HIGH_AFFIX) || Race_if(PM_SLYER_ALIEN) || resists_poison(&youmonst) || is_poison_resistant(youmonst.data) || (powerfulsymbiosis() && is_poison_resistant(&mons[u.usymbiote.mnum])) )
...
#define NoPoison_resistance	( (!Race_if(PM_IMMUNIZER) && u.uprops[DEAC_POISON_RES].intrinsic) || UHaveAids || (uarmc && uarmc->oartifact == ART_ASTRO_LABYRINTH) || (u.impossibleproperty == POISON_RES) || autismweaponcheck(ART_SWORD_OF_BHELEU) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0) )
...
#define NoSick_resistance	( (!Race_if(PM_IMMUNIZER) && u.uprops[DEAC_SICK_RES].intrinsic) || (uarmu && uarmu->oartifact == ART_RADIOLAN) || UHaveAids || (u.impossibleproperty == SICK_RES) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0) )
...
#define Numbed		((HNumbed || ENumbed || Race_if(PM_IRRITATOR) || (autismweaponcheck(ART_GAYGUN) && uarmg && (u.homosexual != 1)) || (uarmc && uarmc->oartifact == ART_DANOISE_S_LEAK) || (uarms && uarms->oartifact == ART_NUMBED_CAN_T_DO) || HeavyNumbed) && !(CannotBeNumbed && !Race_if(PM_IRRITATOR)) )
...
#define FemtrapActiveJette	(FemaleTrapJette || u.uprops[FEMTRAP_JETTE].extrinsic || have_femtrapjette() || Race_if(PM_ARTIFINDER) || autismweaponcheck(ART_HER_UNREACHABLE_BROOK) || (uarm && uarm->oartifact == ART_WHOA_THAT_S_THE_ONE_) || (uarmu && uarmu->oartifact == ART_HEEEEELEEEEEN && !Role_if(PM_ACTIVISTOR)) )
#define FemtrapActiveIna	(FemaleTrapIna || u.uprops[FEMTRAP_INA].extrinsic || have_femtrapina() || (uarmc && uarmc->oartifact == ART_INA_S_APPRENTICESHIP) )
...
#define FemtrapActiveAntje	(FemaleTrapAntje || u.uprops[FEMTRAP_ANTJE].extrinsic || have_femtrapantje() || (uarm && uarm->oartifact == ART_MEET_WOMAN_ANTJE) || (uarmf && uarmf->oartifact == ART_FANNY_S_ANNOYANCE) || (uarmu && uarmu->oartifact == ART_SETH_S_ERROR) )
...
#define FemtrapActiveKerstin	(FemaleTrapKerstin || u.uprops[FEMTRAP_KERSTIN].extrinsic || have_femtrapkerstin() || autismweaponcheck(ART_EN_GARDE____TOUCHE_) || (uarmf && uarmf->oartifact == ART_KERSTIN_S_LITTLE_HELP) || (uarmh && uarmh->oartifact == ART_WHO_CAN_DRIVE_BEST_) || bmwride(ART_KERSTIN_S_COWBOY_BOOST) || autismweaponcheck(ART_GONDOLIN_S_HIDDEN_PASSAGE) )
...
#define FemtrapActiveInge	(FemaleTrapInge || u.uprops[FEMTRAP_INGE].extrinsic || have_femtrapinge() || (uamul && uamul->oartifact == ART_______SCORE) )
...
#define FemtrapActiveKlara	(FemaleTrapKlara || u.uprops[FEMTRAP_KLARA].extrinsic || have_femtrapklara() || (uamul && uamul->oartifact == ART_LARA_S_LAVAWALK) || (uarm && uarm->oartifact == ART_KUSE_MUSE) )
...
#define IntSee_invisible	(HSee_invisible || autismringcheck(ART_FUBBLE) || perceives(youmonst.data))
#define ExtSee_invisible	(ESee_invisible || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SEE_INVIS) ) || (uarmc && uarmc->oartifact == ART_VISIBLE_INVISIBILITITY) || (uarmh && uarmh->oartifact == ART_CHRISTMAS_MEDAL) || (uarmh && uarmh->oartifact == ART_APOTHEOSIS_VEIL) || (uwep && uwep->oartifact == ART_GONDOLIN_S_HIDDEN_PASSAGE) || (uwep && uwep->oartifact == ART_MIRACLE_PAINT) || (uimplant && uimplant->oartifact == ART_CORONATION_CULMINATION) || (ublindf && ublindf->oartifact == ART_EYES_OF_THE_ORACLE) || (uarmh && uarmh->oartifact == ART_ETERNAL_EYE) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (ublindf && ublindf->oartifact == ART_LENSES_OF_TRUTH) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uarmc && uarmc->oartifact == ART_OF_THE_ETHEREAL_EYE) || (uwep && uwep->oartifact == ART_MAUSER_C___CUSTOM) || (uwep && uwep->oartifact == ART_EORLINGAS) || (uwep && uwep->oartifact == ART_LUCKY_DAGGER) || (uwep && uwep->oartifact == ART_THIN_DAGGER) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uwep && uwep->oartifact == ART_LONG_SWORD_OF_ETERNITY) || (uwep && uwep->oartifact == ART_MAGE_STAFF_OF_ETERNITY) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uimplant && uimplant->oartifact == ART_POTATOROK) || (uarmf && uarmf->oartifact == ART_JULIA_S_SLIPPERS) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarmc && uarmc->oartifact == ART_CAN_T_KILL_WHAT_YOU_CAN_T_) || (uwep && uwep->oartifact == ART_SECRETS_OF_INVISIBLE_PLEAS))
...
#define IntTelepat	(HTelepat || autismringcheck(ART_SET_DEFPARM) || (uarmf && uarmf->oartifact == ART_GLITCH_IN_THE_MONITOR) || (uarmf && uarmf->oartifact == ART_SCAN_ME) || (uarmh && uarmh->oartifact == ART_SUPERPATHY) || Race_if(PM_DEVELOPER) || telepathic(youmonst.data) || (Role_if(PM_TOPMODEL) && PlayerInHighHeels) )
#define ExtTelepat	(ETelepat || (uarmf && uarmf->oartifact == ART_SCAN_ME) || (uamul && uamul->oartifact == ART_AMULET_OF_SPLENDOR) || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (uchain && uchain->oartifact == ART_IZQUXXISQUID_S_RESEARCH) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || Race_if(PM_DEVELOPER))
...
#define ExtFear_factor	(EFear_factor || (uarmf && uarmf->oartifact == ART_AERES_RUNNING_SHOES) || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (uarmf && uarmf->oartifact == ART_ELENA_S_EPITOME) || (uarmc && uarmc->oartifact == ART_ARABELLA_S_FEAR) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || (uarmf && uarmf->oartifact == ART_FAR_EAST_RELATION) || (uarmf && uarmf->oartifact == ART_LISSIE_S_SHEAGENTUR))
...
#define IntStealth	(HStealth || autismringcheck(ART_XANATHAR_S_RING_OF_PROOF) || (uarmc && uarmc->oartifact == ART_STEALTH_OR_STEALING) || (uarmc && uarmc->oartifact == ART_WHISPERWIND_CLOAK) || (uarm && uarm->oartifact == ART_HIDDEN_OPERATION) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_RESTROOM_DRENCHING) || (Race_if(PM_MOON_ELF) && ((flags.moonphase >= 1 && flags.moonphase <= 3) || autismringcheck(ART_METAL_GEAR_NETHACK) || (uwep && uwep->oartifact == ART_SUPERSTAB) || (flags.moonphase >= 5 && flags.moonphase <= 7))) )
...
#define ExtAstral_vision	(EAstral_vision || (uarmc && uarmc->oartifact == ART_ASTRO_LABYRINTH) || (uarms && uarms->oartifact == ART_INDEWALBE) || (irissetbonus() >= 4) || (uchain && uchain->oartifact == ART_SIYID) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_IRON_OF_INNERMOST_JOY) || (uwep && uwep->oartifact == ART_ASTRALTOR_TSCHH && uwep->lamplit) || (uwep && uwep->oartifact == ART_ASTRAL_LIGHTWELL && uwep->lamplit) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == ASTRAL_VISION) ) )
...
#define IntRegeneration	(HRegeneration || (uarmg && uarmg->oartifact == ART_ILLNESS_HEAL) || (uarmg && uarmg->oartifact == ART_GAUNTLETS_OF_THE_HEALING_H) || autismringcheck(ART_RING_OF_HYGIENE_S_DISCIPLE) || (uwep && uwep->oartifact == ART_MAEVE_S_OPULENCE) || (uarm && uarm->oartifact == ART_BASTLE_BACK_UP) || (uarmh && uarmh->oartifact == ART_HABIBA_S_MATRONAGE) || regenerates(youmonst.data))
...
#define ExtPolymorph_control	(EPolymorph_control || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == POLYMORPH_CONTROL) ) || (uarmu && uarmu->oartifact == ART_VICTORIA_IS_EVIL_BUT_PRETT) || (uarms && uarms->oartifact == ART_SHEP_O_GLO) || (uamul && uamul->oartifact == ART_UNPOLYVERSE) || (uarmu && uarmu->oartifact == ART_NATALIA_IS_LOVELY_BUT_DANG) || (uchain && uchain->oartifact == ART_BABA_YAGA_S_TETHER) || (uwep && uwep->oartifact == ART_GRANDLEON) || (uamul && uamul->oartifact == ART_AMULET_OF_SPLENDOR) || autismringcheck(ART_HYPOCRITICAL_FUN) || (uwep && uwep->oartifact == ART_UNICORN_DRILL) || autismringcheck(ART_WINDER_SPECCER) || (uarmf && uarmf->oartifact == ART_MALENA_S_LADYNESS) || (uimplant && uimplant->oartifact == ART_YES_YOU_CAN) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_SIGNIFICANT_RNG_JITTER) || (uarmh && uarmh->oartifact == ART_DICTATORSHIP) )
...
#define NoReflecting	( (!Race_if(PM_IMMUNIZER) && u.uprops[DEAC_REFLECTING].intrinsic) || UHaveAids || (uarm && uarm->oartifact == ART_ARMOR_OF_EREBOR && (moves % 4 == 0) ) || (u.impossibleproperty == REFLECTING) || (uarmh && uarmh->oartifact == ART_DISINT_BEAM) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0) )
...
#define SustainAbilityOn	(Fixed_abil || Race_if(PM_SUSTAINER) || (uarmh && uarmh->oartifact == ART_SLIPPING_SUCKING) || (uarms && uarms->oartifact == ART_SYSTEMATIC_CHAOS) || (uarmc && uarmc->oartifact == ART_UNWERTH) || (uarms && uarms->oartifact == ART_NO_REDUCTION_ALLOWED) || (uarmf && uarmf->oartifact == ART_NO_LITTLE_BIT_OF_WISDOM) || (uarms && uarms->oartifact == ART_THAT_IS_SO_SUS) || (uimplant && uimplant->oartifact == ART_HIGHERHOLDER) || (uarm && uarm->oartifact == ART_POISUSTAIN) || (uarms && uarms->oartifact == ART_BONUS_HOLD) || (uamul && uamul->oartifact == ART_FIX_EVERYTHING) || (uarmf && uarmf->oartifact == ART_ELENETTES))
...
#define IntConf_resist	(HConf_resist || (uarmc && uarmc->oartifact == ART_TEAM_BRIDE) || Race_if(PM_EROSATOR) || tech_inuse(T_STAT_RESIST) || FemtrapActiveClaudia )
#define ExtConf_resist	(EConf_resist || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == CONF_RES) ) || (uarmf && uarmf->oartifact == ART_BOOTS_OF_THE_MACHINE) || (uwep && uwep->oartifact == ART_LAUGHTERSNEE) || (uwep && uwep->oartifact == ART_OGRESMASHER____) || (uarmg && uarmg->oartifact == ART_EGASSO_S_GIBBERISH) || (uarmf && uarmf->oartifact == ART_KRAWASAKI_STEERER) || (uwep && uwep->oartifact == ART_BLOOD_MOON) || (uwep && uwep->oartifact == ART_BLOODY_MACE) || (uarmh && uarmh->oartifact == ART_EFGOURD) || (uarmh && uarmh->oartifact == ART_CAP_OF_THENGEL) || (uarmf && uarmf->oartifact == ART_CRASHING_YOUR_SISTER_S_WED) || (uarmf && uarmf->otyp == CATWALK_SANDALS) || (uarmh && uarmh->oartifact == ART_SEE_THE_TOTAL) || (uwep && uwep->oartifact == ART_SEA_ANGERANCHOR) || (uwep && uwep->oartifact == ART_FUNE_NO_IKARI) || (uarmc && uarmc->oartifact == ART_ORTED_SHAPE) || (uarmf && uarmf->oartifact == ART_PURPLE_SANDSTORM) || (uarmh && uarmh->oartifact == ART_CLAUDIA_S_DIRECTION_FINDER) || (uwep && uwep->oartifact == ART_SHINING_SILVER_KATANA) || (uwep && uwep->oartifact == ART_ZANTETSU) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (uarmh && uarmh->oartifact == ART_SAGE_S_HELM) || (uarmf && uarmf->oartifact == ART_SABRINA_S_FREEDOM) || (uarmh && uarmh->oartifact == ART_ENFORCED_MIND) || (uchain && uchain->oartifact == ART_HIMMELIE_S_STRAWBERRY && u.ualign.type == A_LAWFUL) || (uwep && uwep->oartifact == ART_HIMMELIE_S_STRAWBERRY && u.ualign.type == A_LAWFUL) || (uarm && uarm->oartifact == ART_GLARINGADERNADIR) || (uarmh && uarmh->oartifact == ART_ENSNARED_MIND) || (uamul && uamul->oartifact == ART_STAR_OF_HYPERNOTUS) || (uarmg && uarmg->oartifact == ART_GLOVES_OF_VESDA) || (uwep && uwep->oartifact == ART_SPEAR_OF_PEACE) || (uimplant && uimplant->oartifact == ART_SARREFREE) || (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uarmf && uarmf->oartifact == ART_CLONE_) || (uarmf && uarmf->oartifact == ART_UNFELLABLE_TREE && u.burrowed) || (uarmc && uarmc->oartifact == ART_TEAM_BRIDE) || (uarmf && uarmf->oartifact == ART_SANDRA_S_BEAUTIFUL_FOOTWEA) || (uarmf && uarmf->oartifact == ART_SARAH_S_GRANNY_WEAR) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY) || (uarmf && uarmf->oartifact == ART_SARAH_S_SNEAKERS_OF_INSTAN) || (uarmu && uarmu->oartifact == ART_TOTAL_CONTROL) || (uarmc && uarmc->oartifact == ART_FREQUENT_BUT_WEAK_STATUS) )
...
#define IntPsi_resist	(HPsi_resist || (uarmh && uarmh->oartifact == ART_SING_AND_JULIETTA_S_JOINT_) || (uarmh && uarmh->oartifact == ART_ALIEN_MINDWAVE) || (uarmh && objects[uarmh->otyp].oc_material == MT_MENGETIUM) || (Upolyd && attackdamagetype(youmonst.data, AT_BREA, AD_SPC2) ))
...
#define ExtExpBoost	(EExpBoost || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == EXP_BOOST) ) || (uarmf && uarmf->oartifact == ART_NEXT_LEVEL) || have_minimejewel() || autismringcheck(ART_YEEEEEEAH__EXP_SHRINE_) || autismringcheck(ART_SET_DEFPARM) || (uwep && uwep->oartifact == ART_DOUBLE_MYSTERY) || autismringcheck(ART_RING_OF_FAST_LIVING))
...
#define ExtPainSense	(EPainSense || (uarmf && uarmf->oartifact == ART_CUTE_JEANS) || have_feeldasag() || (uwep && uwep->oartifact == ART_TANKFRIEND) || (uamul && objects[uamul->otyp].oc_color == CLR_BLUE) || (uleft && objects[uleft->otyp].oc_color == CLR_BLUE) || (uright && objects[uright->otyp].oc_color == CLR_BLUE) || (uimplant && objects[uimplant->otyp].oc_color == CLR_BLUE) || (ublindf && objects[ublindf->otyp].oc_color == CLR_BLUE) || (uarmf && objects[uarmf->otyp].oc_color == CLR_BLUE) || (uarmh && objects[uarmh->otyp].oc_color == CLR_BLUE) || (uamul && uamul->oartifact == ART_______SCORE) || (uarmg && objects[uarmg->otyp].oc_color == CLR_BLUE) || (uarmc && objects[uarmc->otyp].oc_color == CLR_BLUE) || (uarmu && objects[uarmu->otyp].oc_color == CLR_BLUE) || autismringcheck(ART_PAYN_HU) || (uarms && objects[uarms->otyp].oc_color == CLR_BLUE) || (uarm && objects[uarm->otyp].oc_color == CLR_BLUE) || autismringcheck(ART_SENSELBLUE) || (uwep && objects[uwep->otyp].oc_color == CLR_BLUE) )
...
#define IntDefusing	(HDefusing || (uarmh && uarmh->oartifact == ART_SEEKDESTRUCT) || (uarmg && objects[uarmg->otyp].oc_material == MT_MENGETIUM) )
#define ExtDefusing	(EDefusing || (uarmh && uarmh->oartifact == ART_SEEKDESTRUCT) || (uarmf && uarmf->oartifact == ART_GHETTO_ROCKZ) || (uarm && uarm->oartifact == ART_ARMS_OVERSIGHT) || (uwep && uwep->oartifact == ART_GO_FULLY_OFF) || (uarm && uarm->oartifact == ART_ENCHANTED__OF_DEFUSING) || (uarm && uarm->oartifact == ART_BEEPA_DEVICE) || (uamul && uamul->oartifact == ART_SOIL_MAINTENANCE) || (uwep && uwep->oartifact == ART_HALF_SUPER_ENCHANTMENT) || (uarmc && uarmc->oartifact == ART_WIDERGO) || (uarm && uarm->oartifact == ART_METALPULL) || (uamul && uamul->oartifact == ART_______SCORE) || (uamul && uamul->oartifact == ART_ARVIAT_S_LOAD) || (ublindf && ublindf->otyp == DEFUSING_BOX) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == DEFUSING) ) )
...
#define ExtBurdenedState	(EBurdenedState || (uarm && uarm->oartifact == ART_FUCK_UGGHH_THAT_S_HEAVY_) || (uarms && uarms->oartifact == ART_ITS_NO_GOOD) || autismweaponcheck(ART_INDOBURDENIA_VIRII) || (uamul && uamul->oartifact == ART_EVERY_OTHER_AMULET_IS_THIS) || (uarmg && uarmg->oartifact == ART_SUPERHEAVYKLONK) )
...
#define IntMysteryResist	(HMysteryResist || have_doubleshelljewel() || (uwep && uwep->oartifact == ART_TONA_S_GAMES && (PlayerInConeHeels || PlayerInStilettoHeels) ) || (uarm && uarm->oartifact == ART_KUSE_MUSE) || (uarms && uarms->oartifact == ART_JUST_X__IT) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_TOTAL_PARRY_GAUCHE) )
...
#define ExtWildWeatherEffect	(EWildWeatherEffect || autismweaponcheck(ART_THUNDERDOME_BAR) || autismweaponcheck(ART_FOREST_FIRE) || autismweaponcheck(ART_MAGIHEX) || (uarmh && uarmh->oartifact == ART_SPRING_BREEZE) || (uarmc && uarmc->oartifact == ART_VENTE_MODESTE) || (uarmc && uarmc->oartifact == ART_CORONA_TIME) || (uarmc && uarmc->oartifact == ART_OUT_OF_THE_RAIN) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || (uamul && uamul->oartifact == ART_______SCORE) )
