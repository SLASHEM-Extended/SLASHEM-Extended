...
#define Clairvoyant		(((HClairvoyant && !(u.nonintrinsicproperty == CLAIRVOYANT)) || EClairvoyant || (nohands(youmonst.data) && !Race_if(PM_TRANSFORMER) && uimplant && (goodimplanteffect(uimplant) == CLAIRVOYANT) ) || (uamul && uamul->oartifact == ART_WOUUU) || (uarmh && uarmh->otyp == ART_WSCHIIIIIE_) ) &&\
...
#define Conf_resist		(((HConf_resist && !(u.nonintrinsicproperty == CONF_RES)) || EConf_resist || (nohands(youmonst.data) && !Race_if(PM_TRANSFORMER) && uimplant && (goodimplanteffect(uimplant) == CONF_RES) ) || tech_inuse(T_STAT_RESIST) || (uarmf && uarmf->oartifact == ART_BOOTS_OF_THE_MACHINE) || (uarmf && uarmf->oartifact == ART_SANDRA_S_BEAUTIFUL_FOOTWEA) || (uarms && uarms->oartifact == ART_FIVE_STAR_PARTY) || (uarmu && uarmu->oartifact == ART_TOTAL_CONTROL) || (uarmc && uarmc->oartifact == ART_FREQUENT_BUT_WEAK_STATUS) ) && !NoConf_resist)
