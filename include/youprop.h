...
#define IntFire_resistance	(HFire_resistance || FireImmunity || (uarm && uarm->oartifact == ART_DUEUEUEUET) || (uarm && uarm->oartifact == ART_COAL_PEER) || resists_fire(&youmonst) || is_fire_resistant(youmonst.data) || (uarms && uarms->oartifact == ART_HEATEM_UP) || (uarms && uarms->oartifact == ART_THATS_ENOUGH_RIGHT_THERE) || (powerfulsymbiosis() && is_fire_resistant(&mons[u.usymbiote.mnum])) )
...
#define ExtSickopathy	(ESickopathy || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || (uwep && uwep->oartifact == ART_SCHOSCHO_BARBITUER) )
...
#define ExtSlow_digestion	(ESlow_digestion || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == SLOW_DIGESTION) ) || (uwep && uwep->oartifact == ART_BREAD_FOR_THE_WORLD) || (uwep && uwep->oartifact == ART_HOLY_GRAIL) || (uarmh && uarmh->oartifact == ART_NEVEREATER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_ENTEROGASTER_ALTO) || (uimplant && uimplant->oartifact == ART_BUCKET_HOUSE) || (uarmc && uarmc->oartifact == ART_FEMMY_FATALE) || (uarmh && uarmh->oartifact == ART_WOLF_KING) )
