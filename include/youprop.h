...
#define Fear_resistance		(((HFear_resistance && !(u.nonintrinsicproperty == FEAR_RES)) || EFear_resistance || (nohands(youmonst.data) && !Race_if(PM_TRANSFORMER) && uimplant && (goodimplanteffect(uimplant) == FEAR_RES) ) || tech_inuse(T_STAT_RESIST) || (uarmf && uarmf->oartifact == ART_GRANDPA_S_BROGUES) || (uarmf && uarmf->oartifact == ART_PARANOIA_STRIDE) ) || (uwep && uwep->oartifact == ART_USELESSNESS_OF_PLENTY) && !NoFear_resistance)
