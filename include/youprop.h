...
#define CanHaveBrownSpells (BrownSpells || u.uprops[BROWN_SPELLS].extrinsic || have_brownspellstone() || (uarmf && uarmf->oartifact == ART_ARABELLA_S_GIRL_KICK) || (uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) || carryingarti(ART_BLUEDAYPAPER_BECOMES_SHITT))
...
#define IntPoison_resistance	(HPoison_resistance || (uball && uball->oartifact == ART_HIGH_AFFIX) || Race_if(PM_SLYER_ALIEN) || resists_poison(&youmonst) || is_poison_resistant(youmonst.data) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarm && uarm->oartifact == ART_GREENHUE) || carryingarti(ART_ALL_THE_FRAGRANT_GIRLS) || (uwep && uwep->oartifact == ART_SYLVIE_S_EASY_MODE) || (uarmf && uarmf->oartifact == ART_POI_SEN_SUX) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_PULSE_NODE_OF_GERALT) || (powerfulsymbiosis() && is_poison_resistant(&mons[u.usymbiote.mnum])) )
...
#define FemtrapActiveAnja	(FemaleTrapAnja || u.uprops[FEMTRAP_ANJA].extrinsic || have_femtrapanja() || carryingarti(ART_ALL_THE_FRAGRANT_GIRLS) )
...
#define FemtrapActiveYasaman	(FemaleTrapYasaman || u.uprops[FEMTRAP_YASAMAN].extrinsic || have_femtrapyasaman() || bmwride(ART_KERSIE_PERSIE) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || carryingarti(ART_ALL_THE_FRAGRANT_GIRLS) || (uarms && uarms->oartifact == ART_FROM_MASHHAD_TO_AHVAZ) )
...
#define ExtHalluc_resistance	(EHalluc_resistance || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == HALLUC_RES) ) || carryingarti(ART_ALL_IS_LOST) || (Race_if(PM_BATMAN) && uwep && uwep->oartifact == ART_BLACKSWANDIR))
...
#define FunnyHallu		(Role_if(PM_GOFF) || (uamul && uamul->oartifact == ART_NARCOTIC_TRADE) || (uarmc && uarmc->oartifact == ART_BE_HI_WITHOUT_DRUGS) || RngeFunnyHallu || ishallucinator || HHallucination || (uarmf && uarmf->oartifact == ART_LUSTY_GIGGLE) || u.uprops[MULTISHOES].extrinsic || u.funnyhallutimer || autismweaponcheck(ART_FADED_USELESSNESS) || autismweaponcheck(ART_LANCE_OF_LONGINUS) || (uamul && uamul->oartifact == ART_FUN_ALL) || carryingarti(ART_LEO_IS_ALWAYS_HIGH) || autismweaponcheck(ART_SCHWANZUS_LANGUS) || HeavyHallu || EHallucination || u.uprops[SENSORY_DEPRIVATION].extrinsic || (u.funnyhalluroll != 9999999 && u.usanity > u.funnyhalluroll) || flags.hippie || ( (u.uprops[DEHYDRATION].extrinsic || Dehydration || have_dehydratingstone()) && ((u.dehydrationtime - moves) < 1) )  )
...
#define IntWarning	(HWarning || carryingarti(ART_RED_BUTTON_FLASH) || autismringcheck(ART_CLOTOBBLE) )
...
#define IntKeen_memory	(HKeen_memory || carryingarti(ART_UGGHEN) || (uarmc && uarmc->oartifact == ART_WRITER_S_BLOCK) || (ublindf && objects[ublindf->otyp].oc_material == MT_MENGETIUM) || (youmonst.data->mlet == S_QUADRUPED))
#define ExtKeen_memory	(EKeen_memory || carryingarti(ART_UGGHEN) || (uarmc && uarmc->oartifact == ART_WRITER_S_BLOCK) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == KEEN_MEMORY) ) || (uarmu && uarmu->oartifact == ART_MEMORIAL_GARMENTS) || (uarmc && uarmc->oartifact == ART_MEMORY_AID) || (uarmc && uarmc->oartifact == ART_REMEMBERING_THE_BAD_TIMES) || (uarmf && uarmf->oartifact == ART_DESERT_MEDITATION) || (uarmh && uarmh->oartifact == ART_VERY_MEMORY) || (uamul && uamul->oartifact == ART_GRAY_ENPROTECTION) || carryingarti(ART_MINIMAL_MINI_ME) || (uwep && uwep->oartifact == ART_ICED_OUT_COMPUTER_ON_A_CHA) || (uball && uball->oartifact == ART_ICED_OUT_COMPUTER_ON_A_CHA) || carryingarti(ART_MATTERKEEPER) || (uarm && uarm->oartifact == ART_SPELLNOTFORGET) || (uarmh && uarmh->oartifact == ART_MEMORY_COMMANDER) || (uwep && uwep->oartifact == ART_DESSESGA) || autismringcheck(ART_COMBINE_HOLD) || (uwep && uwep->oartifact == ART_FADING_FROM_MEMORY) || (uarm && uarm->oartifact == ART_REMEMBERANCE___THE_BURST_T) || (uamul && uamul->oartifact == ART_ISRA_S_ANGLE) || (uarms && uarms->oartifact == ART_GOLDEN_DAWN) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_SARREFREE) || (uimplant && uimplant->oartifact == ART_UNFORGETTABLE_EVENT) || (uarmh && uarmh->oartifact == ART_SONG_REMEMBERANCE) || (uarmh && uarmh->oartifact == ART_TRANSMEMORIZER) || (uamul && uamul->oartifact == ART_ARABELLA_S_DICINATOR) || (uarms && uarms->oartifact == ART_GREXIT_IS_NEAR) || (uarmh && uarmh->oartifact == ART_SING_AND_JULIETTA_S_JOINT_) || (uamul && uamul->oartifact == ART_COMPUTER_AMULET) )
