...
#define Sleep_resistance	( !( (Thirst || (uwep && uwep->oartifact == ART_LUISA_S_CHARMING_BEAUTY) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_LUISA_S_CHARMING_BEAUTY) || u.uprops[THIRST].extrinsic || have_thirststone()) && (u.uhunger > 2000 ) ) && !Race_if(PM_KOBOLT) && !Race_if(PM_DEEP_ELF) && !have_sleepstone() && !NoSleep_resistance && ( (HSleep_resistance && !(u.nonintrinsicproperty == SLEEP_RES)) || ESleep_resistance || (uwep && uwep->oartifact == ART_SLEEPLESS_NIGHTS) || (uarmu && uarmu->oartifact == ART_PRISMATIC_SHIRT) || FemaleTrapNatalje || (uarmf && uarmf->oartifact == ART_JESSICA_S_TENDERNESS) || (uarmh && uarmh->oartifact == ART_HAVE_ALL_YOU_NEED) || (uamul && uamul->oartifact == ART_SNOREFEST && (moves % 10 != 0) ) || \
