...
#define VulnerabilityXtra	((have_vulnerabilitystone() == 2) || (uamul && uamul->oartifact == ART_ISRA_S_ANGLE) )
...
#define IntStealth	(HStealth || autismringcheck(ART_XANATHAR_S_RING_OF_PROOF) || (uarmc && uarmc->oartifact == ART_STEALTH_OR_STEALING) || (uarmc && uarmc->oartifact == ART_WHISPERWIND_CLOAK) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_RESTROOM_DRENCHING) || (Race_if(PM_MOON_ELF) && ((flags.moonphase >= 1 && flags.moonphase <= 3) || autismringcheck(ART_METAL_GEAR_NETHACK) || (uwep && uwep->oartifact == ART_SUPERSTAB) || (flags.moonphase >= 5 && flags.moonphase <= 7))) )
...
#define ExtJumping	(EJumping || (uwep && uwep->oartifact == ART_DIZZY_METAL_STORM) || (uarmc && uarmc->oartifact == ART_ROKKO_CHAN_S_SUIT) || (uarmf && uarmf->oartifact == ART_AIRSHIP_DANCING) || (uarmf && uarmf->oartifact == ART_SEVEN_LEAGUE_BOOTS) || (uwep && uwep->oartifact == ART_ON_CLOUD_SEVEN) || (uwep && uwep->oartifact == ART_JUMP_HURRIES) || (uarmf && uarmf->oartifact == ART_JUMP_KICK_ACTION) || (uarmf && uarmf->oartifact == ART_ARVOGENIA_S_BIKER_HEELS && u.usteed) || (uwep && uwep->oartifact == ART_KHAKKHARA_OF_THE_MONKEY) || autismringcheck(ART_MOVERY_GAN) || (uarm && uarm->oartifact == ART_LUKE_S_JEDI_POWER) || (uarmf && uarmf->oartifact == ART_PRINCE_OF_PERSIA) || (uwep && uwep->oartifact == ART_SUPERGIRL_S_JUMP_AND_RUN_F) || (uwep && uwep->oartifact == ART_STRINGFELLOW_HOUKI) || (uamul && uamul->oartifact == ART_ONLY_ONE_ESCAPE) || u.uprops[MULTISHOES].extrinsic)
...
#define NoFast	(!Race_if(PM_IMMUNIZER) && (u.uprops[DEAC_FAST].intrinsic || (uarmf && uarmf->oartifact == ART_END_OF_LEWDNESS) || autismringcheck(ART_FUW_TENK) || UHaveAids || (FemtrapActiveNaomi && !PlayerInHighHeels) || (u.impossibleproperty == FAST) || (Race_if(PM_HUMANOID_ANGEL) && u.ualign.record < 0)))
...
#define ExtResistancePiercing	(EResistancePiercing || (uwep && uwep->oartifact == ART_USELESS_TALK) || (uarmu && uarmu->oartifact == ART_HA_HA_HA_HA___) || (uarm && uarm->oartifact == ART_ALTADOON_HERMA_MORA) || (uwep && uwep->oartifact == ART_MR__OF_HIGH_HEELS) || (uwep && uwep->oartifact == ART_ETHER_DAGGER) || (uwep && uwep->oartifact == ART_DAGGER_SURROUNDED_BY_WIND) || (uarmh && uarmh->oartifact == ART_DAN_THE_MAGE) || (uarmh && uarmh->oartifact == ART_JOLENE_S_ZEAL) || (uarmg && uarmg->oartifact == ART_SPELLSNIPE) || (uwep && uwep->oartifact == ART_MARC_S_WRONG_SPELL) || (uarmf && uarmf->oartifact == ART_SMILE_Z) || (uarmh && uarmh->oartifact == ART_DREHTURN) || (uarmg && uarmg->oartifact == ART_DATLEST_KRANN) || (uarms && uarms->oartifact == ART_TARTSCH) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == RESISTANCE_PIERCING) ) )
...
#define ExtWildWeatherEffect	(EWildWeatherEffect || (uamul && uamul->oartifact == ART_______SCORE) )
