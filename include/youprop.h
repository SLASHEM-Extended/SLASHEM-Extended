...
#define ExtTelepat	(ETelepat || (uarmf && uarmf->oartifact == ART_SCAN_ME) || (uamul && uamul->oartifact == ART_AMULET_OF_SPLENDOR) || (uarmh && uarmh->oartifact == ART_INCREDIBLE_VIEW) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_OMEN_EYE_OF_SELORA_THE_SEE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_JANE_S_BODY_SIZE) || (uchain && uchain->oartifact == ART_IZQUXXISQUID_S_RESEARCH) || (uarms && uarms->oartifact == ART_IT_IS_COMPLETE_NOW) || Race_if(PM_DEVELOPER))
...
#define ExtRegeneration	(ERegeneration || (uimplant && uimplant->oartifact == ART_SIGNIFICANT_RNG_JITTER) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == REGENERATION) ) || have_crashrecoverstone() || (powerfulimplants() && uimplant && uimplant->oartifact == ART_THEY_RE_ALL_YELLOW) || (uwep && uwep->oartifact == ART_BREAD_FOR_THE_WORLD) || (uimplant && uimplant->oartifact == ART_TOO_MUCH_HEALTH) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_CORE_HALO_OF_AURELION_THE_) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_HEARTSHARD_OF_ARAGORN) || (uarmf && uarmf->oartifact == ART_ERROR_IN_PLAY_ENCHANTMENT) )
