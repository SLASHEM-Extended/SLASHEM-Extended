    ...
    int control;
{
	...
	if (has_night_vision && !(u.uprops[WEAKSIGHT].extrinsic || (uarmh && uarmh->oartifact == ART_WOLF_KING) ||  WeakSight || have_weaksightstone() ) && !(uarmh && uarmh->oartifact == ART_FIRE_CHIEF_HELMET) && u.xray_range < (u.nv_range + Sight_bonus) ) {
	    if (!(u.nv_range + Sight_bonus) ) {	/* range is 0 */
		next_array[u.uy][u.ux] |= IN_SIGHT;
		levl[u.ux][u.uy].seenv = SVALL;
		next_rmin[u.uy] = min(u.ux, next_rmin[u.uy]);
		next_rmax[u.uy] = max(u.ux, next_rmax[u.uy]);
	    } else if (( (u.nv_range + Sight_bonus) > 0) && !(u.uprops[WEAKSIGHT].extrinsic || (uarmh && uarmh->oartifact == ART_WOLF_KING) || WeakSight || have_weaksightstone() ) && !(uarmh && uarmh->oartifact == ART_FIRE_CHIEF_HELMET) ) {
