...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uimplant && uimplant->oartifact == ART_VIRAL_CORE_OF_ISKAR_THE_PA) || (uarmh && uarmh->oartifact == ART_SAMIRA_S_EXPRESSION) || (uarmg && uarmg->oartifact == ART_MATTHIAS_S_GAUNTLETS_OF_FA) || (uarmf && uarmf->oartifact == ART_FUNGUS_ERADICATION) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GENDEREQUALNESS) ) return TRUE;
...
playerextrinsicfearres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_DON_T_RUN_IN_FEAR) || (uarmh && uarmh->oartifact == ART_FREDERIK_S_COMBAT_HELMET) || (uarmc && uarmc->oartifact == ART_EMIL_S_MANTLE_OF_RESOLVE) || (uarmg && uarmg->oartifact == ART_CORVIN_S_GRIP_OF_PERSISTEN) ) return TRUE;
...
playerextrinsicspeed()
{
	...
	if ( (uchain && uchain->oartifact == ART_JORMUNGANDR_S_COIL) || (uimplant && uimplant->oartifact == ART_GIULY_AH) || (uarm && uarm->oartifact == ART_MOTA_ROLA) || u.tempveryfastspeed || (uarmc && uarmc->oartifact == ART_CLARA_S_WINDCLOAK) || bmwride(ART_KERSTIN_S_PROJECTION_LEAD) || (uarmc && uarmc->oartifact == ART_HUNTING_TIME_HINT) || (uarmf && uarmf->oartifact == ART_ADHS_DIAGNOSIS) || (uarmg && uarmg->oartifact == ART_GLENNIS_DOWNS) || (uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || bmwride(ART_MARLOWE_S_CAMEL) || (uarmf && uarmf->oartifact == ART_MAREN_S_GALE_BOOTS) || (uarmf && uarmf->oartifact == ART_DARIUS_S_STORMWALKERS) || (uwep && uwep->oartifact == ART_PIPPA_S_URGE) ) return TRUE;
