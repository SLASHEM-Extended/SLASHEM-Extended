...
playerextrinsicdrainres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ALMIS_MANICIA) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_BLOODPUMP_OF_HARKON_BLOODR) ) return TRUE;
...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uimplant && uimplant->oartifact == ART_VIRAL_CORE_OF_ISKAR_THE_PA) || (uarmh && uarmh->oartifact == ART_SAMIRA_S_EXPRESSION) || (uarmg && uarmg->oartifact == ART_MATTHIAS_S_GAUNTLETS_OF_FA) || (uarmc && uarmc->oartifact == ART_BOLWING_S_RAGS) || (uarmf && uarmf->oartifact == ART_FUNGUS_ERADICATION) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GENDEREQUALNESS) ) return TRUE;
...
playerextrinsicfearres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_DON_T_RUN_IN_FEAR) || (uarmh && uarmh->oartifact == ART_FREDERIK_S_COMBAT_HELMET) || (uarmc && uarmc->oartifact == ART_EMIL_S_MANTLE_OF_RESOLVE) || (uarmf && uarmf->oartifact == ART_ELLA_S_VENGEANCE) || (uarmg && uarmg->oartifact == ART_CORVIN_S_GRIP_OF_PERSISTEN) ) return TRUE;
...
playerextrinsicdeathres()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || (uarm && uarm->oartifact == ART_HANGING_THREAD) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) ) return TRUE;
...
playerextrinsicmysteryres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_SUPATECT) || (uarmf && uarmf->oartifact == ART_GNOME_QUEEN_S_SLIPPERS) || (uarmh && uarmh->oartifact == ART_PURPLE_ORC) || (uarmc && uarmc->oartifact == ART_FELL_GARB) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_I_LL_GIVE_YOU_A_GENDER_STA) || (uarm && uarm->oartifact == ART_MYSTIOSITY) || (uarmc && uarmc->oartifact == ART_IS_THE_NOT_SO_VERY_MYSTERI) || (uarmh && uarmh->oartifact == ART_ENWAHSTER) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarm && uarm->oartifact == ART_OAR_SUPER_GRAPHICS_CARD_BU) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || (uarms && uarms->oartifact == ART_MYSTERIOUS_SYMBOL) || (ublindf && ublindf->oartifact == ART_POLINTOTAH) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) ) return TRUE;
...
dodip()
{
	...
	if (IS_WELL(here)) { /* item gets wet, no other effects --Amy */
		...
		if (yn(qbuf) == 'y') {
		    if (Levitation) {
			floating_above(tmp);
		    } else if (u.usteed && !(uwep && uwep->oartifact == ART_SORTIE_A_GAUCHE) && !(powerfulimplants() && uimplant && uimplant->oartifact == ART_READY_FOR_A_RIDE) && !FemtrapActiveKerstin && !bmwride(ART_JAFARO_ON_TOUR) && !(bmwride(ART_DEEPER_LAID_BMW)) && !is_swimmer(u.usteed->data) && !u.usteed->egotype_watersplasher &&
