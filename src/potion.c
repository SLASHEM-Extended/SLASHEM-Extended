...
playerextrinsicshockres()
{
	...
	if ( (uwep && uwep->oartifact == ART_ELEMENTAL_STAFF) || (uarmf && uarmf->oartifact == ART_SARAH_S_REPLACEMENT) || autismringcheck(ART_RING_OF_STEEL_DRAGON) || (uwep && uwep->oartifact == ART_JADE_TALON) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || autismringcheck(ART_RED_YELLOW_ORANGE) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarmf && uarmf->oartifact == ART_MARIA_S_FLEECLE_FLOPS) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uimplant && uimplant->oartifact == ART_FULLPOWER) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || (uarm && uarm->oartifact == ART_EFHECK) ) return TRUE;
