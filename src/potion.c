...
playerextrinsicaggravatemon()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_KUUGRUU__KUUGRUU_) || (FemtrapActiveChloe && uarmf && uarmf->otyp == CHLOE_BALL_HEELS) || (uarmf && uarmf->oartifact == ART_INGRID_S_IRONHOOF_BOOTS) || (uarmc && uarmc->oartifact == ART_LILIANE_S_GLAMOUR_VEIL) ) return TRUE;
...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uimplant && uimplant->oartifact == ART_VIRAL_CORE_OF_ISKAR_THE_PA) || (uarmg && uarmg->oartifact == ART_MATTHIAS_S_GAUNTLETS_OF_FA) || (uarmf && uarmf->oartifact == ART_FUNGUS_ERADICATION) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GENDEREQUALNESS) ) return TRUE;
...
playerextrinsicdeathres()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) ) return TRUE;
...
playerextrinsicfreeaction()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ROUGHGRAVEL_MASTER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uarm && uarm->oartifact == ART_AYZAH_S_HURDLE) || (uarmf && uarmf->oartifact == ART_JANET_S_DETACHMENT) || (ublindf && ublindf->oartifact == ART_HEADHORN) || (uarmf && uarmf->oartifact == ART_RINGFIGHTER_S_GUARD) || (u.usteed && uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || (uarmf && uarmf->oartifact == ART_ERIK_S_MARCHING_BOOTS) || (uwep && uwep->oartifact == ART_PERFECT_HARMONY) ) return TRUE;
...
playerextrinsicflying()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_OTMAR_S_PHANTOM_STEPS) || (ublindf && ublindf->oartifact == ART_MONIQUE_S_NONSENSE) ) return TRUE;
