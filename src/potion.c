...
playerextrinsicfreeaction()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ROUGHGRAVEL_MASTER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uarmf && uarmf->oartifact == ART_RINGFIGHTER_S_GUARD) ) return TRUE;
	...
	register struct obj	*otmp;
{
	...
	case POT_JOLT_COLA:
		...
		pline_The("bottle of Jolt Cola is named %s today!", feminismtrapname(rnd(104)) ); /* keyword: "marlena" */
