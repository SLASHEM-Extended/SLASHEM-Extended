...
long xtime;
{
	...
	if (carryingarti(ART_HEMEPHEELE)) {
		xtime *= 6;
		xtime /= 5;
	}
	...
	PlayerBleeds += xtime;
	if (FunnyHallu) {
		if (PlayerBleeds > 100) You("see some soda shooting out of your body like a graceful fountain!");
		else if (PlayerBleeds > 50) pline("Someone seems to have built a running tap into your body!");
		else pline("You're dripping some funny-looking liquid on the ground!");
	} else {
...
boolean talk;
{
	...
	if (xtime && uarmg && uarmg->oartifact == ART_GLENNIS_DOWNS) set_itimeout(&HeavyConfusion, xtime);
}
...
boolean
isblockheeledb(number)
int number;
{
	if (number == CATWALK_SANDALS || number == KSENIA_PLATFORM_SANDALS || number == NATALJE_BLOCK_HEEL_SANDALS || number == SABINE_BLOCK_HEEL_SANDALS || number == HIPPIE_HEELS || number == SELF_WILLED_HEELS || number == COMMANDER_HEELS || number == COVETED_BOOTS || number == SINFUL_HEELS || number == LADY_BOOTS || number == BASIC_SHOES || number == BORING_BLOCK_HEELS || number == ELENA_COMBAT_BOOTS || number == THAI_COMBAT_BOOTS || number == SHOCK_ISOLATED_HEELS || number == MELTEM_COMBAT_BOOTS || number == KATRIN_COMBAT_BOOTS || number == NADJA_BUCKLED_LADY_SHOES || number == HIGH_SCORING_HEELS || number == KRISTIN_COMBAT_BOOTS || number == DORA_COMBAT_BOOTS || number == JULIA_COMBAT_BOOTS || number == NICOLE_COMBAT_BOOTS || number == LISA_COMBAT_BOOTS || number == SUSANNE_COMBAT_BOOTS || number == NORA_COMBAT_BOOTS || number == ANTJE_COMBAT_BOOTS || number == KERSTIN_COMBAT_BOOTS || number == LARA_COMBAT_BOOTS || number == JENNIFER_COMBAT_BOOTS || number == JETTE_COMBAT_BOOTS || number == VICTORIA_COMBAT_BOOTS || number == HENRIETTA_COMBAT_BOOTS || number == CONNY_COMBAT_BOOTS || number == KATIA_COMBAT_BOOTS || number == MARIYA_COMBAT_BOOTS || number == ELISE_HIPPIE_HEELS || number == RONJA_COMBAT_BOOTS || number == ARIANE_COMBAT_BOOTS || number == JOHANNA_COMBAT_BOOTS || number == MAGDALENA_COMBAT_BOOTS || number == MARLEEN_COMBAT_BOOTS || number == INGE_COMBAT_BOOTS) return TRUE;
...
playerwearssexyflats()
{
	...
	if (OBJ_DESCR(objects[uarmf->otyp])) {
		...
		if (itemhasappearance(uarmf, APP_BALLERINAS)) return TRUE;
		if (itemhasappearance(uarmf, APP_FLATFORM_SANDALS)) return TRUE;
		if (itemhasappearance(uarmf, APP_BUCKLE_SLIPPERS)) return TRUE;
		if (itemhasappearance(uarmf, APP_CUDDLE_BOOTS)) return TRUE;
...
maybeballheels()
{
	...
	if ((uarmf->otyp == CHLOE_BALL_HEELS) ) {
		havetheseheels = TRUE;
		opentoe = TRUE;
	}
...
playerextrinsicaggravatemon()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_KUUGRUU__KUUGRUU_) || (flags.female && (u.genitalhealth_f > 1) && (u.genitalhealth_m < 1)) || (!flags.female && (u.genitalhealth_m > 1) && (u.genitalhealth_f < 1)) || (FemtrapActiveChloe && uarmf && uarmf->otyp == CHLOE_BALL_HEELS) || (uarmf && uarmf->oartifact == ART_INGRID_S_IRONHOOF_BOOTS) || (uarmg && uarmg->oartifact == ART_GODOT_S_DESK_SLAM) || carryingarti(ART_ELOPORTAL) || (uarmc && uarmc->oartifact == ART_LILIANE_S_GLAMOUR_VEIL) ) return TRUE;
...
playerextrinsicdrainres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ALMIS_MANICIA) || carryingarti(ART_GHOSTERCHURCH) || (have_spellcolorartifact() == PINK_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_BLOODPUMP_OF_HARKON_BLOODR) ) return TRUE;
...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uimplant && uimplant->oartifact == ART_VIRAL_CORE_OF_ISKAR_THE_PA) || (uarmh && uarmh->oartifact == ART_SAMIRA_S_EXPRESSION) || (uarmg && uarmg->oartifact == ART_MATTHIAS_S_GAUNTLETS_OF_FA) || (have_spellcolorartifact() == VIOLET_SPELL_STONE) || (uarmc && uarmc->oartifact == ART_BOLWING_S_RAGS) || (uarmf && uarmf->oartifact == ART_FUNGUS_ERADICATION) || carryingarti(ART_DECADENCE_IN_DOSES) || carryingarti(ART_STRONGER_STOMACH) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GENDEREQUALNESS) ) return TRUE;
...
playerextrinsicacidres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_RAMMING_SUPPORT_RIPPAGE && (moves % 3 == 0) ) || (uarmg && uarmg->oartifact == ART_SPECTRATOR && (moves % 5 == 0) ) || (uwep && uwep->oartifact == ART_RESISTOMATIC) || (uwep && uwep->otyp == CHEMISTRY_SPACE_AXE) || (uarmh && uarmh->oartifact == ART_MASSIVE_IRON_CROWN_OF_MORG) || (uarmh && uarmh->oartifact == ART_WAR_MASK_OF_DURIN) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uwep && uwep->oartifact == ART_CREAMRES) || (uarmh && uarmh->oartifact == ART_UNIMPORTANT_ELEMENTS) || (have_spellcolorartifact() == YELLOW_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || autismringcheck(ART_FOLL_DECK) ) return TRUE;
...
playerextrinsicfearres()
{
	...
	if ( (uwep && uwep->oartifact == ART_NADJA_S_BROKEN_NAIL) || (uarmf && uarmf->oartifact == ART_LITTLE_BITCH_IS_RUCTIOUS) || (uarmf && uarmf->oartifact == ART_MANDY_S_RAIDWEAR) || (uarmf && uarmf->oartifact == ART_GRANDPA_S_BROGUES) || (uarmf && uarmf->oartifact == ART_PARANOIA_STRIDE) || (uarmf && uarmf->oartifact == ART_FRONT_TARGET) || (uamul && uamul->oartifact == ART_CONSOLE_TROPHY) || (uarmg && uarmg->oartifact == ART_SIRINE_S_MELLOW_LOOK) || (uwep && uwep->oartifact == ART_USELESSNESS_OF_PLENTY) || (uarmf && uarmf->oartifact == ART_ASTER_IX_BOOKLET) || (uarms && uarms->oartifact == ART_COLORS_TOGETHER) || (uwep && uwep->oartifact == ART_EORLINGAS) ) return TRUE;
	if ( (uarmf && uarmf->oartifact == ART_DON_T_RUN_IN_FEAR) || (uarmh && uarmh->oartifact == ART_FREDERIK_S_COMBAT_HELMET) || (uarmc && uarmc->oartifact == ART_EMIL_S_MANTLE_OF_RESOLVE) || carryingarti(ART_COMPLEX_BUILDED) || carryingarti(ART_TEALGLEAM) || (have_spellcolorartifact() == CYAN_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_ELLA_S_VENGEANCE) || (uarmg && uarmg->oartifact == ART_CORVIN_S_GRIP_OF_PERSISTEN) ) return TRUE;
...
playerextrinsicdeathres()
{
	...
	if ( (uarms && uarms->oartifact == ART_ANTINSTANT_DEATH) || (uamul && uamul->oartifact == ART_MYSTER_DEEP_) || (uarm && uarm->oartifact == ART_UMBRANOX) || (uarmc && uarmc->oartifact == ART_ADILA_S_PINNACLE) || (uarm && uarm->oartifact == ART_ALEIT_GIVESITNOT) || (uarm && uarm->oartifact == ART_STALWART_COSTUME) || (uamul && uamul->oartifact == ART_PERMANENTLY_BLACK) || (uwep && uwep->oartifact == ART_KALI_S_ORBIT) || (uarmf && uarmf->oartifact == ART_AMY_S_MISPURCHASE) || (uarmc && uarmc->oartifact == ART_BLACKER_QUIVER) || (uarm && uarm->oartifact == ART_EMRYS_BACKSCALE) || (uarmc && uarmc->oartifact == ART_PANZER_MANTLE) || (uarmf && uarmf->oartifact == ART_DO_NOT_FEAR_DEATH) || (uarmc && uarmc->oartifact == ART_ORNA_S_DOUBLE) || (uarmf && uarmf->oartifact == ART_JANET_S_DETACHMENT) || (uwep && uwep->oartifact == ART_SYLVIE_S_EASY_MODE) || (uarmf && uarmf->oartifact == ART_DARKFLECT) ) return TRUE;
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || (uarm && uarm->oartifact == ART_HANGING_THREAD) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) || carryingarti(ART_FINISHED_ANYWAY) || carryingarti(ART_MIRIAM_S_FEELING) || carryingarti(ART_EROGE_CHEEK) || (uwep && uwep->oartifact == ART_BLACK_MAW) || carryingarti(ART_LAST_PART) || (uarmf && uarmf->oartifact == ART_ALMUT_S_NOCARE) || (uwep && uwep->oartifact == ART_DEATH_MAW) ) return TRUE;
...
boolean
playerextrinsicmysteryres()
{
	if (EMysteryResist) return TRUE;
	if (carryingarti(ART_DOUBLESHELL)) return TRUE;
	...
	if ( (uarmf && uarmf->oartifact == ART_PLAY_THE_GAME_YOURSELF) || (uarmu && uarmu->oartifact == ART_ALL_IN_ONE_EFF) || (uarm && uarm->oartifact == ART_OOOOOOH) || (uarmf && uarmf->oartifact == ART_MAGIC_BUTTERFLY) || (uarm && uarm->oartifact == ART_YOU_REALLY_HAVE_A_TOTAL_DA) || (uwep && uwep->oartifact == ART_BLOCK_THE_MYST) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_SONIC_TONFA) || (uarms && uarms->oartifact == ART_GONGDE_ERTAL) || (uamul && uamul->oartifact == ART_GIBE_DE_MYST) || (uwep && uwep->oartifact == ART_TONA_S_GAMES && (PlayerInConeHeels || PlayerInStilettoHeels) ) || (carryingarti(ART_ONE_IN_FOUR) && (moves % 4 == 0) ) || (uarm && uarm->oartifact == ART_ELROS_EAERFALAS) ) return TRUE;
	if ( (uarm && uarm->oartifact == ART_OKAY__YOU_WIN_) || (uarms && uarms->oartifact == ART_JUST_X__IT) || (uarms && uarms->oartifact == ART_TLARKMER) || carryingarti(ART_HAVE_IT_ALL_BUT_NOT_GET) || (uarmh && uarmh->oartifact == ART_ANDEN_DRER) || (uarmc && uarmc->oartifact == ART_DOUBLE_PLUS_GOOD) || (flags.female && uarmf && uarmf->oartifact == ART_NANCY_S_ASSISTANCE) || (uarm && uarm->oartifact == ART_JEANETTA_S_REVENGE) || (uarm && uarm->oartifact == ART_FULL_DOSE_LIFE) || (uarmc && uarmc->oartifact == ART_POW_RER) || (uamul && uamul->oartifact == ART_MYSTER_DEEP_) || (uarm && uarm->oartifact == ART_KUSE_MUSE) || (uarmf && uarmf->oartifact == ART_GREEN_MYSTERY) || (uarm && uarm->oartifact == ART_RAND_ARM) ) return TRUE;
	if ( (uwep && uwep->oartifact == ART_SAMA_KATA) || (uarm && uarm->oartifact == ART_ESPECIALLYRANDARM) || (uarmc && uarmc->oartifact == ART_ADILA_S_PINNACLE) || (uarm && uarm->oartifact == ART_GRANT_ESPECIAL) || (uarm && uarm->oartifact == ART_AMMY_S_NORMALNESS) || (uarm && uarm->oartifact == ART_TEH_NORM) || (uarm && uarm->oartifact == ART_RES_CIRCLE) || (uarmg && uarmg->oartifact == ART_IDEA_IN_HEADBACK) || (uarmf && uarmf->oartifact == ART_SUPER_PEEP_TOES) || (uwep && uwep->otyp == MAGISWORD) || (uarm && uarm->oartifact == ART_VERY_SEXUAL) || (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == MYSTERY_RES)) ) return TRUE;
	if ( (uarmc && uarmc->oartifact == ART_SUPATECT) || (uarmf && uarmf->oartifact == ART_GNOME_QUEEN_S_SLIPPERS) || (uarmh && uarmh->oartifact == ART_PURPLE_ORC) || (uarmc && uarmc->oartifact == ART_FELL_GARB) || (have_spellcolorartifact() == METAL_SPELL_STONE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_I_LL_GIVE_YOU_A_GENDER_STA) || (uarm && uarm->oartifact == ART_MYSTIOSITY) || (uarmc && uarmc->oartifact == ART_IS_THE_NOT_SO_VERY_MYSTERI) || (uarmh && uarmh->oartifact == ART_ENWAHSTER) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarm && uarm->oartifact == ART_OAR_SUPER_GRAPHICS_CARD_BU) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || carryingarti(ART_FINISHED_ANYWAY) || (uwep && uwep->oartifact == ART_FEEBILITY) || (uarms && uarms->oartifact == ART_MYSTERIOUS_SYMBOL) || (ublindf && ublindf->oartifact == ART_POLINTOTAH) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) ) return TRUE;
	if ( carryingarti(ART_CINCELOBLO) ) return TRUE;
...
boolean
playerextrinsicdiscountaction()
{
	if (EDiscount_action) return TRUE;
	...
	if ( (uarmg && uarmg->oartifact == ART_INTERMINTER) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmf && uarmf->oartifact == ART_MOST_EROTIC_AIR_CURRENT_NO) || (u.usteed && uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || (uarmf && uarmf->oartifact == ART_OFFZEEH_BRUN) || carryingarti(ART_FINISHED_ANYWAY) || carryingarti(ART_LESS_STICK_CHEW) || (uarmu && uarmu->oartifact == ART_STOCKTON_BRAND) ) return TRUE;
...
boolean
playerextrinsicmagicalbreath()
{
	if (EMagical_breathing) return TRUE;
	if (powerfulimplants() && uimplant && (goodimplanteffect(uimplant) == MAGICAL_BREATHING) ) return TRUE;
	if ( (uarmh && uarmh->oartifact == ART_SHPX_GUVF_FUVG) || (Role_if(PM_ANACHRONIST) && uwep && uwep->oartifact == ART_ANACHRONONONONAUT_PACKAGE) || (uwep && uwep->oartifact == ART_POSEIDON_S_OTHER_TRIDENT) || (uwep && uwep->oartifact == ART_SEAFOAM) || (uarm && uarm->oartifact == ART_INCREDIBLY_SEXY_SQUEAKING) || (uarmf && uarmf->oartifact == ART_HERMES__UNFAIRNESS) || (uwep && uwep->oartifact == ART_SKIN_DEGREE) || (uwep && uwep->oartifact == ART_MUSICAL_SNORKEL) || (uarmf && uarmf->oartifact == ART_FAR_EAST_RELATION) || (uwep && uwep->otyp == UNDERWATER_TRIDENT) || (uwep && uwep->oartifact == ART_GARY_S_RIVALRY) || (uarmf && uarmf->oartifact == ART_HADES_THE_MEANIE) || (uarm && uarm->oartifact == ART_ACTUAL_GRADIATING_WORK) || (uarmc && itemhasappearance(uarmc, APP_AIR_CLOAK)) || (uarmh && uarmh->oartifact == ART_WAR_HELM_OF_THE_DREAMING) ) return TRUE;
	if ( (uarmf && uarmf->oartifact == ART_MARJI_JANA) || (uwep && uwep->oartifact == ART_HELL_OR_HIGH_WATER) || (uwep && uwep->oartifact == ART_LOGGERS_S_OVERKILL) || (uarmh && uarmh->oartifact == ART_HABIBA_S_MATRONAGE) || (uwep && uwep->oartifact == ART_ON_CLOUD_SEVEN) || autismringcheck(ART_VILYA) || (uwep && uwep->oartifact == ART_HK_CLEAR) || (uarmc && uarmc->oartifact == ART_WATERFORCE____) || (uarms && uarms->oartifact == ART_WIDE_EYE_BUBBLE) || (uwep && uwep->oartifact == ART_DAMN_PROCURATION) || (uarm && uarm->oartifact == ART_OUTMINE_OF_METAL) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_JANA_S_MAKE_UP_PUTTY) || (uwep && uwep->oartifact == ART_FOAMONIA_WATER) || (uarmf && uarmf->oartifact == ART_CLONE_) || (uamul && uamul->oartifact == ART_MAGICAL_SPELLING) || (uamul && uamul->oartifact == ART_AMULET_OF_SPLENDOR) || autismringcheck(ART_JANA_S_DIMINISHER) || (uarmc && uarmc->oartifact == ART_SATAN_S_SUGGESTION) ) return TRUE;
	if ( (uarmc && uarmc->oartifact == ART_JANA_S_SECRET_CAR) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_NEWFOUND_AND_USEFUL) || (uarmc && uarmc->oartifact == ART_JANA_S_GRAVE_WALL) || (uarmf && uarmf->oartifact == ART_UNDERWATER_LOVE) || (uarm && uarm->oartifact == ART_ROCKET_IMPULSE) || (uwep && uwep->oartifact == ART_TRIDENT_OF_POSEIDON) || (uwep && uwep->oartifact == ART_HIGH_ORIENTAL_PRAISE) || (uwep && uwep->oartifact == ART_PLLTSCH) || (uarmh && uarmh->oartifact == ART_VACUUM_CLEANER_DEATH) || (uarmc && uarmc->oartifact == ART_LAURA_S_SWIMSUIT) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_SIGNIFICANT_RNG_JITTER) || (uarmf && uarmf->oartifact == ART_JANA_S_VAGINAL_FUN) || (uarmf && uarmf->oartifact == ART_SHORTFALL) || (uarmf && uarmf->oartifact == ART_SANDRA_S_BEAUTIFUL_FOOTWEA) || (uarmf && uarmf->oartifact == ART_FUN_ALL_IN_ONE) || (uarmf && uarmf->oartifact == ART_ALLYNONE) || (Role_if(PM_TOPMODEL) && PlayerInHighHeels) ) return TRUE;
	if ( (uarmc && uarmc->oartifact == ART_MANTLE_OF_THE_MANTA_RAY) || (uarmc && uarmc->oartifact == ART_OUT_OF_THE_RAIN) || (uwep && uwep->oartifact == ART_TEZCATLIPOCA_S_BUBBLESTORM) || (uarmf && uarmf->oartifact == ART_AIR_BLISTER) || (uarms && uarms->oartifact == ART_FLIERHACH) || (uarmg && uarmg->oartifact == ART_WATER_WINGS) || (uwep && uwep->oartifact == ART_MECHINEFELL) || (uchain && uchain->oartifact == ART_MECHINEFELL) || (uwep && uwep->oartifact == ART_ZERATHUL_S_COMET) || (uchain && uchain->oartifact == ART_ZERATHUL_S_COMET) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GREAT_HARBOR_WAVE) || (uarmf && uarmf->oartifact == ART_ELIZAH_S_SINKER) || (uarmh && uarmh->oartifact == ART_DEEPERBREATH) ) return TRUE; 
	if ( (uarmh && uarmh->oartifact == ART_DANTE_S_ASHEN_VISOR) || carryingarti(ART_DEPTH_SURVIVE) || carryingarti(ART_UNDEAD_BECOME_HAVE) || (uarmg && uarmg->oartifact == ART_CLARA_S_GLOVES_OF_THE_TIDE) ) return TRUE;
...
playerextrinsicpsires()
{
	...
	if ( (uamul && uamul->oartifact == ART_PRECIOUS_UNOBTAINABLE_PROP) || (uarm && uarm->oartifact == ART_MINDMEND) || autismringcheck(ART_KONDRE) || (uamul && uamul->oartifact == ART_ISRA_S_ANGLE) || (uwep && uwep->oartifact == ART_CHAINS_OF_THE_ABYSS) || (uarmh && uarmh->oartifact == ART_PSYSTRYKE) || (uarm && uarm->oartifact == ART_ULTRAVIOLET_RAINBOW) || (uarmh && uarmh->oartifact == ART_GHOSTER_VISOR) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarmh && uarmh->oartifact == ART_SING_AND_JULIETTA_S_JOINT_) || (ublindf && ublindf->oartifact == ART_SYLBE_S_LACK) || (uarmh && uarmh->oartifact == ART_AMARA_S_MINDGUARD_HELM) || (uarmc && uarmc->oartifact == ART_EMIL_S_MANTLE_OF_RESOLVE) ) return TRUE;
	if ( (uarmh && uarmh->oartifact == ART_SPIREHEAD) || (uarmf && uarmf->oartifact == ART_MAURAH_S_ABSOLUTE_HEART_SH) || carryingarti(ART_CASTLEPSYCHE) ) return TRUE;
...
playerextrinsicsleepres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_CLOAK_OF_THE_UNHELD_ONE) || (uarmc && uarmc->oartifact == ART_CLOAK_OF_THE_UNHELD_POTATO) || (uarmc && uarmc->oartifact == ART_STRIPED_SHIRT_OF_THE_FALSE) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (uarmf && uarmf->oartifact == ART_SYLVIE_S_YAWN) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uarmc && uarmc->oartifact == ART_NUTKNU_KOLAS) || (uarmh && uarmh->oartifact == ART_ENSNARED_MIND) || autismringcheck(ART_BUT_YOU_MUST) || (uarmf && uarmf->oartifact == ART_DAPHNE_S_BOREDOM) || autismringcheck(ART_IT_S_NOT_BEDTIME_YET) || autismringcheck(ART_GRISHUH) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarmc && uarmc->oartifact == ART_WAKESTAY) || (uarms && uarms->oartifact == ART_KAVYA_S_HASPLE) || (uarmu && uarmu->oartifact == ART_SETH_S_ERROR) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uwep && uwep->oartifact == ART_MECHINEFELL) || (uchain && uchain->oartifact == ART_MECHINEFELL) ) return TRUE;
	if ( (uarmf && uarmf->oartifact == ART_KARIN_S_SHARP_EDGE) || (ublindf && ublindf->oartifact == ART_LEANDRA_S_WORD) || (uarmf && uarmf->oartifact == ART_SOLVEJG_S_SQUEEZE_IN) || (have_spellcolorartifact() == ORANGE_SPELL_STONE) || (ublindf && ublindf->oartifact == ART_SYLBE_S_LACK) || (uarm && uarm->oartifact == ART_AYZAH_S_HURDLE) || carryingarti(ART_STAY_AWAKE___) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || carryingarti(ART_UNDEAD_BECOME_HAVE) || (uarmc && uarmc->oartifact == ART_GODOT_S_COFFEE_SUPPLY) || carryingarti(ART_RHAAAAAAAAAAAAAR_) || carryingarti(ART_ENJOY_THE_MESSAGE_FLEET___) || (uwep && uwep->oartifact == ART_PERFECT_HARMONY) ) return TRUE;
...
playerextrinsicdisintres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_SOFTWOOD) || autismringcheck(ART_KUPA_STABILITY) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarm && uarm->oartifact == ART_ARMOR_OF_RETRIBUTION) || (have_spellcolorartifact() == BLACK_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_AMY_S_MISPURCHASE) || (uarms && uarms->oartifact == ART_BLACK_TOWER) || (uarm && uarm->oartifact == ART_ALEIT_GIVESITNOT) || (uarmc && uarmc->oartifact == ART_ORNA_S_DOUBLE) || (uarmc && uarmc->oartifact == ART_TOBIAS__IRONWEAVE_DRAPE && (moves % 5 == 0)) || (uarmu && uarmu->oartifact == ART_METALHOLD) || (uarms && uarms->oartifact == ART_DISTANT_THIRD) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) || carryingarti(ART_FORMFALL) || (uarmf && uarmf->oartifact == ART_ALMUT_S_NOCARE) ) return TRUE;
...
playerextrinsicfreeaction()
{
	...
	if ( (uwep && uwep->oartifact == ART_HEAVY_CROSSBOW_OF_ETERNITY) || (uamul && uamul->oartifact == ART_AMULET_OF_INGWE) || (uarm && uarm->oartifact == ART_THEY_SMOKED_POT_) || (uarm && uarm->oartifact == ART_VENGEANCE_OF_ETERNITY) || (uwep && uwep->oartifact == ART_AMALGAMATED_SKIES) || (uwep && uwep->oartifact == ART_ABSURD_OVERPOWEREDNESS) || autismringcheck(ART_BUT_YOU_MUST) || autismringcheck(ART_RING_OF_STEEL_DRAGON) || (uwep && uwep->oartifact == ART_PURE_BLACK_DIABLO) || (uwep && uwep->oartifact == ART_DAMON_S_SHOCK) || (uamul && uamul->oartifact == ART_HUMBLE_STUMBLE) || (uamul && uamul->oartifact == ART_DUMDUMDUM__DUMDUMDUM) || (uwep && uwep->oartifact == ART_MARC_S_WRONG_SPELL) || (uarmc && uarmc->oartifact == ART_FLECTION_INNA_BED) || (uarms && uarms->oartifact == ART_WENG_S_MAHT) || (uarms && uarms->oartifact == ART_KAVYA_S_HASPLE) || (uarmh && uarmh->oartifact == ART_SING_AND_JULIETTA_S_JOINT_) || (uarmc && uarmc->oartifact == ART_JENNELLE_S_IMMEDIATIVITY) || (uarmh && uarmh->oartifact == ART_TRACK_STOPID) ) return TRUE;
	if ( (uarmf && uarmf->oartifact == ART_ROUGHGRAVEL_MASTER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uarm && uarm->oartifact == ART_AYZAH_S_HURDLE) || (uarmf && uarmf->oartifact == ART_JANET_S_DETACHMENT) || (ublindf && ublindf->oartifact == ART_HEADHORN) || (uarmf && uarmf->oartifact == ART_RINGFIGHTER_S_GUARD) || (u.usteed && uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || (uarmf && uarmf->oartifact == ART_ERIK_S_MARCHING_BOOTS) || (uarm && uarm->oartifact == ART_IMPORTANT_DESIGN) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || (uwep && uwep->oartifact == ART_PERFECT_HARMONY) || carryingarti(ART_NARNIA_S_CLASSITY) || carryingarti(ART_LEGENDARIUM_PERCENTAGE) ) return TRUE;
	if ( carryingarti(ART_FREESWING) ) return TRUE;
...
playerextrinsicfireres()
{
	...
	if ((uamul && uamul->oartifact == ART_DEMOBLING) || (uarmf && uarmf->oartifact == ART_AMPERSAND_HAREM) || (uarmf && uarmf->oartifact == ART_DESEAMING_GAME) || (uarms && uarms->oartifact == ART_SPECTRATE_ETTECKOR) || (uwep && uwep->oartifact == ART_SPECTRATE_ETTECKOR) || (uarmf && uarmf->oartifact == ART_SILVESTERBLAM) || (uarmf && uarmf->oartifact == ART_MY_LOVE) || (uarmf && uarmf->oartifact == ART_OH_THE_ELEMENTS) || carryingarti(ART_SPECTRUM_PLING) || (uwep && uwep->oartifact == ART_DEATH_S_FATHOM) || (uarm && uarm->oartifact == ART_CHANGERING_ENVIROMENT) || (uarms && uarms->oartifact == ART_HEATEM_UP) || (uwep && uwep->oartifact == ART_PURPLE_SPECTRUM) || (uarm && uarm->oartifact == ART_COAL_PEER) || autismringcheck(ART_RING_OF_EVERYTHING_RESISTA) || (uarm && uarm->oartifact == ART_FIRE_COLD_AEFF) || (uarm && uarm->oartifact == ART_DUEUEUEUET) ) return TRUE;
	...
	if ( (uwep && uwep->oartifact == ART_ELEMENTAL_STAFF) || (uwep && uwep->oartifact == ART_JADE_TALON) || (uarmu && uarmu->oartifact == ART_CRIMSON_PLATE) || (uwep && uwep->oartifact == ART_DICHOTOMY) || bmwride(ART_KERSIE_PERSIE) || (have_spellcolorartifact() == RED_SPELL_STONE) || (uarm && uarm->oartifact == ART_EFHECK) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || autismringcheck(ART_RED_YELLOW_ORANGE) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarmh && uarmh->oartifact == ART_SPRING_BREEZE) || (uarmc && uarmc->oartifact == ART_DELLEN) || u.arti_carry_fragrantgirls || (uarmf && uarmf->oartifact == ART_MARIA_S_FLEECLE_FLOPS) || (uimplant && uimplant->oartifact == ART_FULLPOWER) || (uwep && uwep->oartifact == ART_ELEMENTALIST_S_CODEX) || carryingarti(ART_DESIREE_S_MMMMM_WORTHINESS) || carryingarti(ART_COMPLEX_BUILDED) || (uarmf && uarmf->oartifact == ART_RUE_SCENT) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) ) return TRUE;
...
playerextrinsiccoldres()
{
	...
	if ((uarmu && uarmu->oartifact == ART_GIVE_ME_STROKE__JO_ANNA) || (uarmf && uarmf->oartifact == ART_CLAUDIA_S_SELF_WILL) || (uarmf && uarmf->oartifact == ART_RUTH_S_UNDEAD_INLAY) || (uarmf && uarmf->oartifact == ART_NOW_YOU_LOOK_LIKE_A_BEGGAR) || (uwep && uwep->oartifact == ART_ELEMENTS_TIME_THREE) || (uarms && uarms->oartifact == ART_SPECTRATE_ETTECKOR) || (uwep && uwep->oartifact == ART_SPECTRATE_ETTECKOR) || (uarmf && uarmf->oartifact == ART_SARAH_S_SNEAKERS_OF_INSTAN) || carryingarti(ART_SPECTRUM_PLING) || (uarmf && uarmf->oartifact == ART_OH_THE_ELEMENTS) || (uarmf && uarmf->oartifact == ART_GRAVY_HIDE) || (uarmf && uarmf->oartifact == ART_DYKE_RESISTS) || (uarmf && uarmf->oartifact == ART_ONSET_OF_WINTER) ) return TRUE;
	if ((flags.moonphase >= 1 && flags.moonphase <= 3) && uarmc && uarmc->oartifact == ART_VERENA_S_CLOAK_OF_BLOOMING) return TRUE;
	...
	if ( (uarm && uarm->oartifact == ART_VERY_HOT_IN_HERE) || u.cryopancaketurns || (uwep && uwep->oartifact == ART_JADE_TALON) || (uwep && uwep->oartifact == ART_ELEMENTAL_STAFF) || (uarmc && uarmc->oartifact == ART_NUTKNU_KOLAS) || (uarmc && uarmc->oartifact == ART_FLEECY_CORN) || (uarmf && uarmf->oartifact == ART_SARAH_S_REPLACEMENT) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || (uwep && uwep->oartifact == ART_DICHOTOMY) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (have_spellcolorartifact() == WHITE_SPELL_STONE) || u.arti_carry_fragrantgirls || (uarmh && uarmh->oartifact == ART_SPRING_BREEZE) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarm && uarm->oartifact == ART_EFHECK) || (uarmf && uarmf->oartifact == ART_MARIA_S_FLEECLE_FLOPS) || (uwep && uwep->oartifact == ART_ELEMENTALIST_S_CODEX) || (uimplant && uimplant->oartifact == ART_FULLPOWER) ) return TRUE;
	if ( (uarmc && uarmc->oartifact == ART_INGRID_S_WEATHERED_SHROUD) || carryingarti(ART_UNDEAD_BECOME_HAVE) || (uarmf && uarmf->oartifact == ART_RUE_SCENT) ) return TRUE;
...
playerextrinsicshockres()
{
	...
	if ((uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (uarms && uarms->oartifact == ART_SPECTRATE_ETTECKOR) || (uwep && uwep->oartifact == ART_SPECTRATE_ETTECKOR) || (uarmf && uarmf->oartifact == ART_SARAH_S_SNEAKERS_OF_INSTAN) || carryingarti(ART_SPECTRUM_PLING) || (uarmf && uarmf->oartifact == ART_DYKE_RESISTS) || (uarmf && uarmf->oartifact == ART_OH_THE_ELEMENTS) || (uwep && uwep->oartifact == ART_DEATH_S_FATHOM) ) return TRUE;
	...
	if ( (uwep && uwep->oartifact == ART_ELEMENTAL_STAFF) || (uarm && uarm->otyp == TESLA_POWER_ARMOR) || (uarmf && uarmf->oartifact == ART_SARAH_S_REPLACEMENT) || autismringcheck(ART_RING_OF_STEEL_DRAGON) || (uwep && uwep->oartifact == ART_JADE_TALON) || (uarmu && uarmu->oartifact == ART_EVERY_EVENTUALITY) || autismringcheck(ART_RED_YELLOW_ORANGE) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarmf && uarmf->oartifact == ART_MARIA_S_FLEECLE_FLOPS) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (have_spellcolorartifact() == BLUE_SPELL_STONE) || (uimplant && uimplant->oartifact == ART_FULLPOWER) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || carryingarti(ART_EARTHSPARK) || (uarm && uarm->oartifact == ART_EFHECK) ) return TRUE;
...
playerextrinsicpoisonres()
{
	...
	if (autismringcheck(ART_FOLL_DECK) || u.arti_carry_fragrantgirls || (uwep && uwep->oartifact == ART_MECHINEFELL) || (uchain && uchain->oartifact == ART_MECHINEFELL) || (uball && uball->oartifact == ART_HIGH_AFFIX) || (have_spellcolorartifact() == GREEN_SPELL_STONE) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_PULSE_NODE_OF_GERALT) || (uarmf && uarmf->oartifact == ART_DORIKA_S_COLORBLOCK) || (uarm && itemhasappearance(uarm, APP_GROMY_ROBE)) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || carryingarti(ART_DECADENCE_IN_DOSES) || carryingarti(ART_UNDEAD_BECOME_HAVE) || (uarmf && uarmf->oartifact == ART_MARIA_S_FLEECLE_FLOPS) || (uarmc && uarmc->oartifact == ART_BOLWING_S_RAGS) || carryingarti(ART_GREENFACE) || (uarm && uarm->oartifact == ART_EFHECK) ) return TRUE;
...
playerextrinsicflying()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ALMIS_MANICIA) || (uarms && uarms->oartifact == ART_FIVEFLY) || (uarm && uarm->oartifact == ART_GREAT_TOME) || autismringcheck(ART_ABOVECLOUD) || (uball && uball->oartifact == ART_GLIDEN_WIDE) || (uarmf && uarmf->oartifact == ART_WINGLE_BOOTS) || autismringcheck(ART_WEMPEN_LO) || (uarmh && uarmh->oartifact == ART_ARABELLA_S_BEAUTY_BIRD) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GRAV_NODE_OF_SERAPHIS_VALE) || (uwep && uwep->oartifact == ART_HELICOPTER_CHAIN) || (uarmf && uarmf->oartifact == ART_WINGELWINGELWINGEL) || (uarmc && uarmc->oartifact == ART_DRIVECHAIR_OF_ETERNITY) || (uchain && uchain->oartifact == ART_HELICOPTER_CHAIN) || (uarmf && uarmf->oartifact == ART_SOLVEJG_S_SQUEEZE_IN) || (uarmc && uarmc->oartifact == ART_SETH_S_FLYING_CAPE) ) return TRUE;
	if ( (uarmf && uarmf->oartifact == ART_OTMAR_S_PHANTOM_STEPS) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || (ublindf && ublindf->oartifact == ART_MONIQUE_S_NONSENSE) || carryingarti(ART_GRACIOUS_SWAN) || carryingarti(ART_HOPSCOTCH_CUP) ) return TRUE;
...
playerextrinsicspeed()
{
	...
	if ( (uchain && uchain->oartifact == ART_JORMUNGANDR_S_COIL) || (uimplant && uimplant->oartifact == ART_GIULY_AH) || (uarm && uarm->oartifact == ART_MOTA_ROLA) || u.tempveryfastspeed || (uarmc && uarmc->oartifact == ART_CLARA_S_WINDCLOAK) || bmwride(ART_KERSTIN_S_PROJECTION_LEAD) || (uarmc && uarmc->oartifact == ART_HUNTING_TIME_HINT) || (uarmf && uarmf->oartifact == ART_ADHS_DIAGNOSIS) || (uarmg && uarmg->oartifact == ART_GLENNIS_DOWNS) || (uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || bmwride(ART_MARLOWE_S_CAMEL) || (uarmf && uarmf->oartifact == ART_MAREN_S_GALE_BOOTS) || autismringcheck(ART_TANGO_ALPHA_BRAVO) || (uarmf && uarmf->oartifact == ART_DARIUS_S_STORMWALKERS) || carryingarti(ART_FOR_ME_IT_ALWAYS_GOES_FORW) || carryingarti(ART_UNSTOPPABLE_RUNNER) || (uwep && uwep->oartifact == ART_PIPPA_S_URGE) ) return TRUE;
...

{
	...
	if (uarm && uarm->oartifact == ART_OAR_SUPER_GRAPHICS_CARD_BU && rn2(2)) {
		pline("Your armor prevents the bad effect!");
		return;
	}
	...
	if (uarmu && uarmu->oartifact == ART_SOLINE_S_RUFFLE && rn2(10)) {
		pline("Your shirt prevents the bad effect!");
		return;
	}

	if (uarmf && uarmf->oartifact == ART_LUSTY_GIGGLE && rn2(2)) { /* only works against regular bad effects, not really bad ones --Amy */
		pline("Your giggling prevents the bad effect!");
		return;
	}

	if (uwep && uwep->oartifact == ART_RADIANT_WARDEN && rn2(2)) {
		pline("Your warden prevents the bad effect!");
		return;
	}
...

{
	...
	if (uarm && uarm->oartifact == ART_OAR_SUPER_GRAPHICS_CARD_BU && !rn2(3)) {
		pline("Your armor prevents the bad effect!");
		return;
	}
	...
	if (uarmu && uarmu->oartifact == ART_SOLINE_S_RUFFLE && rn2(5)) {
		pline("Your shirt prevents the bad effect!");
		return;
	}
...
gain_nastinator()
{
	/* keyword: timerun */
		...
		switch (rnd(288)) {
			...
			case 286: u.nastinator286 = 1; break;
			case 287: u.nastinator287 = 1; break;
			case 288: u.nastinator288 = 1; break;
		}

}

/* cure all ongoing nasty traps, by Amy; only works on intrinsics (permanent or timeout), not ones caused by an item */
void
cure_nasty_traps()
{
	...
	SoundEffectBug = 0L;
	TimerunBug = 0L;
	AntimatterProblem = 0L;
	BoundDayChoiceEffect = 0L;
	MulchBug = 0L;
	SpellCooldownBug = 0L;
	HaveAPinBug = 0L;
	TurbodullBug = 0L;
...
boolean canresist; /* can the drain be resisted by equipment, being certain roles etc.? TRUE if it can */
{
	...
	if (canresist && rn2(10) && statdrained == A_CHA && (uarmf && uarmf->oartifact == ART_MAURAH_S_ABSOLUTE_HEART_SH)) {
		Your("heels prevent the stat drain!");
		return;
	}

	if (canresist && uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES && rn2(10)) {
		Your("cloak prevents the stat drain!");
		return;
	}

	if (canresist && uarmf && uarmf->oartifact == ART_LEGENDARY_JESSICA && rn2(10)) {
		Your("sandals prevent the stat drain!");
		return;
	}
...
have_anorexia()
{
	...
	if (uarmg && uarmg->oartifact == ART_ILLNESS_HEAL) return TRUE;
...
int debtamount;
boolean guaranteed; /* if TRUE, the player WILL get the debts, no matter which resistances to debt-increasing effects they have --Amy */
{
	if (guaranteed) {
		/* guaranteed debts are supposed to be *completely impossible to resist* --Amy */
		u.superdebt += debtamount;
	} else {
		if (uarmh && uarmh->oartifact == ART_BIG_REAR_END) {
			You("refuse to pay debts.");
		} else {
...
register struct obj *dwpn;
{
	...
	if (dwpn->oartifact == ART_ECLIPSE_MAW) return FALSE;

	if (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) return FALSE;
	...
	if (TurbodullBug || u.uprops[TURBODULL_BUG].extrinsic || have_turbodullstone() || (uarmf && uarmf->oartifact == ART_GUELDRE_S_COIN_CLING) || (ublindf && ublindf->oartifact == ART_ARABELLA_S_SEE_NO_EVIL_MON)) minenchant -= 5;
	...
	if (TurbodullBug || u.uprops[TURBODULL_BUG].extrinsic || have_turbodullstone() || (uarmf && uarmf->oartifact == ART_GUELDRE_S_COIN_CLING) || (ublindf && ublindf->oartifact == ART_ARABELLA_S_SEE_NO_EVIL_MON)) dullchance /= 5;
...
boolean canresist; /* if this is TRUE, mithril equipment and other stuff allows you to resist */
{
	...
	if (canresist && uarmf && uarmf->oartifact == ART_LEGENDARY_JESSICA && rn2(4)) {
		pline("Your sandals prevent the stat loss!");
		return;
	}

	if (canresist && uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES && rn2(50)) {
		pline("Your cloak prevents the stat loss!");
		return;
	}
...
/* allow the player to select any martial arts style they want, including ones not available via martial style tech --Amy */
void
choosemartialstyle()
{
	...
	while (madechoice == 0) {
		...
		} else if (yn("Switch to the 'capoeira' style (improved AC and increased to-hit/damage with fists after you've just moved, non-green monsters spawn hostile, standing still for too long reduces your AC and melee to-hit)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_CAPOEIRA;
		...
		} else if (yn("Switch to the 'judo' style (break holding attacks, boosts dexterity, more frequency for special martial arts strikes, reduces your damage output)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_JUDO;
		} else if (yn("Switch to the 'taekwondo' style (powers up your kick attack, can block projectiles if both of your hands are free, can also be used while wielding a weapon, your melee attacks have reduced to-hit)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_TAEKWONDO;
		} else if (yn("Switch to the 'muay thai' style (speeds you up, boosts unarmed damage, makes your AC worse, your spells fail more often and are more expensive, your ranged weapons have reduced to-hit)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_MUAYTHAI;
		} else if (yn("Switch to the 'silat' style (boosts unarmed to-hit, unarmed attacks can sometimes stun opponents, improves your AC, makes your inventory weigh more and slows you down slightly)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_SILAT;
		} else if (yn("Switch to the 'krav maga' style (greatly boosts unarmed damage, causes aggravate monster, reduces your regeneration rate and can send enemies into a frenzy)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_KRAVMAGA;
		} else if (yn("Switch to the 'bojutsu' style (requires quarterstaff, allows you to block projectiles with the staff, boosts your AC and allows you to sometimes evade rays, reduces your melee damage output)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_BOJUTSU;
		} else if (yn("Switch to the 'kung fu' style (slightly improves your unarmed and kick damage and boosts unarmed to-hit, kick hits enemies more often, your weapons are less effective)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_KUNGFU;
		} else if (yn("Switch to the 'haidong gumdo' style (requires jeontu geom, boosts melee damage and to-hit and allows you to use martial arts techniques while wielding a jeontu geom, reduces your melee to-hit whenever you're not wielding a jeontu geom)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_HAIDONGGUMDO;
		} else if (yn("Switch to the 'karate' style (higher unarmed to-hit, kick cannot be clumsy and not be dodged, can occasionally avoid bad effects, aggravates monsters, reduces your ability to use weapons)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_KARATE;
		} else if (yn("Switch to the 'marshal arts' style (armor and shield don't hinder your martial prowess, reduced dexterity)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_MARSHALARTS;
		} else if (yn("Switch to the 'wrestling' style (dual-wielding with at least one hand free occasionally paralyzes enemies, better STR/DEX, less INT/WIS, magic vacuum, fuck over and killer spawns)?")=='y') {
 			madechoice = 1; u.martialstyle = MARTIALSTYLE_WRESTLING;
		} else if (yn("Switch to the 'kobudo' style (bonus damage with iron chains and they occasionally debuff enemies, reduced melee to-hit)?")=='y') {
			madechoice = 1; u.martialstyle = MARTIALSTYLE_KOBUDO;
		} else if (yn("Switch to the 'booyakasha' style (knuckles deal more damage on consecutive hits, teleport control while wielding knuckles, resist fear, reduces INT, CON and WIS, doubles alignment losses)?")=='y') {
...
nivellate()
{
	...
	if (type == 0) { /* HP */
		...
		if (powerfulimplants() && uimplant && uimplant->oartifact == ART_VITAELITAET) {
			lowerceiling *= 6;
			lowerceiling /= 5;
			upperceiling *= 6;
			upperceiling /= 5;
		...
		if (uimplant && uimplant->oartifact == ART_CORE_HALO_OF_AURELION_THE_) {
			lowerceiling *= 11;
			lowerceiling /= 10;
			upperceiling *= 11;
			upperceiling /= 10;
		}
		if (uimplant && uimplant->oartifact == ART_COVID_DOES_NOT_EXIST) {
			lowerceiling *= (powerfulimplants() ? 10 : 6);
			lowerceiling /= 5;
			upperceiling *= (powerfulimplants() ? 10 : 6);
			upperceiling /= 5;
		}
		if (Race_if(PM_ZAUR)) {
			lowerceiling *= 3;
...
boost_power_value()
{
	...
	if (uleft && uleft->oartifact == ART_TANGO_ALPHA_BRAVO) boostvalue += 5;
	if (uright && uright->oartifact == ART_TANGO_ALPHA_BRAVO) boostvalue += 5;
 ...
 * this effect can be resisted, while the increasesanity_noeffect below can not (this is by design) */
void
...
int snamount;
{
	...
	if (uarmc && uarmc->oartifact == ART_DAENNAE_DAENNAE_SCHNAEDDAE && rn2(10)) return;
	...
	if (uimplant && uimplant->oartifact == ART_NIENNA_S_FEELING) {
		snamount /= (powerfulimplants() ? 5 : 2);
	}

	if (uarm && uarm->oartifact == ART_BEXLEY_S_CUTESY) snamount /= 2;
...
int drainamount;
{
	...
	if (uarmf && uarmf->oartifact == ART_SEE_BY_LAKE) {
		if (drainamount == 1) {
			if (rn2(10)) return;
		} else {
			if (rn2(2)) return;
...
register struct obj *otmp;
{
	...
	if (otmp && otmp->oartifact == ART_ALCHEM_LOAD) {
		make_bottle(TRUE);
		make_bottle(TRUE);
		make_bottle(TRUE);
		make_bottle(TRUE);
	...
	register struct obj	*otmp;
{
	...
	case POT_WATER:
		...
		if (otmp->oartifact == ART_EMERGENCY_WATER) {
			u.uhunger += 1000;
			newuhs(FALSE);
			pline("That water really hit the spot!");
		}
	...
	register boolean curesick, cureblind;
{
	...
	if (uimplant && uimplant->oartifact == ART_PULSE_NODE_OF_GERALT) {
		nhp *= 13;
		nhp /= 10;
	}
...
int healamount;
{
	...
	if (powerfulimplants() && uimplant && uimplant->oartifact == ART_IMANI_S_POINT) {
		amounttoheal *= 2;
		youmonst.movement += 6;
	}
 ...
 */
{
	...
	if (carried(obj)) {
		...
		owornmask &= ~W_ARMU;
	    if (owornmask & W_TOOL && obj->otyp != BLINDFOLD && obj->otyp != CLIMBING_SET && obj->otyp != PROTECTIVE_GLASSES && obj->otyp != DEFUSING_BOX && obj->otyp != EYECLOSER && obj->otyp != DRAGON_EYEPATCH && obj->otyp != CONDOME && obj->otyp != SOFT_CHASTITY_BELT &&
...
dodip()
{
	...
	if (IS_WELL(here)) { /* item gets wet, no other effects --Amy */
		...
		if (yn(qbuf) == 'y') {
		    if (Levitation) {
			floating_above(tmp);
		    } else if (u.usteed && !(uwep && uwep->oartifact == ART_SORTIE_A_GAUCHE) && !carryingarti(ART_HEIGHTBOOST) && !(powerfulimplants() && uimplant && uimplant->oartifact == ART_READY_FOR_A_RIDE) && !FemtrapActiveKerstin && !bmwride(ART_JAFARO_ON_TOUR) && !bmwride(ART_MARIKE_S_WALL_BUTT) && !(bmwride(ART_DEEPER_LAID_BMW)) && !is_swimmer(u.usteed->data) && !u.usteed->egotype_watersplasher &&
...
{
	static int skates = 0;
	if (!skates) skates = find_appearance_armor(APP_SNOW_BOOTS);
	static int skates2 = 0;
	if (!skates2) skates2 = find_appearance_armor(APP_WINTER_BOOTS);
	static int skates3 = 0;
	if (!skates3) skates3 = find_appearance_armor(APP_WINTER_STILETTOS);
	static int skates4 = 0;
	if (!skates4) skates4 = find_appearance_armor(APP_SKI_HEELS);
	static int skates5 = 0;
	if (!skates5) skates5 = find_appearance_armor(APP_CYAN_SNEAKERS);
	...
	if (uarmf && uarmf->oartifact == ART_GODOT_S_SUIT_SHOES) return TRUE;
