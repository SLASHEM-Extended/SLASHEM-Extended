...
playerextrinsicmagicalbreath()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_MANTLE_OF_THE_MANTA_RAY) || (uarmc && uarmc->oartifact == ART_OUT_OF_THE_RAIN) || (uwep && uwep->oartifact == ART_TEZCATLIPOCA_S_BUBBLESTORM) || (uarmf && uarmf->oartifact == ART_AIR_BLISTER) || (uarms && uarms->oartifact == ART_FLIERHACH) || (uarmg && uarmg->oartifact == ART_WATER_WINGS) ) return TRUE; 
...
playerextrinsicpsires()
{
	...
	if ( (uamul && uamul->oartifact == ART_PRECIOUS_UNOBTAINABLE_PROP) || (uarm && uarm->oartifact == ART_MINDMEND) || autismringcheck(ART_KONDRE) || (uamul && uamul->oartifact == ART_ISRA_S_ANGLE) || (uarmh && uarmh->oartifact == ART_PSYSTRYKE) || (uarm && uarm->oartifact == ART_ULTRAVIOLET_RAINBOW) ) return TRUE;
...
playerextrinsicsleepres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_CLOAK_OF_THE_UNHELD_ONE) || (uarmc && uarmc->oartifact == ART_CLOAK_OF_THE_UNHELD_POTATO) || (uarmc && uarmc->oartifact == ART_STRIPED_SHIRT_OF_THE_FALSE) || (uarmh && uarmh->oartifact == ART_CROWN_OF_THE_PERCIPIENT) || (uarmf && uarmf->oartifact == ART_SYLVIE_S_YAWN) || (uarmc && uarmc->oartifact == ART_RUFFLED_SHIRT_OF_THE_ARIST) || (uarmc && uarmc->oartifact == ART_NUTKNU_KOLAS) || (uarmh && uarmh->oartifact == ART_ENSNARED_MIND) || autismringcheck(ART_BUT_YOU_MUST) || (uarmf && uarmf->oartifact == ART_DAPHNE_S_BOREDOM) || autismringcheck(ART_IT_S_NOT_BEDTIME_YET) || autismringcheck(ART_GRISHUH) || (ublindf && ublindf->oartifact == ART_TOTALLY_THE_SPECTRUM) || (uarmc && uarmc->oartifact == ART_WAKESTAY) || (uarms && uarms->oartifact == ART_KAVYA_S_HASPLE) || (uarmu && uarmu->oartifact == ART_SETH_S_ERROR) ) return TRUE;
...
playerextrinsicflying()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ALMIS_MANICIA) || (uarms && uarms->oartifact == ART_FIVEFLY) || (uarm && uarm->oartifact == ART_GREAT_TOME) || autismringcheck(ART_ABOVECLOUD) || (uarmf && uarmf->oartifact == ART_WINGLE_BOOTS) || autismringcheck(ART_WEMPEN_LO) || (uarmh && uarmh->oartifact == ART_ARABELLA_S_BEAUTY_BIRD) ) return TRUE;
