...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) ) return TRUE;
...
playerextrinsicdiscountaction()
{
	...
	if ( (uarmg && uarmg->oartifact == ART_INTERMINTER) ) return TRUE;
