...
playerextrinsicdeathres()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) ) return TRUE;
...
playerextrinsicfreeaction()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ROUGHGRAVEL_MASTER) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uarm && uarm->oartifact == ART_AYZAH_S_HURDLE) || (uarmf && uarmf->oartifact == ART_JANET_S_DETACHMENT) || (ublindf && ublindf->oartifact == ART_HEADHORN) || (uarmf && uarmf->oartifact == ART_RINGFIGHTER_S_GUARD) || (u.usteed && uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || (uarmf && uarmf->oartifact == ART_ERIK_S_MARCHING_BOOTS) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || (uwep && uwep->oartifact == ART_PERFECT_HARMONY) ) return TRUE;
