...
playerextrinsicdeathres()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || (uarm && uarm->oartifact == ART_HANGING_THREAD) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) || carryingarti(ART_FINISHED_ANYWAY) || carryingarti(ART_MIRIAM_S_FEELING) || carryingarti(ART_EROGE_CHEEK) || (uwep && uwep->oartifact == ART_BLACK_MAW) || carryingarti(ART_LAST_PART) || (uwep && uwep->oartifact == ART_DEATH_MAW) ) return TRUE;
...
playerextrinsicpsires()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_SPIREHEAD) || carryingarti(ART_CASTLEPSYCHE) ) return TRUE;
