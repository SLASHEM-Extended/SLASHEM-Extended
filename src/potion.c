...
playerextrinsicdrainres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_ALMIS_MANICIA) || (have_spellcolorartifact() == PINK_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_JESSICA_S_RAINBOW) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_BLOODPUMP_OF_HARKON_BLOODR) ) return TRUE;
...
playerextrinsicsickres()
{
	...
	if ( (uarmc && uarmc->oartifact == ART_EVERY_CONDITION_SUIT) || (uarmg && uarmg->oartifact == ART_HOBART_THE_GOLDEN) || (uarmg && uarmg->oartifact == ART_GASTER_FIL) || (uarmc && uarmc->oartifact == ART_KWULL_GONG) || (uarmc && uarmc->oartifact == ART_LIGNES_ENZYMES) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (uimplant && uimplant->oartifact == ART_VIRAL_CORE_OF_ISKAR_THE_PA) || (uarmh && uarmh->oartifact == ART_SAMIRA_S_EXPRESSION) || (uarmg && uarmg->oartifact == ART_MATTHIAS_S_GAUNTLETS_OF_FA) || (have_spellcolorartifact() == VIOLET_SPELL_STONE) || (uarmc && uarmc->oartifact == ART_BOLWING_S_RAGS) || (uarmf && uarmf->oartifact == ART_FUNGUS_ERADICATION) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_GENDEREQUALNESS) ) return TRUE;
...
playerextrinsicfearres()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_DON_T_RUN_IN_FEAR) || (uarmh && uarmh->oartifact == ART_FREDERIK_S_COMBAT_HELMET) || (uarmc && uarmc->oartifact == ART_EMIL_S_MANTLE_OF_RESOLVE) || (have_spellcolorartifact() == CYAN_SPELL_STONE) || (uarmf && uarmf->oartifact == ART_ELLA_S_VENGEANCE) || (uarmg && uarmg->oartifact == ART_CORVIN_S_GRIP_OF_PERSISTEN) ) return TRUE;
...
playerextrinsicdeathres()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_HILMA_S_AGE) || (uarmf && uarmf->oartifact == ART_MARTINA_S_SUPERFLEECE) || (uarm && uarm->oartifact == ART_HANGING_THREAD) || (uarmc && uarmc->oartifact == ART_NIMER_AGAIN) || (uwep && uwep->oartifact == ART_BLACK_MAW) || (uwep && uwep->oartifact == ART_DEATH_MAW) ) return TRUE;
...
playerextrinsicmysteryres()
{
	...
	if (have_doubleshelljewel()) return TRUE;
...
playerextrinsicdiscountaction()
{
	...
	if ( (uarmg && uarmg->oartifact == ART_INTERMINTER) || (uarmc && uarmc->oartifact == ART_GERMAN_CHANCELLOR_SAYS_) || (powerfulimplants() && uimplant && uimplant->oartifact == ART_DIDI_S_LEG_PAINTING) || (uarmf && uarmf->oartifact == ART_MOST_EROTIC_AIR_CURRENT_NO) || (u.usteed && uamul && uamul->oartifact == ART_ADELAIDE_S_RACING) || (uarmf && uarmf->oartifact == ART_OFFZEEH_BRUN) || (uarmu && uarmu->oartifact == ART_STOCKTON_BRAND) ) return TRUE;
...
playerextrinsicmagicalbreath()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_DANTE_S_ASHEN_VISOR) || (uarmg && uarmg->oartifact == ART_CLARA_S_GLOVES_OF_THE_TIDE) ) return TRUE;
...
playerextrinsicpsires()
{
	...
	if ( (uarmh && uarmh->oartifact == ART_SPIREHEAD) ) return TRUE;
...
playerextrinsicstoneres()
{
	...
	if (autismringcheck(ART_FIRMBLOCK) || (uarmc && uarmc->oartifact == ART_TOBIAS__IRONWEAVE_DRAPE && (moves % 5 == 0)) || (uarmf && uarmf->oartifact == ART_PORYSTONE) || (uarmf && uarmf->oartifact == ART_ANASTASIA_S_PLAYTIME) ) return TRUE;
...
playerextrinsicflying()
{
	...
	if ( (uarmf && uarmf->oartifact == ART_OTMAR_S_PHANTOM_STEPS) || (ublindf && ublindf->oartifact == ART_SORELIE_S_ROPE) || (ublindf && ublindf->oartifact == ART_MONIQUE_S_NONSENSE) ) return TRUE;
...
dodip()
{
	...
	if (IS_WELL(here)) { /* item gets wet, no other effects --Amy */
		...
		if (yn(qbuf) == 'y') {
		    if (Levitation) {
			floating_above(tmp);
		    } else if (u.usteed && !(uwep && uwep->oartifact == ART_SORTIE_A_GAUCHE) && !(powerfulimplants() && uimplant && uimplant->oartifact == ART_READY_FOR_A_RIDE) && !FemtrapActiveKerstin && !bmwride(ART_JAFARO_ON_TOUR) && !bmwride(ART_MARIKE_S_WALL_BUTT) && !(bmwride(ART_DEEPER_LAID_BMW)) && !is_swimmer(u.usteed->data) && !u.usteed->egotype_watersplasher &&
