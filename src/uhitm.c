...
#undef useup_eggs
	...
	if ((!hittxt || (Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_CAMPERSTRIKER) || ishaxor || Hallucination) ) &&			/*( thrown => obj exists )*/
	  (!destroyed || (thrown && m_shot.n > 1 && m_shot.o == obj->otyp))) {
		...
		else if ((Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_CAMPERSTRIKER) || ishaxor || Hallucination) && !rn2(20) && (PlayerHearsSoundEffects) ) {
...
/* For vamps */
		...
		if (!already_killed) {
			...
			if ((Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_CAMPERSTRIKER) || ishaxor || Hallucination) && !thrown && !rn2(20) && (PlayerHearsSoundEffects) ) {
...
struct attack *mattk;
{
	...
	while ((otmp = mdef->minvent) != 0) {
	    ...
	    if (otmp && otmp->mstartinvent && !(otmp->oartifact) && !(otmp->fakeartifact) && (!rn2(3) || (rn2(100) < u.musableremovechance) || LootcutBug || u.uprops[LOOTCUT_BUG].extrinsic || have_lootcutstone() || !timebasedlowerchance() ) && !stack_too_big(otmp) ) {
			delobj(otmp);
			return;
	    }
	    if (otmp && otmp->mstartinventB && !(otmp->oartifact) && !(otmp->fakeartifact) && (!rn2(4) || (rn2(100) < u.equipmentremovechance) || !timebasedlowerchance() ) && !stack_too_big(otmp)) {
			delobj(otmp);
			return;
	    }
...
register int roll;
{
	...
	} else if ((Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_CAMPERSTRIKER) || ishaxor || Hallucination) && !rn2(20) && canspotmon(mdef) && flags.verbose && (PlayerHearsSoundEffects) ) {
...
struct attack *mattk;		/* null means we find one internally */
{
	...
	case AD_LAVA:
	    ...
	    if(!mon->mcan && is_flammable(obj) && !obj->oerodeproof && obj->otyp != SPE_BOOK_OF_THE_DEAD && obj->otyp != AMULET_OF_YENDOR && obj->otyp != CANDELABRUM_OF_INVOCATION && obj->otyp != BELL_OF_OPENING && obj->oartifact != ART_KEY_OF_LAW && obj->oartifact != ART_KEY_OF_CHAOS && obj->oartifact != ART_KEY_OF_NEUTRALITY ) {
			...
			else if (obj->oeroded == MAX_ERODE && (!rn2(2) || !(uarmf && uarmf->oartifact == ART_LUISA_S_IRRESISTIBLE_CHARM) ) && carried(obj) )
			{
	...
	/* Unfortunately I can't seem to make it work for non-player inventory objects without crashing. --Amy */
			...
			else if (obj->oeroded2 == MAX_ERODE && (!rn2(2) || !(uarmf && uarmf->oartifact == ART_LUISA_S_IRRESISTIBLE_CHARM) ) && carried(obj) )
			{
	...
	case AD_SHRD:
		...
		else if(obj->otyp != SPE_BOOK_OF_THE_DEAD && obj->otyp != AMULET_OF_YENDOR && obj->otyp != CANDELABRUM_OF_INVOCATION && obj->otyp != BELL_OF_OPENING && obj->oartifact != ART_KEY_OF_LAW && obj->oartifact != ART_KEY_OF_CHAOS && obj->oartifact != ART_KEY_OF_NEUTRALITY ) {
