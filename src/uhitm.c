...
/* For vamps */
		...
		if (!already_killed) {
			...
			if ((Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_CAMPERSTRIKER) || Role_if(PM_HUSSY) || Role_if(PM_GANG_SCHOLAR) || ishaxor || Hallucination) && !thrown && !rn2(20) && (PlayerHearsSoundEffects) ) {
				...
				case 431: pline("%s dies and unlocks the 'Killed by %s' achievement in the process!", Monnam(mon), plname); break;
				...
				case 446: pline("%s says 'All right, %s, I know you'd stand no chance against me in a serious fight but I'm feeling generous, so I'm letting you win this time.'", Monnam(mon), plname); break;
...
register struct attack *mattk;
{
 ...
 physical:
	...
	if (mdef->data == &mons[PM_LITTLE_POISON_IVY] || mdef->data == &mons[PM_CHAREY] || mdef->data == &mons[PM_SWEET_ASIAN_POISON_IVY] || mdef->data == &mons[PM_FIRST_DUNVEGAN] || mdef->data == &mons[PM_PERCENTI_HAS_LOST___] || mdef->data == &mons[PM_PERCENTI_IS_IMMUNE_TO_THE_ATTACK_]) {
...
register int roll;
{
	...
	if (could_seduce(&youmonst, mdef, mattk) && rn2(5) ) {
		...
		case 487: pline("%s sticks out %s tongue to mock you for your weak attacks.", Monnam(mdef)); break;
