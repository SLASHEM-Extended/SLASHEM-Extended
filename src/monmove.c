...
boolean digest_meal;
{
	...
	/* super regene */
	if (mon->data == &mons[PM_MESHERA_ALPHA_DEFORMED_ANGEL] || mon->data == &mons[PM_TESTER] || mon->data == &mons[PM_JUERE_DEMON_SOLDIER] || mon->data == &mons[PM_JENNIFER_DEMON_SOLDIER] || mon->data == &mons[PM_RENAI_OVER_MESHERA]) {
		mon->mhp += 20;
		if (mon->mhp > mon->mhpmax) mon->mhp = mon->mhpmax;
	}
...
postmov:
	if(mmoved == 1 || mmoved == 3) {
		...
		if (ptr == &mons[PM_GHOUL] || ptr == &mons[PM_GHAST] || ptr == &mons[PM_DOGSHIT_SEARCHER] || ptr == &mons[PM_STINKING_ALIEN] || ptr == &mons[PM_GASTLY] || ptr == &mons[PM_PHANTOM_GHOST]
