...
madnesseffect:
		...
		case SPELL_COOLDOWN_TRAP:
			if (SpellCooldownBug) break;
			u.cnd_nastytrapamount++;

			SpellCooldownBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case TURBODULL_TRAP:
			if (TurbodullBug) break;
			u.cnd_nastytrapamount++;

			TurbodullBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;
		...
		case REDDAM_TRAP:
			...
			ReddamBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;
		...
		case REDINC_TRAP:
			...
			RedincBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;
		...
		case MULCH_TRAP:
			...
			MulchBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case INACCURACY_TRAP:
			if (InaccuracyBug) break;
			u.cnd_nastytrapamount++;

			InaccuracyBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case MONSTER_MULTISHOT_TRAP:
			if (MonsterMultishotBug) break;
			u.cnd_nastytrapamount++;

			MonsterMultishotBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case DO_YOU_HAVE_A_PIN_TRAP:
			if (HaveAPinBug) break;
			u.cnd_nastytrapamount++;

			HaveAPinBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case COSTLY_FAILURE_TRAP:
			if (CostlyFailureBug) break;
			u.cnd_nastytrapamount++;

			CostlyFailureBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;

		case INVENTORY_SIZE_TRAP:
			if (InventorySizeBug) break;
			u.cnd_nastytrapamount++;

			InventorySizeBug = rnz(nastytrapdur * (monster_difficulty() + 1));
			break;
		 ...
		 case ANTIMATTER_TRAP:
			...
			if (AntimatterProblem) break;
			u.cnd_nastytrapamount++;

			AntimatterProblem = rnz(nastytrapdur * (monster_difficulty() + 1));

		 break;

		 case BOUND_DAY_CHOICE_TRAP:

			if (BoundDayChoiceEffect) break;
			u.cnd_nastytrapamount++;

			BoundDayChoiceEffect = rnz(nastytrapdur * (monster_difficulty() + 1));

		 break;

		 case ANTISWITCH_TRAP:

			if (AntiswitchBug) break;
			u.cnd_nastytrapamount++;

			AntiswitchBug = rnz(nastytrapdur * (monster_difficulty() + 1));
...
skillrandomizeredo:
		...
		case LITTLE_MARIE_TRAP:
		 ...
		 case CHLOE_TRAP:
			...
			if (!FemtrapActiveRuth) {
				pline("Whoops... you seem to have stumbled into a trap that was set by Chloe.");
				pline("Oh no, you'll have to spend all your money for a pair of designer heels! As if you didn't have enough money troubles already!");
			}
			u.cnd_feminismtrapamount++;
			if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
			if (Role_if(PM_EMERA)) emerafrenzy();

			FemaleTrapChloe = rnz(femmytrapdur * (monster_difficulty() + 1));
			if (rn2(3)) FemaleTrapChloe += 100;
			if (!rn2(3)) FemaleTrapChloe += rnz(500);
		 ...
		 case JUDITH_TRAP:
			...
			if (!FemtrapActiveRuth) {
				pline("Whoops... you seem to have stumbled into a trap that was set by Judith.");
				pline("Now you have to do battle whenever you want to wear a pair of high heels, and if you lose, you're not allowed to wear them!");
			}
			u.cnd_feminismtrapamount++;
			if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
			if (Role_if(PM_EMERA)) emerafrenzy();

			FemaleTrapJudith = rnz(femmytrapdur * (monster_difficulty() + 1));
			if (rn2(3)) FemaleTrapJudith += 100;
			if (!rn2(3)) FemaleTrapJudith += rnz(500);

		 break;

		 case SABINE_TRAP:

			if (FemaleTrapSabine) break;
			if (!dontreveal && !FemtrapActiveRuth) seetrap(trap);

			if (!FemtrapActiveRuth) {
				pline("Whoops... you seem to have stumbled into a trap that was set by Sabine.");
				pline("You're afraid that the girls will kick you in the back of your %s!", body_part(HEAD) );
			}
			u.cnd_feminismtrapamount++;
			if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
			if (Role_if(PM_EMERA)) emerafrenzy();

			FemaleTrapSabine = rnz(femmytrapdur * (monster_difficulty() + 1));
			if (rn2(3)) FemaleTrapSabine += 100;
			if (!rn2(3)) FemaleTrapSabine += rnz(500);
		...
		break;
		 ...
		 case NASTINESS_TRAP:
			...
			case 276: MulchBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 277: SpellCooldownBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 278: HaveAPinBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 279: TurbodullBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 280: InaccuracyBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 281: InventorySizeBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 282: CostlyFailureBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 283: ReddamBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 284: RedincBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 285: MonsterMultishotBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 286: AntiswitchBug += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 287: BoundDayChoiceEffect += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
			case 288: AntimatterProblem += rnz(nastytrapdur * (monster_difficulty() + 1)); break;
...
register struct monst *mtmp;
{
	...
	if (!trap) {
	    ...
	    register int tt = trap->ttyp; /* suuuuuuuper great variable name, vanilla devteam. why can't it be named "ttype" --Amy */
...
glovecheck:		    target = which_armor(mtmp, W_ARMG);
 ...
 mfiretrap:
		...
		/* nasty traps: they can trigger if the monster has MS_NASTYTRAP --Amy */
		case SUPERSCROLLER_TRAP:
		case NUPESELL_TRAP:
		case ACTIVE_SUPERSCROLLER_TRAP:
		case RMB_LOSS_TRAP:
...
/* hit player's inventory with antimatter: every item has 10% chance of being affected --Amy */
void
...
register int bodypart;
{
	...
	if (Role_if(PM_CYBERNINJA) && rn2(5) && !(DoorningHappens) ) {
	...
	if ((evilfriday || DoorningHappens) && !rn2(10)) {
		pline("SCREEEEEECH!");
		aggravate();
	}
...
boulderdone:
	...
	if (!rn2(10) || evilfriday || DoorningHappens) {
