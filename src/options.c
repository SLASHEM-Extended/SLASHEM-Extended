...
/* WAC made the [ xx pts]  dmg display optional */
	...
	{"hybriddroughter", &flags.hybriddroughter, FALSE, SET_IN_FILE}, 
	{"hybridvanillaoid", &flags.hybridvanillaoid, FALSE, SET_IN_FILE}, 
...
static char def_inv_order[MAXOCLASSES] = {
	...
	TOOL_CLASS, GEM_CLASS, ROCK_CLASS, BALL_CLASS, CHAIN_CLASS, VENOM_CLASS, 0, 0,
};
...
boolean tinitial, tfrom_file;
{
	...
	if (match_optname(opts, "hybridhardmoder", 15, FALSE)) {
		flags.hybridization++;
	}
	if (match_optname(opts, "hybridstunfish", 14, FALSE)) {
		flags.hybridization++;
	}
	...
	if (match_optname(opts, "hybriddroughter", 15, FALSE)) {
		flags.hybridization++;
	}
	if (match_optname(opts, "hybridvanillaoid", 16, FALSE)) {
		flags.hybridization++;
	}
