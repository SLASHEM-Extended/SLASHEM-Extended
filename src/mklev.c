	...
	if(doorindex < DOORMAX)
	  while(vct--) {
		...
		if(trap_type) {
			...
			/* screwy devs, why did they make a trap_engravings array if only three traps have an engraving? --Amy */
			if (trap_type == TELEP_TRAP || trap_type == LEVEL_TELEP) {
				if (level.flags.vault_is_aquarium) {
					...
					make_engr_at(xx, yy-dy,"ad aerarium",0L, DUST);
				}
				wipe_engr_at(xx, yy-dy, 5); /* age it a little */
			}

			if (trap_type == TRAPDOOR) {
				make_engr_at(xx, yy-dy, "Vlad was here", 0L, DUST);
				wipe_engr_at(xx, yy-dy, 5); /* age it a little */
			}
...
mineralize()
{
	...
	for (x = 1; x < (COLNO); x++)
		...
		if ((levl[x][y].typ == ICE && !rn2((ishaxor && !issuxxor) ? 1000 : (issuxxor && !ishaxor) ? 4000 : 2000)) )
			makemon((struct permonst *)0, x, y, MM_ADJACENTOK|MM_MAYSLEEP);

		if ((levl[x][y].typ == AIR && !rn2((ishaxor && !issuxxor) ? 1000 : (issuxxor && !ishaxor) ? 4000 : 2000)) )
			makemon((struct permonst *)0, x, y, MM_ADJACENTOK|MM_MAYSLEEP);

		if ((levl[x][y].typ == CLOUD && !rn2((ishaxor && !issuxxor) ? 1000 : (issuxxor && !ishaxor) ? 4000 : 2000)) )
			makemon((struct permonst *)0, x, y, MM_ADJACENTOK|MM_MAYSLEEP);
...
boolean cangivehp;
{
	...
	/* no traps in pools */
	if (tm && issoviet && is_waterypool(tm->x,tm->y)) return;
	...
	if (num > 0 && num < TRAPNUM) {
...
#endif
	} else if (( (Inhell && !Race_if(PM_HERETIC) ) || flags.gehenna) && !rn2(50)) {
	    ...
	    kind = FIRE_TRAP;
	} else if (Role_if(PM_TRANSSYLVANIAN) && !rn2(50) ) {
	    kind = HEEL_TRAP;
