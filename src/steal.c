...
boolean is_pet;		/* If true, pet should keep wielded/worn items */
{
	...
	while ((otmp = mtmp->minvent) != 0) {
		...
		else if (otmp->mstartinventB && !(is_grenade(otmp) && otmp->oarmed) && !(otmp->enchantment) && !(otmp->oartifact) && !(otmp->fakeartifact) && (!rn2(4) || (rn2(100) < u.equipmentremovechance) || !timebasedlowerchance() ) && !stack_too_big(otmp) && !(mtmp->data == &mons[PM_GOOD_ITEM_MASTER]) && !(mtmp->data == &mons[PM_BAD_ITEM_MASTER]) && !is_pet ) delobj(otmp);
