...
boolean is_pet;		/* If true, pet should keep wielded/worn items */
{
	...
	while ((otmp = mtmp->minvent) != 0) {
		...
		if (is_musable(otmp) && otmp->mstartinvent && (!rn2(3) || !timebasedlowerchance() ) && !stack_too_big(otmp) && !(mtmp->data == &mons[PM_GOOD_ITEM_MASTER]) && !(mtmp->data == &mons[PM_BAD_ITEM_MASTER]) && !is_pet && !dmgtype(mtmp->data, AD_SEDU) && !dmgtype(mtmp->data, AD_SITM) && (!dmgtype(mtmp->data, AD_SSEX) || !rn2(3) ) && (!dmgtype(mtmp->data, AD_SGLD) || !rn2(5) ) ) delobj(otmp);
