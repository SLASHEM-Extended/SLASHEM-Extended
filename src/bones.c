...
d_level *lev;
{
	...
	return (boolean)(((sptr = Is_special(lev)) != 0 && !sptr->boneid)
		...
		|| (flags.lostsoul && !(u.lostsoulescape) && In_dod(lev) && lev->dlevel == (depth(&medusa_level) - 1) )
		|| (flags.lostsoul && !(u.lostsoulescape) && In_dod(lev) && lev->dlevel == (depth(&medusa_level)) )
		|| (flags.lostsoul && !(u.lostsoulescape) && In_dod(lev) && lev->dlevel == (depth(&medusa_level) + 1) )
		|| (flags.uberlostsoul && !(u.lostsoulescape) && In_gehennom(lev) && (lev->dlevel == 23 || lev->dlevel == 24 || lev->dlevel == 25))
