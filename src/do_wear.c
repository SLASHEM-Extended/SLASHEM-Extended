...
glibr()
{
	...
	if (u.twoweap && otmp && (otmp->otyp != BRASS_KNUCKLES) && !(otmp->oartifact == ART_GREEN_ASSISTANT) && (otmp->otyp != SUPER_KNUCKLES) && (otmp->otyp != ULTRA_KNUCKLES) && (otmp->otyp != ELITE_KNUCKLES) ) {
		...
		xfl++;
		if (( (otmp->otyp != LOADSTONE && otmp->otyp != HEALTHSTONE && otmp->otyp != LUCKSTONE && otmp->otyp != MANASTONE && otmp->otyp != SLEEPSTONE && otmp->otyp != LOADBOULDER && (otmp->otyp != AMULET_OF_YENDOR || u.freeplaymode) && (otmp->otyp != FAKE_AMULET_OF_YENDOR || u.freeplaymode) && otmp->otyp != STARLIGHTSTONE && otmp->otyp != STONE_OF_MAGIC_RESISTANCE && !is_nastygraystone(otmp) && !is_feminismstone(otmp) ) || (!otmp->cursed && (otmp->otyp != AMULET_OF_YENDOR || u.freeplaymode) && (otmp->otyp != FAKE_AMULET_OF_YENDOR || u.freeplaymode) ) ) && !(otmp->otyp == LUCKSTONE && isevilvariant && !otmp->cursed && !otmp->blessed && Luck < 0))
			dropx(otmp);
	}
	otmp = uwep;
