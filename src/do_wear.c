...
boolean noisy;
{
	...
	if (noisy) { pline_The("%s will not fit on your body.", which);
		...
		if (yn("Try to put it on anyway?") == 'y') {
			...
			nomul(-2, "putting on armor that didn't fit");
			youmonst.movement = 0; /* ugly hack */
		    return 0;}
			}
...
/*WAC cap off the Monk's ac bonus to -11 */
	...
	if (PlayerInHighHeels && !(AllSkillsUnskilled || u.uprops[SKILL_DEACTIVATED].extrinsic || (uarmc && uarmc->oartifact == ART_PALEOLITHIC_ELBOW_CONTRACT) || have_unskilledstone())) { /* extra AC --Amy */
