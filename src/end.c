...
stellarisdone:
	...
	if ((uwep && uwep->oartifact == ART_BLACK_MARK) && how <= GENOCIDED) {
		pline("But wait...");
		Your("weapon %s!", !Blind ? "begins to glow" : "feels warm");
		if (how == CHOKING) You("vomit ...");
		You_feel("much better!");
...
blackmarkdone:
	...
	if ((uarmu && uarmu->oartifact == ART_TILLMANN_S_TARGET) && how <= GENOCIDED) {
		pline("But wait...");
		Your("shirt %s!", !Blind ? "begins to glow" : "feels warm");
		if (how == CHOKING) You("vomit ...");
		You_feel("much better!");
		pline_The("shirt crumbles to dust!");
		useup(uarmu);
...
postmandone:
	...
	if ((uarmc && uarmc->oartifact == ART_BLOBLO) && how <= GENOCIDED) {
		pline("But wait...");
		Your("cloak %s!", !Blind ? "begins to glow" : "feels warm");
		if (how == CHOKING) You("vomit ...");
		You_feel("much better!");
		pline_The("cloak crumbles to dust!");
		useup(uarmc);
...
versusinstadone:
	...
	if ((uarmc && uarmc->oartifact == ART_DISQUALIFICATION_WARD) && (u.uhp > 0) && (u.uhpmax > 0) && (u.superdebt < 50000) && how < GENOCIDED) {
		pline("But wait...");
		Your("cloak %s!", !Blind ? "begins to glow" : "feels warm");
		if (how == CHOKING) You("vomit ...");
		You_feel("much better!");
...
wendydone:
	...
	if (carryingarti(ART_EMERGENCY_BOOT_DISK) && how < GENOCIDED) {
		pline("But wait! You have perilous life saving!");
		...
		if (yn_function("Come back to life?", ynchars, 'y') == 'y' ) {
			...
			if (wanttodie) {
				pline("Nyehehe-hehe-he, you would have lifesaved but you said you want your possessions identified! GAME OVER!");
				goto emergencybootdone;
			}

			if (u.ulevel > 2) {
			    losexp("emergency rebooting", TRUE, FALSE);
			    losexp("emergency rebooting", TRUE, FALSE);
			    pline("You hit the 'exit' button to escape from the grave! There you are again, back from the dead...");
			    if(u.uhpmax <= 0) u.uhpmax = 1;	/* arbitrary */
			    savelife(how);
...
rodneydone:
	...
	bones_ok = (iflags.bones || (BonesLevelChange || u.uprops[BONES_CHANGE].extrinsic || have_bonestone() || (uarmu && uarmu->oartifact == ART_ELVIS_S_BONES_FILE) ) ) && (how < GENOCIDED) && can_make_bones(); /* dthexpl patch */
