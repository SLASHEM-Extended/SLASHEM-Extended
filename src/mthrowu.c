...
boolean verbose;  /* give message(s) even when you can't see what happened */
{
	...
	} else if (mtmp->data == &mons[PM_DNETHACK_ELDER_PRIEST_TM_] || mtmp->data == &mons[PM_ATHLEANNIE] || mtmp->data == &mons[PM_FLUIDATOR_ANDER] || mtmp->data == &mons[PM_FART_ATHLETE] || mtmp->data == &mons[PM_TOWELBEAR] || mtmp->data == &mons[PM_LIGHTSABER_ART_JEDI] || mtmp->data == &mons[PM_MR__CONCLUSIO] || mtmp->data == &mons[PM_YOUR_GAME_ENDS_NOW]|| mtmp->data == &mons[PM_ELITE_GENDAME] || mtmp->data == &mons[PM_LILAC_FEMMY] || mtmp->data == &mons[PM_GREEN]) { /* will never be hit by monsters' ranged attacks */
	    if (!ismimic) {
