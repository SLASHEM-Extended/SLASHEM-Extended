...
"segfaults",
"queefing gas",
"menstruational liquid",
"some icky bodily fluid",
"pus",
"necrosis",
...
const char *name;	/* if null, then format `obj' */
{
	...
	is_polearm = (obj && (objects[obj->otyp].oc_skill == P_POLEARMS || objects[obj->otyp].oc_skill == P_LANCE || objects[obj->otyp].oc_skill == P_GRINDER || obj->otyp == AKLYS || obj->otyp == BLOW_AKLYS || obj->otyp == REACH_TRIDENT || obj->otyp == SPINED_BALL || obj->otyp == CHAIN_AND_SICKLE || obj->otyp == LAJATANG || obj->otyp == LASER_CHAIN));
...
shieldblockboo:
		...
		use_skill(P_SHIEN, rnd(8)); /* would take forever to train otherwise --Amy */
	...
	} else if (uarm && uarm->oartifact == ART_BACKOBLOK && !rn2(2)) {
	...
	register struct obj *obj;
{
	...
	while(range-- > 0) { /* Actually the loop is always exited by break */
		...
		if ((mtmp = m_at(bhitpos.x, bhitpos.y)) != 0) {
		    ...
		    if (hitu && singleobj->oartifact == ART_BURGLED_NIGHT_SCYTHE && !(Race_if(PM_PLAYER_NIBELUNG) && rn2(5))) {
				pline("Collusion!");
				litroomlite(FALSE);
		    }
