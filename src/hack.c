...
boolean guess;
{
	...
	while (n != 0) {
		...
		int dirmax = (BishopGridbug || u.uprops[BISHOP_GRIDBUG].extrinsic || have_bishopstone() || (uarmg && uarmg->oartifact == ART_LINE_CAN_PLAY_BY_YOURSELF) || (uwep && uwep->oartifact == ART_KILLER_PIANO) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_KILLER_PIANO) || u.umonnum == PM_GRID_BUG || u.umonnum == PM_WEREGRIDBUG || u.umonnum == PM_GRID_XORN || u.umonnum == PM_STONE_BUG || u.umonnum == PM_DELETERIOUS_BUG || u.umonnum == PM_NATURAL_BUG || u.umonnum == PM_WEAPON_BUG || u.umonnum == PM_MELEE_BUG || u.umonnum == PM_VORPAL_GRID_BUG || u.umonnum == PM_SPECIFIC_BUG || u.umonnum == PM_CONTAMINATED_BUG || (Race_if(PM_WEAPON_BUG) && !Upolyd))? 4 : 8;
