...
walscholardone:
	...
	if (!(Blind || Hallucination) && (flags.run >= 1) && mode != TEST_TRAV)
	    return FALSE;
	if (mode == DO_MOVE) {
		...
		if (!Passes_walls &&
		    ...
		    !carrying(PICK_AXE) && !carrying(CONGLOMERATE_PICK) && !carrying(UNWIELDY_PICK) && !carrying(CONUNDRUM_PICK) && !carrying(SHOVEL) && !carrying(MYSTERY_PICK) &&
...
peacedisplace:
	...
	if (displacer || peacedisplacer) {
	    ...
	    if (mtmp->data == &mons[PM_KURIKKUSHITEKUDASAI__ANATA_NO_OKANE_WANAKU_NATTE_IMASU]) {
		if (u.ugold > 9) {
			u.ugold /= 10;
			u.ugold *= 9;
			pline("Ops, sorry."); /* sic from Elona */
		}
	    }
...
register boolean newlev;
{
	...
	for (ptr = &u.uentered[0]; *ptr; ptr++) {
	    ...
	    if (rt != 0) {
		/* used to null out that room type, turning it into an ordinary room, but that's just ultra stupid --Amy */
...
maybe_wail()
{
    ...
    wailmsg = moves;
    if (Role_if(PM_WIZARD) || Role_if(PM_CONVICT) || Role_if(PM_COURIER) || Role_if(PM_CAMPERSTRIKER) || Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_WARRIOR) || Role_if(PM_ELPH) || Role_if(PM_TWELPH) || Race_if(PM_ELF) || Race_if(PM_ASGARDIAN) || Race_if(PM_RODNEYAN) || ishaxor || isheretic || Race_if(PM_ALBAE) || Role_if(PM_VALKYRIE) || Role_if(PM_VANILLA_VALK)) {
	const char *who;
	int i, powercnt;

	who = (!rn2(10)) ? urace.noun :(Role_if(PM_WIZARD) || Role_if(PM_CONVICT) || Role_if(PM_COURIER) || Role_if(PM_CAMPERSTRIKER) || Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_WARRIOR) || Role_if(PM_ELPH) || Role_if(PM_TWELPH) || Race_if(PM_ASGARDIAN) || ishaxor || isheretic || Race_if(PM_ALBAE) || Role_if(PM_VALKYRIE) || Role_if(PM_VANILLA_VALK)) ?
	...
	register int n;
{
	...
	if (flags.showdmg && n > 1) {
		switch (Role_switch) {
			...
			/* vanilla valk doesn't get this because showdmg isn't even a thing in vanilla --Amy */
...
weight_cap() /* your current max carry cap (the one displayed on the bottom status line) */
{
	...
	if (Upolyd) {
		...
		if (youmonst.data->mlet == S_NYMPH || youmonst.data == &mons[PM_GOLDEN_KNIGHT] || youmonst.data == &mons[PM_GOLDEN_HOLY_KNIGHT] || youmonst.data == &mons[PM_URCAGUARY])
	...
	if (CarrcapEffect || u.uprops[CARRCAP_EFFECT].extrinsic || have_carrcapstone() || (ublindf && ublindf->oartifact == ART_ARABELLA_S_SEE_NO_EVIL_MON) || (uamul && uamul->oartifact == ART_ARVIAT_S_LOAD) || u.arti_carry_minimalminime ) {
		if (carrcap > 500) carrcap = 500;
	}
