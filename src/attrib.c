...
recalc_health()
{
	...
	if (carryingarti(ART_MAYWON_MONTH) && (getmonth() == 4)) u.uhealbonus += 10;
...
int oldlevel, newlevel;
{
	...
	while (abil || rabil) {
		...
		if(oldlevel < abil->ulevel && newlevel >= abil->ulevel) {
			...
			if (Race_if(PM_YUGGER) || carryingarti(ART_HAVE_IT_ALL_BUT_NOT_GET) ) { /* doesn't get positive ones --Amy */
...
int x;
{
	...
	if (x == A_STR) {
		...
		if (carryingarti(ART_HUGKICK) && uarmf && itemhasappearance(uarmf, APP_HUGGING_BOOTS)) tmp += 10;
		...
		if (carryingarti(ART_SEXY_DYKE_BITCH)) tmp += 1;
		...
		if (carryingarti(ART_SPORTAL_FIGHTER)) tmp += 10;
		if (uarmh && itemhasappearance(uarmh, APP_THINKING_HELMET)) tmp += 1;
		if (carryingarti(ART_REPREHENDATION_OUTTA_NOWHE)) tmp += 2;
		if (ublindf && ublindf->oartifact == ART_THEY_COME_FROM_HOLLAND) tmp += 1;
		if (uarmc && uarmc->oartifact == ART_ANCHORAGE_BATTLECOAT) tmp += 5;
		if (uleft && uleft->oartifact == ART_HOH_UNNE) tmp += 5;
		if (uwep && uwep->oartifact == ART_WEE_HOUR_EFFECT && morningtime()) tmp += 20;
		if (uright && uright->oartifact == ART_HOH_UNNE) tmp += 5;
		if (carryingarti(ART_ORDERLY_PROPER)) tmp += 5;
		...
		if (uwep && uwep->oartifact == ART_ALL_IN_ALL_IS_THE_BEST_BAC) tmp += 2;
		...
		if (uwep && uwep->oartifact == ART_SUCKSTRIKE) tmp -= 3;
		if (u.twoweap && uswapwep && uswapwep->oartifact == ART_SUCKSTRIKE) tmp -= 3;
		if (u.uhs >= WEAK) tmp--;
		...
		if (uarmf && uarmf->oartifact == ART_SEE_BY_LAKE) tmp += 1;
		if (carryingarti(ART_IRREGULAR_RADIANCE)) tmp += 10;
		...
		if (uleft && uleft->oartifact == ART_CROQUE_FORD) tmp += 4;
		if (uright && uright->oartifact == ART_CROQUE_FORD) tmp += 4;
		if (ublindf && ublindf->oartifact == ART_TILL_S_EXCEPTION) tmp += 2;
		if (uarmf && uarmf->oartifact == ART_JANET_S_DETACHMENT) tmp += 5;
		if (powerfulimplants() && uimplant && uimplant->oartifact == ART_SANTANA_S_CHICANERY) tmp += 1;
		if (carryingarti(ART_REPREHENDATION_OUTTA_NOWHE)) tmp += 2;
		if (uarmf && uarmf->oartifact == ART_LOVELANE) tmp += 5;
		if (carryingarti(ART_SEXY_DYKE_BITCH)) tmp += 5;
		...
		if (uleft && uleft->oartifact == ART_CROQUE_FORD) tmp += 4;
		if (uright && uright->oartifact == ART_CROQUE_FORD) tmp += 4;
		if (carryingarti(ART_HANG_WITH_THE_MIND)) tmp += 2;
		if (carryingarti(ART_REPREHENDATION_OUTTA_NOWHE)) tmp += 2;
		...
		if (x == A_INT && carryingarti(ART_ARE_YOU_SO_UNABLE_)) tmp += 2;
		if (x == A_INT && carryingarti(ART_HAHAHA_NOW_YOU_NEED_TO_BE_)) tmp += 2;
		if (x == A_INT && ublindf && ublindf->oartifact == ART_BERIT_S_SAGE) tmp += 10;
		if (x == A_WIS && ublindf && ublindf->oartifact == ART_BERIT_S_SAGE) tmp += 15;
		if (x == A_WIS && carryingarti(ART_FINISHED_ANYWAY)) tmp += 10;
		if (uarmf && uarmf->oartifact == ART_SEE_BY_LAKE) tmp += 1;
		if (x == A_INT && carryingarti(ART_CHAUNDRA_POKA)) tmp += 4;
		if (x == A_WIS && carryingarti(ART_CHAUNDRA_POKA)) tmp += 2;
		...
		if (x == A_WIS && uwep && uwep->oartifact == ART_ALL_IN_ALL_IS_THE_BEST_BAC) tmp += 2;
		if (uarmf && uarmf->oartifact == ART_LORENZI_S_CLEANING_RESIDUE) tmp += 2;
		...
		if (x == A_INT && uwep && uwep->oartifact == ART_ALL_IN_ALL_IS_THE_BEST_BAC) tmp -= 2;
		if (x == A_INT && u.twoweap && uswapwep && uswapwep->oartifact == ART_ALL_IN_ALL_IS_THE_BEST_BAC) tmp -= 2;
		...
		if (uwep && uwep->oartifact == ART_PLAY_FIGUT_EIGHT) tmp += 8;
		if (uwep && uwep->oartifact == ART_WEE_HOUR_EFFECT && morningtime()) tmp += 5;
		...
		if (uarmu && uarmu->oartifact == ART_SETH_S_ERROR) tmp += 3;
		if (uarm && uarm->oartifact == ART_GARYX) tmp += 1;
		if (carryingarti(ART_ORDERLY_PROPER)) tmp += 8;
		...
		if (carryingarti(ART_SEXY_DYKE_BITCH)) tmp += 1;
		if (carryingarti(ART_FINISHED_ANYWAY)) tmp += 10;
