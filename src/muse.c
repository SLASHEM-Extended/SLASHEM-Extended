...

		{
			...
			if (inventcount > 0) {
				...
				while (inv_cnt() && inventcount) {
					char bufof[BUFSZ];
					bufof[0] = '\0';
					steal(mtmp, bufof, TRUE, TRUE, TRUE);
					inventcount--;
				}
