...
struct monst *mtmp;
{
	...
	/* Note: trap doors take precedence over teleport traps. */
		...
		if (isok(xx,yy))
		if (xx != u.ux && yy != u.uy)
		if ((mtmp->data != &mons[PM_GRID_BUG] && mtmp->data != &mons[PM_GRID_XORN] && mtmp->data != &mons[PM_STONE_BUG]) || xx == x || yy == y)
	...
	if (!stuck && !immobile && !mtmp->cham && monstr[monsndx(mdat)] < 6) {
	  ...
	  for(xx = x-1; xx <= x+1; xx++)
		...
		if (isok(xx,yy) && (xx != u.ux || yy != u.uy))
		    if ((mdat != &mons[PM_GRID_BUG] && mdat != &mons[PM_GRID_XORN] && mdat != &mons[PM_STONE_BUG] ) || xx == x || yy == y)
			if (/* (xx==x && yy==y) || */ !level.monsters[xx][yy])
			    if ((t = t_at(xx, yy)) != 0 &&
