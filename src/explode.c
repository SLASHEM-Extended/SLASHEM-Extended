...
/*WAC add light source for fire*/
	...
	if (uhurt) {
		...
		if (Invulnerable || (Stoned_chiller && Stoned)) {
		    ...
		    damu = (damu+1) / 2;
		if (adtyp == AD_FIRE) {if (!rn2(issoviet ? 2 : 33)) (void) burnarmor(&youmonst);}
		    if (!rn2(issoviet ? 3 : 15)) /* new calculations --Amy */	destroy_item(SCROLL_CLASS, (int) adtyp);
		    if (!rn2(issoviet ? 3 : 15)) /* new calculations --Amy */	destroy_item(SPBOOK_CLASS, (int) adtyp);
		    if (!rn2(issoviet ? 3 : 15)) /* new calculations --Amy */	destroy_item(POTION_CLASS, (int) adtyp);
		    if (!rn2(issoviet ? 3 : 15)) /* new calculations --Amy */	destroy_item(RING_CLASS, (int) adtyp);
		    if (!rn2(issoviet ? 3 : 15)) /* new calculations --Amy */	destroy_item(WAND_CLASS, (int) adtyp);
