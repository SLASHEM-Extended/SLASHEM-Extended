...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */           
	case ARMOR_CLASS:
		...
		( (otmp->otyp == WEDGE_SANDALS || otmp->otyp == REPEATABLE_BOOTS || otmp->otyp == DANCING_SHOES || otmp->otyp == ANGER_HELM || otmp->otyp == SCRIPTED_HELMET || otmp->otyp == GAUNTLETS_OF_RAINBOW_MOUD || otmp->otyp == POKELIE_GLOVES || otmp->otyp == ASS_KICKER_BOOTS || otmp->otyp == TOP_HELMET || otmp->otyp == ABCDE_HELMET || otmp->otyp == COLOR_CHANGING_CONE || otmp->otyp == SWEET_MOCASSINS || otmp->otyp == SOFT_SNEAKERS || otmp->otyp == CLOGS ) && !rn2(10) ) ||
