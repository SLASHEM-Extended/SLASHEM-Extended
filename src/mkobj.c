...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */                   
	case WEAPON_CLASS:
		...
		if (otmp->otyp == PISTOL_BULLET) otmp->quan += rnd(50);
		...
		if (otmp->otyp == LEAD_RIFLE_BULLET) otmp->quan += rnd(50);
		if (otmp->otyp == ANTIMATTER_RIFLE_BULLET) otmp->quan += rnd(50);
		if (otmp->otyp == SNIPER_BULLET) otmp->quan += rnd(50);
		if (otmp->otyp == LEAD_SNIPER_BULLET) otmp->quan += rnd(50);
		...
		if (otmp->otyp == AUTO_SHOTGUN_SHELL) otmp->quan += rnd(30);
...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */           
	case ARMOR_CLASS:
		...
		if (otmp->otyp == WOWEE_GAUNTLETS) otmp->spe += rnd(10);

		if (itemhasappearance(otmp, APP_REPAIR_WORTHY_BOOTS)) {
			if (!rn2(3)) otmp->oeroded = rnd(3);
		...
		if ((artif != 3) && itemhasappearance(otmp, APP_INSTAFEMINISM_SHOES)) {
...
/*                     otmp->spe = rn2(4) - rn2(3); */
	...
	if (otmp && otmp->oclass != COIN_CLASS && trophylevel()) otmp->rknown = TRUE;
...
#undef CEILDIV
	...
	if (obj->otyp == CORPSE && obj->corpsenm >= LOW_PM) {
		...
		return (int) 0; /* gold shouldn't weigh anything! --Amy */
	else if (obj->otyp == HEAVY_IRON_BALL && obj->owt != 0)
		return((int)(obj->owt));	/* kludge for "very" heavy iron ball */
	return(wt ? wt*(int)obj->quan : (int) 0); /* weightless items should be *really* weightless --Amy */
}

static int treefruits[] = {APPLE,ORANGE,PEAR,BANANA,EUCALYPTUS_LEAF,LEMON,CHERRY};
static int treefruitsX[] = {APPLE,ORANGE,PEAR,BANANA,EUCALYPTUS_LEAF,CHERRY,ASIAN_PEAR,BLACK_HENBANE,MOTHERWORT,SWAMP_TENTACLE,VERMILION_SPONGE,LICHOR,AMBROSIA,WATER_HYACINTH,SPIDDAL_STICK,HARRADA,MEADOWFOAM,LEMON,MELON,SLIME_MOLD,LUMP_OF_ROYAL_JELLY,HONEYCOMB,WATERMELON,WHITE_PEACH,MAGIC_BANANA};
...
register struct obj *otmp;
{
	...
	if (objects[otyp].oc_material == MT_LACQUER && evilfriday) return TRUE;
