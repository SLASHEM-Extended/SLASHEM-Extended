...
stupidsegfault:
	...
	if (mtmp->ispriest || mtmp->isminion) {
	    ...
	    if (StarlitBug || u.uprops[STARLIT_BUG].extrinsic || have_starlitskystone() || autismweaponcheck(ART_STARRING_INFERNO) || (uarmf && uarmf->oartifact == ART_STAR_SOLES) || (uimplant && uimplant->oartifact == ART_ARABELLA_S_SEXY_CHARM) || (uarmg && uarmg->oartifact == ART_RAAAAAAAARRRRRRGH) || MonsterGlyphHallu) {
		strcat(buf, "monster");
		return buf;
	    }
	...
	if (mtmp->isshk && !do_hallu) {
	    ...
	    if (StarlitBug || u.uprops[STARLIT_BUG].extrinsic || have_starlitskystone() || autismweaponcheck(ART_STARRING_INFERNO) || (uarmf && uarmf->oartifact == ART_STAR_SOLES) || (uimplant && uimplant->oartifact == ART_ARABELLA_S_SEXY_CHARM) || (uarmg && uarmg->oartifact == ART_RAAAAAAAARRRRRRGH) || MonsterGlyphHallu) strcat(buf, "monster");
...
"Bus driver",
	...
	"Dark Lighter",
	"Hunkbasher",
	"Lightsaber Form Trainer",
	"Style Changer",
	"Whirler",
	"Jedi Without Jedi Powers",
	"Chargeless Power",
	"Fizzle-Fizzer",
	"Blacklight Master",
	"Riding Apprentice",
