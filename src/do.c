...
dodrop()
{
	...
	if (NoDropProblem || u.uprops[DROP_BUG].extrinsic || have_dropbugstone() || (uball && uball->oartifact == ART_DOWNLIFT) || (uamul && uamul->oartifact == ART_ARABELLA_S_SWOONING_BEAUTY) ) {
...
const char *verb;
{
	...
	if (obj->otyp == BOULDER && boulder_hits_pool(obj, x, y, FALSE))
		...
		if (((mtmp = m_at(x, y)) && mtmp->mtrapped) ||
			...
			if (!Passes_walls && !throws_rocks(youmonst.data) && !(uarmg && uarmg->oartifact == ART_MOUNTAIN_FISTS) ) {
...
register struct obj *obj;
{
	...
	case AMULET_OF_SPELLCASTING:
		pline_The("toilet seems to be casting a spell!");
		break;
...
rerollchaloc:
	...
	if (SatanEffect || u.uprops[SATAN_EFFECT].extrinsic || have_satanstone() || (uarm && uarm->oartifact == ART_SATAN_S_REVERENCE) || (uarmh && uarmh->oartifact == ART_DARK_OMBRIA) || (uarm && uarm->oartifact == ART_NATAS_IS_BACK) ) {
		if (SatanEffectXtra) {
