...
# endif
	...
	if (HAS_ROGUE_IBM_GRAPHICS && iflags.use_color) {
		...
		if ((Hyperbluewalls || u.uprops[HYPERBLUEWALL_BUG].extrinsic || have_hyperbluestone() || (uarms && uarms->oartifact == ART_DOLORES__VIRGINITY) || (uarms && uarms->oartifact == ART_BLUE_SHIRT_OF_DEATH) ) && (offset >= S_vwall && offset <= S_hcdoor) && !(offset > S_rockwall && offset <= S_tunnelwall)) color = CLR_BRIGHT_BLUE;
		if ((Hyperbluewalls || u.uprops[HYPERBLUEWALL_BUG].extrinsic || have_hyperbluestone() || (uarms && uarms->oartifact == ART_DOLORES__VIRGINITY) || (uarms && uarms->oartifact == ART_BLUE_SHIRT_OF_DEATH) ) && (offset == S_dungwall)) color = CLR_BRIGHT_BLUE;
