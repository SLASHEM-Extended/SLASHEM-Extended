...
register struct monst *mtmp;
{
	...
	if (!rn2(100) && timebasedlowerchance() ) (void) mongets(mtmp, SCR_STANDARD_ID);
	...
	switch (ptr->mlet) {
	    ...
	    case S_TRAPPER:
			...
			if(ptr == &mons[PM_GRAY_SNIPER]) {
				...
				otmp = mksobj(rnd_class(RIGHT_MOUSE_BUTTON_STONE,NASTY_STONE), TRUE, FALSE);
				otmp->quan = 1;
				otmp->owt = weight(otmp);
				(void) mpickobj(mtmp, otmp, TRUE);
...
register int	mmflags;
{
	...
	*mtmp = zeromonst;		/* clear all entries in structure */
	...
	} else if (ptr->msound == MS_NEMESIS && ptr->mlet != S_NEMESE && mndx != PM_TRUE_MISSINGNO && mndx != PM_ETHEREAL_MISSINGNO && mndx != PM_STARLIT_SKY && mndx != PM_DARK_STARLIT_SKY && mndx != PM_BLACK_STARLIT_SKY && mndx != PM_RED_STARLIT_SKY && mndx != PM_BROWN_STARLIT_SKY && mndx != PM_GREEN_STARLIT_SKY && mndx != PM_PURPLE_STARLIT_SKY && mndx != PM_YELLOW_STARLIT_SKY && mndx != PM_ORANGE_STARLIT_SKY && mndx != PM_CYAN_STARLIT_SKY && mndx != PM_VIOLET_STARLIT_SKY) {
	...
	if (mndx == PM_SCROLLER_MASTER || mndx == PM_BOULDER_MASTER || mndx == PM_ITEM_MASTER || mndx == PM_GOOD_ITEM_MASTER || mndx == PM_HOLE_MASTER || mndx == PM_TRAP_MASTER) monkilled(mtmp, "", AD_PHYS); /* leave no trace of this monster --Amy */
