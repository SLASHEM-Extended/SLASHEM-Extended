...
dokick()
{
	...
	if(MON_AT(x, y)) {
		...
		if (!DEADMONSTER(mtmp) &&
		    ...
		    mtmp->mx == x && mtmp->my == y &&
		    !memory_is_invisible(x, y) && !(mtmp->data->msound == MS_DEEPSTATE) &&
