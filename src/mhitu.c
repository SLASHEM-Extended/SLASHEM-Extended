...
elena8:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == DANCING_SHOES) || (footwear && footwear->otyp == ANASTASIA_DANCING_SHOES) || (footwear && footwear->otyp == FRIEDERIKE_DANCING_SHOES) || mtmp->data == &mons[PM_FEMALE_DANCING_SHOE] || (footwear && itemhasappearance(footwear, APP_LOAFERS)) || (randomsexyheels == 10) ) ) {
elena9:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elena12:
		...
		make_hallucinated(HHallucination + monsterlev,FALSE,0L);
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ( (FemtrapActiveKerstin && spawnswithsandals(mtmp->data)) || (footwear && itemhasappearance(footwear, APP_SUPERHARD_SANDALS)) || mtmp->data == &mons[PM_SEXY_CLAUDIA] || mtmp->data == &mons[PM_CLAUDIA_S_CUTE_WOODEN_SANDAL] || mtmp->data == &mons[PM_LOVELY_WOODEN_GETA] || (randomsexyheels == 30) ) ) {
elenaWOODSANDAL:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
...
elenacuddle:
			...
			if ((!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_HEAP_OF_SHIT_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_TREADED_HEELS) ) || (footwear && footwear->otyp == HENRIETTA_COMBAT_BOOTS) || mtmp->data == &mons[PM_HENRIETTEN_SHOE] || (randomsexyheels == 29) ) ) {
elenahos:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
