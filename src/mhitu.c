...
elenacuddle:
			...
			if ((!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_HEAP_OF_SHIT_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_TREADED_HEELS) ) || (footwear && footwear->otyp == HENRIETTA_COMBAT_BOOTS) || mtmp->data == &mons[PM_HENRIETTEN_SHOE] || mtmp->data == &mons[PM_HENRIETTA_SHOE] || (randomsexyheels == 29) ) ) {
elenahos:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
