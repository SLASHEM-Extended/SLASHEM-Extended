...
kseniaagain:
		 ...
		 * by having only 1 in (legscratching squared) chance for an increase --Amy */
			...
			if (mtmp->data == &mons[PM_ARABELLA_SHOE]) {
				if (!FemaleTrapArabella) {
					FemaleTrapArabella = rnz(3000);
					pline("Oh no, the master of nasty traps is laying out her snares to get you!");
				}
			}

			if (mtmp->data == &mons[PM_ANASTASIA_SHOE]) {
				if (!FemaleTrapAnastasia) {
					FemaleTrapAnastasia = rnz(3000);
					pline("Suddenly, you feel that you're going to step into a heap of shit.");
				}
			}

			if (mtmp->data == &mons[PM_HENRIETTA_SHOE]) {
				if (!FemaleTrapHenrietta) {
					FemaleTrapHenrietta = rnz(3000);
					pline("You feel that someone is going to open the zippers of your boots, making you fumble into a heap of dog shit.");
				}
			}

			if (mtmp->data == &mons[PM_KATRIN_SHOE]) {
				if (!FemaleTrapKatrin) {
					FemaleTrapKatrin = rnz(3000);
...
karinrepeat:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == WEDGE_SANDALS) || (footwear && footwear->otyp == EVELINE_WEDGE_SANDALS) || (footwear && footwear->otyp == IRINA_WEDGE_SANDALS) || (footwear && footwear->otyp == JASIEEN_WEDGE_SANDALS) || (footwear && footwear->otyp == CLOGS) || (footwear && footwear->otyp == KSENIA_PLATFORM_SANDALS) || (footwear && footwear->otyp == KLARA_PLATFORM_SANDALS) || (footwear && footwear->otyp == NATALIA_WEDGE_SANDALS) || (footwear && footwear->otyp == GUDRUN_WEDGE_SANDALS) || (footwear && footwear->otyp == ELLA_WEDGE_SANDALS) || (footwear && footwear->otyp == MAY_BRITT_SANDALS) || (footwear && footwear->otyp == KERSTIN_WOODEN_SANDALS) || (footwear && footwear->otyp == ANNEMARIE_PLATFORM_SANDALS) || (footwear && footwear->otyp == KARIN_LADY_SANDALS) || (footwear && footwear->otyp == CLAUDIA_WOODEN_SANDALS) || (footwear && itemhasappearance(footwear, APP_CALF_LEATHER_SANDALS)) || (footwear && itemhasappearance(footwear, APP_WOODEN_CLOGS)) || (footwear && itemhasappearance(footwear, APP_PRINTED_WEDGES)) || (footwear && itemhasappearance(footwear, APP_FLUFFY_SANDALS)) || (footwear && itemhasappearance(footwear, APP_GEOMETRY_HEELS)) || (footwear && itemhasappearance(footwear, APP_WEDGE_ESPADRILLES)) || mtmp->data == &mons[PM_ANIMATED_WEDGE_SANDAL] || mtmp->data == &mons[PM_LOVELY_WEDGE_SANDAL] || mtmp->data == &mons[PM_CUTE_GETA] || mtmp->data == &mons[PM_THICK_PLATFORM_SANDAL] || mtmp->data == &mons[PM_CUTE_GIRL_CLOG] || mtmp->data == &mons[PM_WOODEN_LADY_CLOG] || mtmp->data == &mons[PM_TORN_WEDGE_SANDAL] || (randomsexyheels == 1) || mtmp->data == &mons[PM_WEREWEDGESANDAL] || mtmp->data == &mons[PM_CUTE_WOODEN_CLOG] || mtmp->data == &mons[PM_EVELYN_S_SILVER_GLITTER_SANDAL] || mtmp->data == &mons[PM_MIRACLE_PRETTY_GIRL_SANDAL] || mtmp->data == &mons[PM_BATHING_SANDAL] || mtmp->data == &mons[PM_ADRIANA_S_EROTIC_SANDAL] || mtmp->data == &mons[PM_BUCKLE_GIRL_SANDAL] || mtmp->data == &mons[PM_ETHEREAL_BATHING_SANDAL] || mtmp->data == &mons[PM_CORK_ESPADRILLE] || mtmp->data == &mons[PM_LOVELY_BUCKLE_SANDAL] || mtmp->data == &mons[PM_HOLIDAY_PLATFORM_SANDAL] || mtmp->data == &mons[PM_HUMAN_WEREWEDGESANDAL]) ) {
elenaWDG:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == ATSUZOKO_BOOTS) || (footwear && footwear->otyp == BUM_BUM_BOOTS) || (footwear && footwear->otyp == KATHARINA_PLATFORM_BOOTS) || (footwear && footwear->otyp == LARISSA_PLATFORM_BOOTS) || (footwear && footwear->otyp == SABRINA_PLATFORM_BOOTS) || (footwear && footwear->otyp == MADELEINE_PLATEAU_BOOTS) || (footwear && footwear->otyp == ANTJE_COMBAT_BOOTS) || (footwear && footwear->otyp == SING_PLATFORM_BOOTS) || (footwear && itemhasappearance(footwear, APP_BUFFALO_BOOTS)) || (footwear && itemhasappearance(footwear, APP_WHITE_BUFFALO_BOOTS)) || (footwear && itemhasappearance(footwear, APP_CHELSEA_BOOTS)) || (footwear && itemhasappearance(footwear, APP_PLATEAU_BOOTS)) || mtmp->data == &mons[PM_ANIMATED_ATSUZOKO_BOOT] || mtmp->data == &mons[PM_PLATEAU_GIRL] || mtmp->data == &mons[PM_PROFILE_KATI] || mtmp->data == &mons[PM_BROWN_BOOT_BITCH] || mtmp->data == &mons[PM_VERY_THICK_GIRL] || mtmp->data == &mons[PM_LARVA_LINE] || mtmp->data == &mons[PM_KATI_S_PLATEAU_BOOT] || mtmp->data == &mons[PM_VAMPIRIC_FEMMY] || mtmp->data == &mons[PM_VERENA_S_PLATEAU_BOOT] || mtmp->data == &mons[PM_PLATFORMINA_S_THICK_WOMENS_SHOE] || mtmp->data == &mons[PM_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_WHITE_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_PINK_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_RED_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_BLUE_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_OCHRE_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_BLACK_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_CYAN_GIRL_PLATFORM_BOOT] || mtmp->data == &mons[PM_SUPER_STRONG_GIRL] || (randomsexyheels == 2) || mtmp->data == &mons[PM_BUFFALO_HC_GIRL] || mtmp->data == &mons[PM_SUE_LYN_S_PLATEAU_BOOT] || mtmp->data == &mons[PM_FLAT_COMBAT_BOOT]) ) {
elena1:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elena7:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == ASS_KICKER_BOOTS) || (footwear && footwear->otyp == YASAMAN_LEATHER_BOOTS) || (footwear && itemhasappearance(footwear, APP_LEATHER_FLEECE_BOOTS)) || (randomsexyheels == 9) || mtmp->data == &mons[PM_SUPER_STRONG_GIRL] || mtmp->data == &mons[PM_FOURFOLD_SHOE_ENEMY] || mtmp->data == &mons[PM_DANIELLE_S_COMBAT_BOOT]) ) {
elena8:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == DANCING_SHOES) || (footwear && footwear->otyp == ANASTASIA_DANCING_SHOES) || (footwear && footwear->otyp == FRIEDERIKE_DANCING_SHOES) || mtmp->data == &mons[PM_FEMALE_DANCING_SHOE] || mtmp->data == &mons[PM_ANASTASIA_SHOE] || (footwear && itemhasappearance(footwear, APP_LOAFERS)) || (randomsexyheels == 10) ) ) {
elena9:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == SWEET_MOCASSINS) || (footwear && footwear->otyp == BRIGHT_CYAN_BEAUTIES) || (footwear && footwear->otyp == JESSICA_LADY_SHOES) || mtmp->data == &mons[PM_MARJI] || mtmp->data == &mons[PM_HEROINE_MOCASSIN] || (footwear && footwear->otyp == SOLVEJG_MOCASSINS) || (footwear && itemhasappearance(footwear, APP_MARJI_SHOES)) || (randomsexyheels == 11) ) ) {
elena10:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elena11:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == HIPPIE_HEELS) || (footwear && footwear->otyp == ELISE_HIPPIE_HEELS) || (footwear && itemhasappearance(footwear, APP_LOLITA_BOOTS)) || (footwear && itemhasappearance(footwear, APP_HOMO_SHOES)) || (footwear && itemhasappearance(footwear, APP_PARTICULARLY_TERRIBLE_BOOTS)) || (footwear && itemhasappearance(footwear, APP_EROTIC_BOOTS)) || (footwear && itemhasappearance(footwear, APP_SUPER_COMFY_HEELS)) || (footwear && itemhasappearance(footwear, APP_SPUTA_BOOTS)) || mtmp->data == &mons[PM_ANIMATED_PROSTITUTE_SHOE] || (randomsexyheels == 13) || mtmp->data == &mons[PM_WEREPROSTITUTESHOE] || mtmp->data == &mons[PM_HUMAN_WEREPROSTITUTESHOE] || mtmp->data == &mons[PM_SUNA_S_FLEECY_BLOCK_HEELED_BOOT] || mtmp->data == &mons[PM_HIPPIE_HEEL_BOOT] || mtmp->data == &mons[PM_SPIDER_FAGUS] || mtmp->data == &mons[PM_BULLETATOR_WHORE] || mtmp->data == &mons[PM_COLUMNAR_HEEL_BOOT] || mtmp->data == &mons[PM_LUISA_S_SPUTA_FLOWING_BLOCK_HEEL_BOOT]) ) {
elena12:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elena13:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == FEMININE_PUMPS) || (footwear && footwear->otyp == ANITA_LADY_PUMPS) || (footwear && footwear->otyp == LYDIA_LADY_PUMPS) || (footwear && footwear->otyp == JANET_LADY_PUMPS) || (footwear && footwear->otyp == DIDIER_SACHS_SHOES) || (footwear && footwear->otyp == JANINA_LADY_PUMPS) || (footwear && footwear->otyp == RHEA_LADY_PUMPS) || (footwear && footwear->otyp == WENDY_LEATHER_PUMPS) || (footwear && footwear->otyp == JUDITH_LEATHER_PUMPS) || (footwear && footwear->otyp == NELLY_LADY_PUMPS) || (footwear && footwear->otyp == DESTRUCTIVE_HEELS) || (footwear && footwear->otyp == AUTOSCOOTER_HEELS) || (footwear && footwear->otyp == VIOLET_BEAUTY_HEELS) || (footwear && itemhasappearance(footwear, APP_VELVET_PUMPS)) || (footwear && itemhasappearance(footwear, APP_SLING_PUMPS)) || (footwear && itemhasappearance(footwear, APP_STONE_CARVED_HEELS)) || (footwear && itemhasappearance(footwear, APP_MISSYS)) || (footwear && itemhasappearance(footwear, APP_FEELGOOD_HEELS)) || (footwear && itemhasappearance(footwear, APP_PARAGRAPH_SHOES)) || (footwear && itemhasappearance(footwear, APP_BONE_HEELS)) || (footwear && itemhasappearance(footwear, APP_BRIDAL_SHOES)) || (footwear && itemhasappearance(footwear, APP_ORGASM_PUMPS)) || (footwear && itemhasappearance(footwear, APP_PRINCESS_PUMPS)) || (footwear && itemhasappearance(footwear, APP_ALLIGATOR_PUMPS)) || (footwear && itemhasappearance(footwear, APP_SEXY_HEELS)) || (footwear && itemhasappearance(footwear, APP_SHADOWY_HEELS)) || (footwear && itemhasappearance(footwear, APP_MARY_JANES)) || mtmp->data == &mons[PM_FOURFOLD_SHOE_ENEMY] || mtmp->data == &mons[PM_ANIMATED_SEXY_LEATHER_PUMP] || mtmp->data == &mons[PM_WERESEXYLEATHERPUMP] || mtmp->data == &mons[PM_DELFI_ROCKZ] || mtmp->data == &mons[PM_ALL_TIME_FAVZ] || mtmp->data == &mons[PM_HAPPY_CLOUD] || mtmp->data == &mons[PM_WOOD_FEELING_INDIGO] || mtmp->data == &mons[PM_KATRIN_SHOE] || mtmp->data == &mons[PM_WOOD_FEELING_PAPAYA] || mtmp->data == &mons[PM_WOOD_FEELING_MINT] || mtmp->data == &mons[PM_BLACK_MANDALA] || mtmp->data == &mons[PM_UNDERWATER_LOVE] || mtmp->data == &mons[PM_TIRE_ROCKZ] || mtmp->data == &mons[PM_PRETTY_FEMALE_PARAGRAPH_SHOE] || mtmp->data == &mons[PM_TIRE_ROCKZ_BABYPINK] || mtmp->data == &mons[PM_SWEET_CHERRY] || mtmp->data == &mons[PM_ROCKZ_ARMY] || mtmp->data == &mons[PM_DEATHHEAD] || mtmp->data == &mons[PM_MASK_OFF] || mtmp->data == &mons[PM_SPARKLING_GOLD] || mtmp->data == &mons[PM_PURPLE_SANDSTORM] || mtmp->data == &mons[PM_SQUARE_ZONE] || mtmp->data == &mons[PM_CUTE_JEANS] || mtmp->data == &mons[PM_LOVELY_CORNFLOWER] || mtmp->data == &mons[PM_NANCY_S_RED_LADY_PUMP] || mtmp->data == &mons[PM_GHETTO_ROCKZ] || mtmp->data == &mons[PM_INGRID_S_LADY_PUMP] || mtmp->data == &mons[PM_LAST_UNICORN] || mtmp->data == &mons[PM_JUNGLE_LOVE] || mtmp->data == &mons[PM_FRESH_PINK] || mtmp->data == &mons[PM_BLUE_BLOSSOM] || mtmp->data == &mons[PM_SIXX_PAXX] || mtmp->data == &mons[PM_NEXT_LEVEL] || mtmp->data == &mons[PM_PATCHWORK_MISSYS] || mtmp->data == &mons[PM_TOXIC_PINK] || mtmp->data == &mons[PM_MAGIC_BUTTERFLY_MISSYS] || mtmp->data == &mons[PM_MY_LOVE_YIN] || mtmp->data == &mons[PM_MY_LOVE_YANG] || mtmp->data == &mons[PM_SKETCH_IT] || mtmp->data == &mons[PM_POOR_PUMPS] || mtmp->data == &mons[PM_BUNNY_ROCKZ] || mtmp->data == &mons[PM_CAT_ROCKZ] || mtmp->data == &mons[PM_COLLEGE_ROCKZ] || mtmp->data == &mons[PM_WELCOME_ON_BOARD] || mtmp->data == &mons[PM_BACTERIA_MISSYS] || mtmp->data == &mons[PM_SILVER_LADY_PUMP] || mtmp->data == &mons[PM_ELEGANT_CONE_HEEL_PUMP] || mtmp->data == &mons[PM_BANDANA_ROCKZ] || mtmp->data == &mons[PM_SCAN_ME_MISSYS] || mtmp->data == &mons[PM_SMILE_Z] || mtmp->data == &mons[PM_SECRET_RAINBOW] || mtmp->data == &mons[PM_YELLY] || mtmp->data == &mons[PM_SPACE_SHUTTLE_MISSYS] || mtmp->data == &mons[PM_ART_ROCKZ] || mtmp->data == &mons[PM_COVER_UP] || mtmp->data == &mons[PM_POKERFACE_MISSYS] || mtmp->data == &mons[PM_KITTEN_HEEL_PUMPS] || mtmp->data == &mons[PM_ROSE_ROCKZ] || mtmp->data == &mons[PM_ORANGE_VELVET_PUMP] || mtmp->data == &mons[PM_YES_I_ROCKZ] || mtmp->data == &mons[PM_WHITE_KARMA] || mtmp->data == &mons[PM_BRITTA_S_SEXY_PUMP] || mtmp->data == &mons[PM_BLACK_KARMA] || mtmp->data == &mons[PM_STREET_ROCKZ] || mtmp->data == &mons[PM_HUMAN_WERESEXYLEATHERPUMP] || mtmp->data == &mons[PM_ARVOGENIC_TOPMODEL] || (randomsexyheels == 15) || mtmp->data == &mons[PM_ANIMATED_BEAUTIFUL_FUNNEL_HEELED_PUMP] || mtmp->data == &mons[PM_WEREBEAUTIFULFUNNELHEELEDPUMP] || mtmp->data == &mons[PM_HUMAN_WEREBEAUTIFULFUNNELHEELEDPUMP] || mtmp->data == &mons[PM_BLOODY_BEAUTIES] || mtmp->data == &mons[PM_DISINTERESTED_OLDER_WOMAN] || mtmp->data == &mons[PM_JOY_PORN_STAR]) ) {
elena14:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == LADY_BOOTS) || (footwear && footwear->otyp == NADJA_BUCKLED_LADY_SHOES) || (footwear && footwear->otyp == BASIC_SHOES) || (FemtrapActiveKerstin && spawnswithblockheels(mtmp->data) ) || (footwear && footwear->otyp == LISA_COMBAT_BOOTS) || (footwear && footwear->otyp == MAGDALENA_COMBAT_BOOTS) || (footwear && footwear->otyp == SELF_WILLED_HEELS) || (footwear && footwear->otyp == COMMANDER_HEELS) || (footwear && itemhasappearance(footwear, APP_DYKE_BOOTS)) || (footwear && itemhasappearance(footwear, APP_ICEBLOCK_HEELS)) || (footwear && itemhasappearance(footwear, APP_SISTER_SHOES)) || (randomsexyheels == 16) || mtmp->data == &mons[PM_FOURFOLD_SHOE_ENEMY] || mtmp->data == &mons[PM_NORMAL_BLOCK_HEEL_BOOT] || mtmp->data == &mons[PM_SUPER_STRONG_GIRL] || mtmp->data == &mons[PM_ERIKA_S_HIGH_HEELED_SANDAL] || mtmp->data == &mons[PM_CHEATING_BLOCK_HEEL_BOOT] || mtmp->data == &mons[PM_CHEAP_BLOCK_HEEL_BOOT] || mtmp->data == &mons[PM_STAIRWELL_HEEL_BOOT] || mtmp->data == &mons[PM_CORNELIA_S_EROTIC_BLOCK_HEEL_SANDAL] || mtmp->data == &mons[PM_CUTE_BLOCK_HEEL_SANDAL] || mtmp->data == &mons[PM_ANIMATED_LADY_BOOT] ) ) {
elenalady:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elenaitaly:
			...
			/* stiletto sandals use M4_SANDALS */
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && footwear->otyp == STILETTO_SANDALS) || (footwear && footwear->otyp == PATRICIA_STEEL_CAPPED_SANDALS) || (footwear && itemhasappearance(footwear, APP_BALLET_HEELS)) || (footwear && itemhasappearance(footwear, APP_PARAGRAPH_SANDALS)) || (randomsexyheels == 18) || mtmp->data == &mons[PM_ANIMATED_PRETTY_SANDAL] || mtmp->data == &mons[PM_STEEL_CAPPED_SANDAL] || mtmp->data == &mons[PM_RAZOR_SHARP_HIGH_HEEL]) ) {
elenass:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
elenacuddle:
			...
			if ((!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_HEAP_OF_SHIT_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_TREADED_HEELS) ) || (footwear && footwear->otyp == HENRIETTA_COMBAT_BOOTS) || mtmp->data == &mons[PM_HENRIETTEN_SHOE] || mtmp->data == &mons[PM_TREADED_FEMMY] || mtmp->data == &mons[PM_HENRIETTA_SHOE] || (randomsexyheels == 29) ) ) {
elenahos:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_WEDGE_BOOTS) ) || (footwear && footwear->otyp == MELISSA_WEDGE_BOOTS) || (footwear && footwear->otyp == HANNAH_CUDDLE_BOOTS) || (footwear && itemhasappearance(footwear, APP_WEDGE_SNEAKERS) ) || (randomsexyheels == 22) || mtmp->data == &mons[PM_TOPLESS_NURSE] || (footwear && footwear->otyp == KRISTINA_PLATFORM_SNEAKERS) || mtmp->data == &mons[PM_NORDIC_LADY] || mtmp->data == &mons[PM_ANN_KATHRIN_S_CUDDLY_BOOT] || mtmp->data == &mons[PM_ATHLETIC_FEMMY] || mtmp->data == &mons[PM_LISA_S_CUDDLY_BOOT] ) ) {
elena18:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_HUGGING_BOOTS) ) || (footwear && footwear->otyp == INA_HUGGING_BOOTS) || (footwear && footwear->otyp == ANNA_HUGGING_BOOTS) || (footwear && footwear->otyp == ARABELLA_HUGGING_BOOTS) || (footwear && footwear->otyp == LUISA_HUGGING_BOOTS) || (footwear && footwear->otyp == LISELOTTE_HUGGING_BOOTS) || (footwear && footwear->otyp == GRETA_HUGGING_BOOTS) || (footwear && footwear->otyp == JANE_HUGGING_BOOTS) || (footwear && footwear->otyp == SUE_LYN_WINTER_BOOTS) || (footwear && footwear->otyp == CHARLOTTE_HUGGING_BOOTS) || (footwear && footwear->otyp == LITTLE_MARIE_HUGGING_BOOTS) || (footwear && footwear->otyp == LUDGERA_HIKING_BOOTS) || (footwear && footwear->otyp == KATI_GIRL_BOOTS) || (footwear && footwear->otyp == LAURA_WINTER_BOOTS) || (footwear && footwear->otyp == SARAH_HUGGING_BOOTS) || (footwear && footwear->otyp == MAURAH_HUGGING_BOOTS) || mtmp->data == &mons[PM_ANIMATED_HUGGING_BOOT] || mtmp->data == &mons[PM_NASTY_FEMMY] || mtmp->data == &mons[PM_SAUCY_FEMMY] || mtmp->data == &mons[PM_SLEEPY_LADY] || mtmp->data == &mons[PM_WEREHUGGINGBOOT] || mtmp->data == &mons[PM_LULU] || mtmp->data == &mons[PM_HUMAN_WEREHUGGINGBOOT] || mtmp->data == &mons[PM_THE_EXTRA_FLEECY_BUNDLE_HER_HUGGING_BOOT] || mtmp->data == &mons[PM_LUISA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_REBECCA_S_HUGGING_BOOT] || (randomsexyheels == 23) || mtmp->data == &mons[PM_BITCHY_LARA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_MARLEEN_S_HUGGING_BOOT] || mtmp->data == &mons[PM_VILEA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_LISELOTTE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_OVERSLEPT_GIRL] || mtmp->data == &mons[PM_AMELJE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_MELANIE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_HUGGING_BOOT_GIRL_WITH_A_BMW] || mtmp->data == &mons[PM_BUNDLE_NADJA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_MARIE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_SUPER_STRONG_GIRL] || mtmp->data == &mons[PM_POKEZUPFI] || mtmp->data == &mons[PM_KRISTIN_S_HUGGING_BOOT] || mtmp->data == &mons[PM_ARABELLA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_ARABELLA_SHOE] || mtmp->data == &mons[PM_SHY_ASIAN_GIRL] || mtmp->data == &mons[PM_MARIAN_S_PERSIAN_BOOT] || (footwear && footwear->otyp == JEANETTA_GIRL_BOOTS) || mtmp->data == &mons[PM_LAURA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_LITTLE_MARIE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_CHARLOTTE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_STUPID_BLONDE_GIRL] || mtmp->data == &mons[PM_FRIEDERIKE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_SOPHIA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_ADRIANA_S_SEXY_LACQUER_BOOT] || mtmp->data == &mons[PM_HEIKE_S_HUGGING_BOOT] || mtmp->data == &mons[PM_BETTI_S_HUGGING_BOOT] || mtmp->data == &mons[PM_MARIA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_HUSSY_FEMMY] || mtmp->data == &mons[PM_FEMALE_HIKING_BOOT] || mtmp->data == &mons[PM_GIRL_HUGGING_BOOT] || mtmp->data == &mons[PM_ANOREXIC_FEMMY] || (footwear && footwear->otyp == MARLENA_HIKING_BOOTS) || mtmp->data == &mons[PM_INDIAN_QUEEN] || mtmp->data == &mons[PM_GLITTER_FLAX] || mtmp->data == &mons[PM_EMMELIE_S_WHITE_HUGGING_BOOT] || mtmp->data == &mons[PM_DORA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_THE_HUGGING_TOPMODEL_HER_HUGGING_BOOT] || mtmp->data == &mons[PM_BUNDLE_NADJA] || mtmp->data == &mons[PM_JOHANETTA_S_WINTER_BOOT] || mtmp->data == &mons[PM_SADISTIC_ASIAN_GIRL] || mtmp->data == &mons[PM_ANJA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_LOVING_ASIAN_GIRL] || mtmp->data == &mons[PM_STARLET_BUNDLE] || mtmp->data == &mons[PM_ABSOLUTELY_CUDDLY_GIRL] || mtmp->data == &mons[PM_ALIDA_S_HUGGING_BOOT] || mtmp->data == &mons[PM_GRENEUVENIC_TOPMODEL] || mtmp->data == &mons[PM_OVERSLEPT_TROLL] || mtmp->data == &mons[PM_FANNY_S_LOVELY_WINTER_BOOT] || mtmp->data == &mons[PM_OFFICER_INA] || mtmp->data == &mons[PM_HUGGER_DRAGON] || mtmp->data == &mons[PM_TAIL_GUTTER] ) ) {
elena19:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				randomkick = rnd(3);
...
elena21:
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_ANKLE_BOOTS) ) || mtmp->data == &mons[PM_EMMA_S_ANKLE_BOOT] || mtmp->data == &mons[PM_MADELEINE_S_ANKLE_BOOT] || (randomsexyheels == 26) || mtmp->data == &mons[PM_BETTINA_S_COMBAT_BOOT] || mtmp->data == &mons[PM_JANINA_S_COMBAT_BOOT] || mtmp->data == &mons[PM_ARABELLA_S_HIGH_HEELED_LADY_SHOE] || mtmp->data == &mons[PM_HC_HIGHHEEL_HUSSY] || mtmp->data == &mons[PM_STAR_BOOT] ) ) {
elena22:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
			...
			if ( (!rn2(3) || player_shades_of_grey() ) && (!issoviet || !rn2(5)) && ((footwear && itemhasappearance(footwear, APP_BLOCK_HEELED_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_BLOCKCHOC_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_REMORA_HEELS) ) || (footwear && itemhasappearance(footwear, APP_HEELED_CHELSEA_BOOTS) ) || (footwear && itemhasappearance(footwear, APP_HEELED_HUGGING_BOOTS) ) || (footwear && footwear->otyp == VICTORIA_COMBAT_BOOTS) || (footwear && footwear->otyp == JETTE_COMBAT_BOOTS) || (footwear && footwear->otyp == DORA_COMBAT_BOOTS) || (footwear && footwear->otyp == SUSANNE_COMBAT_BOOTS) || (footwear && footwear->otyp == MARLEEN_COMBAT_BOOTS) || (footwear && footwear->otyp == NORA_COMBAT_BOOTS) || (footwear && footwear->otyp == KRISTIN_COMBAT_BOOTS) || (footwear && footwear->otyp == MELTEM_COMBAT_BOOTS) || (footwear && footwear->otyp == JULIA_COMBAT_BOOTS) || (footwear && footwear->otyp == CONNY_COMBAT_BOOTS) || (footwear && footwear->otyp == KATIA_COMBAT_BOOTS) || (footwear && footwear->otyp == MARIYA_COMBAT_BOOTS) || (footwear && footwear->otyp == RONJA_COMBAT_BOOTS) || (footwear && footwear->otyp == ARIANE_COMBAT_BOOTS) || (footwear && footwear->otyp == JOHANNA_COMBAT_BOOTS) || (footwear && footwear->otyp == INGE_COMBAT_BOOTS) || (footwear && footwear->otyp == NICOLE_COMBAT_BOOTS) || (footwear && footwear->otyp == ELENA_COMBAT_BOOTS) || (footwear && footwear->otyp == THAI_COMBAT_BOOTS) || (footwear && footwear->otyp == SHOCK_ISOLATED_HEELS) || (footwear && footwear->otyp == KERSTIN_COMBAT_BOOTS) || (footwear && footwear->otyp == LARA_COMBAT_BOOTS) || (footwear && footwear->otyp == JENNIFER_COMBAT_BOOTS) || (footwear && footwear->otyp == KATRIN_COMBAT_BOOTS) || (footwear && footwear->otyp == COVETED_BOOTS) || (footwear && footwear->otyp == HIGH_SCORING_HEELS) || mtmp->data == &mons[PM_ANIMATED_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_SLAP_HEELED_SANDAL_BOOT] || mtmp->data == &mons[PM_WEREBLOCKHEELEDCOMBATBOOT] || mtmp->data == &mons[PM_HUMAN_WEREBLOCKHEELEDCOMBATBOOT] || mtmp->data == &mons[PM_SHY_LAURA_S_LOVELY_COMBAT_BOOT] || mtmp->data == &mons[PM_LILLY_S_FLEECY_COMBAT_BOOT] || mtmp->data == &mons[PM_SEXY_OVERKNEE_BOOT] || (randomsexyheels == 27) || mtmp->data == &mons[PM_HANNAH_S_COMBAT_BOOT] || mtmp->data == &mons[PM_SABINE_S_ZIPPER_BOOT] || mtmp->data == &mons[PM_LARISSA_S_BLOCK_HEELED_BOOT] || mtmp->data == &mons[PM_NICOLE_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_ANTJE_S_BLOCK_HEELED_BOOT] || mtmp->data == &mons[PM_EROGENOUS_BRAT] || mtmp->data == &mons[PM_FOLLOWER_OF_SILLY_TABOOS] || mtmp->data == &mons[PM_LISA_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_FOURFOLD_SHOE_ENEMY] || mtmp->data == &mons[PM_BLOCK_HEELED_MARY_JANE] || mtmp->data == &mons[PM_KRISTIN_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_CLASSIC_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_RUEA_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_JUEN_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_RUTH_S_BLOCK_HEELED_LADY_BOOT] || mtmp->data == &mons[PM_PATRICIA_S_COMBAT_BOOT] || mtmp->data == &mons[PM_DESIREE_S_COMBAT_BOOT] || mtmp->data == &mons[PM_INGE_S_COMBAT_BOOT] || mtmp->data == &mons[PM_CORINA_S_SPECIAL_COMBAT_BOOT] || mtmp->data == &mons[PM_KATRIN_S_COMBAT_BOOT] || mtmp->data == &mons[PM_BIRGIT_S_LADY_BOOT] || mtmp->data == &mons[PM_BLOCK_HEELED_GIRL] || mtmp->data == &mons[PM_LAURA_S__SISTER__COMBAT_BOOT] || mtmp->data == &mons[PM_POWERFUL_BLONDE_GIRL] || mtmp->data == &mons[PM_SISTER_COMBAT_BOOT] || mtmp->data == &mons[PM_KARATE_FEMMY] || mtmp->data == &mons[PM_MARLEEN_S_BLOCK_HEELED_COMBAT_BOOT] || mtmp->data == &mons[PM_CZECH_WENCH] || mtmp->data == &mons[PM_UNFORTUNATE_FOREST] || mtmp->data == &mons[PM_BITCHY_LARA_S_BLOCK_HEELED_BOOT] || mtmp->data == &mons[PM_PERSONA_NON_GRATA] || mtmp->data == &mons[PM_BLOCK_HEELED_PUSSY] ) ) {
elena23:
				u.cnd_shoedamageamount++;
				if (Role_if(PM_SOCIAL_JUSTICE_WARRIOR)) sjwtrigger();
				monsterlev = ((mtmp->m_lev) + 1);
...
/*	Work out the armor class differential	*/
	...
	if (uarmf && uarmf->otyp == PRINCESSEN_SHOES && !rn2(10)) tmp = -200; /* automiss */
...
/* The gnome swings her spiked battle boot! Ouch! The spiked boot soles bore themselves into your skin! The spikes were poisoned! The poison was deadly... Do you want your possessions identified? */
	...
	if (FemtrapActiveTanja && spawnswithsneakers(mtmp->data) && humanoid(mtmp->data) && mtmp->female) {
		...
		if (!rn2(50)) {
			switch (rnd(10)) {
				case 1: a->adtyp = AD_STUN; break;
				case 2: a->adtyp = AD_GLIB; break;
				case 3: a->adtyp = AD_NUMB; break;
				case 4: a->adtyp = AD_DISP; break;
				case 5: a->adtyp = AD_FEAR; break;
				case 6: a->adtyp = AD_NPRO; break;
				case 7: a->adtyp = AD_CHKH; break;
				case 8: a->adtyp = AD_FUMB; break;
				case 9: a->adtyp = AD_TREM; break;
				case 10: a->adtyp = AD_PAIN; break;
				default: break;
			}
		}
...
struct monst *mon;
{
	...
	if (mon == &youmonst) {
		...
		if (uarmc && uarmc->oartifact == ART_DISS_PLACE && armpro < 3) armpro = 3;
		if (uarmc && uarmc->oartifact == ART_DISS_PLACE && is_grassland(u.ux, u.uy) && armpro < 5) armpro = 5;
	...
	register struct attack  *mattk;
{
	...
	if (YouTakeMaximumDamage) tmp = (int)mattk->damn * (int)mattk->damd * 10;
	...
	switch(atttyp) {
		...
		if(!rn2(20)){
			if (!has_head(youmonst.data) || Role_if(PM_COURIER) ) {
				...
				if(!uarmh){
					dmg = (ABASE(A_WIS) <= ATTRABSMIN(A_WIS)) ? ( 2 * (Upolyd ? u.mh : u.uhp) + 400) : (dmg * 2); 
...
dopois:
 ...
 do_stone:
		...
		if ((!mtmp->mcan || u.ustuck == mtmp) && !(uactivesymbiosis && sticks(&mons[u.usymbiote.mnum]) ) && !sticks(youmonst.data)) {
			...
			if (is_drowningpool(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !(uwep && uwep->oartifact == ART_REGULARER_ANCHOR) && !Race_if(PM_KORONST)) {
		 ...
		 * Even if you're unbreathing, swimming or whatever. Your stuff isn't immune just because you are.  --Amy	*/
			...
			if (is_lava(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS)) {
		...
		/* This can only be done by monsters that can go on lava (flying couatls for example) and have wrap attacks */
			...
			if (is_urinelake(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS)) {
			...
			if (is_moorland(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS)) {
			...
			if (is_shiftingsand(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS)) {
			...
			if (is_styxriver(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS)) {
				pline("%s pulls you into the styx river!", Monnam(mtmp));
				contaminate(rnd(10), TRUE);
			}

			if (is_urinelake(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !rn2(StrongFlying ? 5 : 3)) {
			...
			if (is_moorland(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !rn2(StrongFlying ? 5 : 3)) {
			...
			if (is_shiftingsand(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !rn2(StrongFlying ? 5 : 3)) {
			...
			if (is_styxriver(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !rn2(StrongFlying ? 5 : 3)) {
			...
			if (is_lava(mtmp->mx,mtmp->my) && !(uarmf && uarmf->otyp == ANTI_DROWNING_BOOTS) && !rn2(StrongFlying ? 5 : 3)) {
		...
		if (!rn2(3) && atttyp != AD_SEDU && !issoviet && !(StealersActive) ) {
	...
	register struct attack  *mattk;
{
	...
	if (YouTakeMaximumDamage) tmp = (int)mattk->damn * (int)mattk->damd;
	int	tim_tmp;
...
do_stone2:
		...
		if (!rn2(3) && atttypA != AD_SEDU && !issoviet && !(StealersActive) ) {
			You_feel("a tug on your knapsack"); break;
		}
	...
	register struct attack  *mattk;
{
	...
	if (YouTakeMaximumDamage) dmgplus = (int)mattk->damn * (int)mattk->damd;
	...
	switch(atttypB) {
	    ...
	    case AD_SEDU:
	    case AD_SSEX:
		if (!rn2(3) && !issoviet && !(StealersActive) && canseemon(mtmp) && mtmp->mcansee ) break; /* no message, we don't want too much spam --Amy */

		if ((StealersActive) && !mtmp->mcan && canseemon(mtmp) && mtmp->mcansee) {
		...
		if (!mtmp->mcan && canseemon(mtmp) && !resists_blnd(&youmonst)
		    ...
		    if (YouTakeMaximumDamage) blnd = (int)mattk->damn * (int)mattk->damd;
		...
		if(!rn2(20)){
			if (!has_head(youmonst.data) || Role_if(PM_COURIER) ) {
				...
				if(!uarmh){
					dmgplus = (ABASE(A_WIS) <= ATTRABSMIN(A_WIS)) ? ( 2 * (Upolyd ? u.mh : u.uhp) + 400) : (dmgplus * 2); 
     ...
     if(!mtmp->mcan && canseemon(mtmp) && couldsee(mtmp->mx, mtmp->my) && mtmp->mcansee && (issoviet || !rn2(10)) &&
       ...
       if (u.usleep){
         ...
         if (ABASE(A_INT) > ATTRABSMIN(A_INT) && !rn2(10)) {
           /* adjattrib gives dunce cap message when appropriate */
...
register int n;
{
	...
	if (HardModeEffect || u.uprops[HARD_MODE_EFFECT].extrinsic || have_hardmodestone() || autismweaponcheck(ART_SWORD_OF_SPITE) || autismringcheck(ART_RING_OF_FAST_LIVING) || autismweaponcheck(ART_PAINBOWSWANDIR) || autismweaponcheck(ART_RAISING_HEART) || (uimplant && uimplant->oartifact == ART_IME_SPEW) || (uarm && uarm->oartifact == ART_CHEST_TANK)) n = n * 2;
	...
	if (uarmf && uarmf->oartifact == ART_INGRID_S_SECRETARY_OFFICE && n > 0 && !rn2(25)) {
		n = 0;
		Your("cone heels nullify the damage!");
		return;
	}
...
skiptreason:
	...
	if (multi >= 0 && ACURR(A_CON) == 1 && !rn2(200)) {
		You("gasp!");
		nomul(-2, "gasping", TRUE);
	}
	...
	/* Yes, real-life pregnancy doesn't work like this. But I want to avoid having to make complicated functions,
	   so the player will just get an egg that immediately hatches and may be tame. --Amy */
		...
		if (flags.female) {
			...
			pline("Oh! %s is pregnant!",noit_Monnam(mon)); verbalize("Please take good care of my baby, %s!",playeraliasname);
		}
