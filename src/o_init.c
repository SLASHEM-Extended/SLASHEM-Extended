...
find_flier_cloak()
{
    ...
    for (i = HAWAIIAN_SHIRT; i <= LEVITATION_BOOTS; i++) {
	if (itemnumwithappearance(i, APP_FLIER_CLOAK)) return i;
    }

    impossible("flier cloak not found?");
    return -1;	/* not 0, or caller would try again each move */
}
...
find_dogbone_helmet()
{
    ...
    for (i = HAWAIIAN_SHIRT; i <= LEVITATION_BOOTS; i++) {
	if (itemnumwithappearance(i, APP_DOGBONE_HELMET)) return i;
    }

    impossible("dogbone helmet not found?");
    return -1;	/* not 0, or caller would try again each move */
}
...
find_biker_helmet()
{
    ...
    for (i = HAWAIIAN_SHIRT; i <= LEVITATION_BOOTS; i++) {
	if (itemnumwithappearance(i, APP_BIKER_HELMET)) return i;
    }

    impossible("biker helmet not found?");
    return -1;	/* not 0, or caller would try again each move */
}
