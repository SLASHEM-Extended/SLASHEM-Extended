	...
	struct	obj	**pickp;
{
	...
	if (xlock.usedtime && picktyp == xlock.picktyp) {
		...
		if (AutoDestruct || u.uprops[AUTO_DESTRUCT].extrinsic || (uarmf && uarmf->oartifact == ART_KHOR_S_REQUIRED_IDEA) || have_autodestructstone() || (uarm && uarm->oartifact == ART_DESTRUCTO_S_COAT) || (uchain && uchain->oartifact == ART_SIYID) ) picklock();
		return(1);
	    }
	}
...
doforce()		/* try to force a chest with your weapon */
{
	...
	if(xlock.usedtime && picktyp == xlock.picktyp) {
		...
		if (AutoDestruct || u.uprops[AUTO_DESTRUCT].extrinsic || (uarmf && uarmf->oartifact == ART_KHOR_S_REQUIRED_IDEA) || have_autodestructstone() || (uarm && uarm->oartifact == ART_DESTRUCTO_S_COAT) || (uchain && uchain->oartifact == ART_SIYID) ) forcelock();
		return(1);
	    } else if (xlock.door) {
		You("resume your attempt to force the door.");
		set_occupation(forcedoor, "forcing the door", 0);
		if (AutoDestruct || u.uprops[AUTO_DESTRUCT].extrinsic || (uarmf && uarmf->oartifact == ART_KHOR_S_REQUIRED_IDEA) || have_autodestructstone() || (uarm && uarm->oartifact == ART_DESTRUCTO_S_COAT) || (uchain && uchain->oartifact == ART_SIYID) ) forcedoor();
		return(1);
	    }
	}
		...
		if (mtmp->isshk || mtmp->data == &mons[PM_ORACLE])		
		  if (Role_if(PM_JEDI) || Role_if(PM_SHADOW_JEDI) || Role_if(PM_HEDDERJEDI))
			...
			if (Role_if(PM_JEDI) ? (u.uen < 5) : Role_if(PM_SHADOW_JEDI) ? (u.uen < 5) : Role_if(PM_HEDDERJEDI) ? (u.uen < 5) : Race_if(PM_BORG) ? (u.uen < 7) : (u.uen < 10) ) pline("I don't think %s would appreciate that. Besides, you need %d mana in order to use the force.", mon_nam(mtmp), Role_if(PM_JEDI) ? 5 : Role_if(PM_SHADOW_JEDI) ? 5 : Role_if(PM_HEDDERJEDI) ? 5 : Race_if(PM_BORG) ? 7 : 10);
				...
				mtmp->masleep = 0;
				if (mtmp->mtame)
					monflee(mtmp, (dmg ? rnd(dmg) : 1), FALSE, FALSE, FALSE);
				else
					mtmp->mflee = 0;
...
struct obj *otmp;
{
	...
	case MT_FLESH:	disposition = "is mashed";
		break;
	case MT_FILM:	disposition = "evaporates";
		break;
	...
	case MT_SECREE:	disposition = "decomposes";
		break;
	case MT_ALKALINE:	disposition = "decomposes";
		break;
