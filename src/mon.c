...
#endif
	if ((level.flags.graveyard || mdat == &mons[PM_WRAITH] || mdat == &mons[PM_HUMAN_WRAITH] || mdat == &mons[PM_ACID_WRAITH]  || mdat == &mons[PM_GUD_WRAITH]|| mdat == &mons[PM_KING_WRAITH] || mdat == &mons[PM_WIGHTSPECTRE] || mdat == &mons[PM_CORONA_WRAITH] || mdat == &mons[PM_ROTTEN_WRAITH] || mdat == &mons[PM_SPECTRAL_WRAITH] || mdat == &mons[PM_DARKHORN_WRAITH] || mdat == &mons[PM_KODAK] || mdat == &mons[PM_BITCHY_LITTLE_ONE] || mdat == &mons[PM_BUTTO_WRAITH] || mdat == &mons[PM_ALU_WRAITH] || mdat == &mons[PM_ZIELUF] || mdat == &mons[PM_HUDDLED_WRAITH] || mdat == &mons[PM_HITTABLE_WRAITH] || mdat == &mons[PM_BIG_WRAITH] || mdat == &mons[PM_CREVICE_WRAITH] || mdat == &mons[PM_HUGE_WRAITH] || mdat == &mons[PM_GIGANTIC_WRAITH] || mdat == &mons[PM_WAFER_THIN_MINT] || mdat == &mons[PM_WRAITH_SHAMAN] || mdat == &mons[PM_NASTY_WRAITH] || mdat == &mons[PM_SHINE_WRAITH] || mdat == &mons[PM_CREEPING___] ) && is_undead(mdat) && !is_reviver(mdat) && mdat != &mons[PM_TROLL_ZOMBIE] && mdat != &mons[PM_EGO_TROLL_MUMMY] && mdat != &mons[PM_EVIL_TROLL_MUMMY] && mdat != &mons[PM_TROLL_PERMAMIMIC_MUMMY] && mdat != &mons[PM_TROLL_MUMMY] && mdat != &mons[PM_VECNA] && rn2(3)) return TRUE;
...
register struct monst *mtmp;
{
	...
	switch(mndx) {
		...
		if (mtmp->mrevived && rn2(20)) {
			...
			(void) mksobj_at(TITANIUM_HORN, x, y, TRUE, FALSE, FALSE);
		goto default_1;
		...
		if (mtmp->mrevived && rn2(20)) {
			...
			(void) mksobj_at(REPLICA_UNICORN_HORN, x, y, TRUE, FALSE, FALSE);
		goto default_1;
	    case PM_SKY_UNICORN:
		if (mtmp->mrevived && rn2(20)) {
			if (canseemon(mtmp))
...
/*	Nymphs take everything.  Most monsters don't pick up corpses. */
		...
		if (otmp->otyp == CORPSE && mtmp->data->mlet != S_NYMPH && mtmp->data != &mons[PM_GOLDEN_KNIGHT] && mtmp->data != &mons[PM_GOLDEN_HOLY_KNIGHT] && mtmp->data != &mons[PM_URCAGUARY] &&
			...
			otmp->corpsenm != PM_YUNICOR_LIZARD &&
			...
			otmp->corpsenm != PM_CONTACT_LIZARD &&
			otmp->corpsenm != PM_CORK_ESPADRILLE &&
			otmp->corpsenm != PM_PACKED_LIZARD &&
			otmp->corpsenm != PM_UNSILIZARD &&
			otmp->corpsenm != PM_SILILIZARD &&
...
struct obj *otmp;
{
	...
	if (mdat == &mons[PM_GOLDEN_HOLY_KNIGHT])
		return (boolean)(otmp->oclass != ROCK_CLASS);
	...
	long flag;
{
	...
	if (flag & ALLOW_DIG) {
		...
		rockok = (m_carrying(mon, PICK_AXE) || m_carrying(mon, CONGLOMERATE_PICK) || m_carrying(mon, UNWIELDY_PICK) || m_carrying(mon, CONUNDRUM_PICK) || m_carrying(mon, SHOVEL) || m_carrying(mon, MYSTERY_PICK) || m_carrying(mon, BRONZE_PICK) || m_carrying(mon, BRICK_PICK) || m_carrying(mon, MYSTERIOUS_PICK) || m_carrying(mon, NANO_PICK) ||
...
impossible("A monster looked at a very strange trap of type %d.", ttmp->ttyp);
			...
			if ((ttmp->ttyp != RUST_TRAP
				...
				&& ttmp->ttyp != REDINC_TRAP
				&& ttmp->ttyp != MULCH_TRAP
				&& ttmp->ttyp != INACCURACY_TRAP
				&& ttmp->ttyp != MONSTER_MULTISHOT_TRAP
				&& ttmp->ttyp != DO_YOU_HAVE_A_PIN_TRAP
				&& ttmp->ttyp != COSTLY_FAILURE_TRAP
				&& ttmp->ttyp != INVENTORY_SIZE_TRAP
		...
		while (rn2(25)) {
			...
			for (tryct = 0; tryct < 2000; tryct++) {
				...
				if (isok(x, y) && ((levl[x][y].typ > DBWALL) || canbeinawall) && !(t_at(x, y)) ) {
					...
					if (rtrap == ONE_WAY_PORTAL) rtrap = ROCKTRAP;
...
boolean was_swallowed;			/* digestion */
{
	...
	if ( (u.uprops[STARVATION_EFFECT].extrinsic || StarvationEffect || (uarmc && uarmc->oartifact == ART_FEMMY_FATALE) || have_starvationstone() || (ublindf && ublindf->oartifact == ART_TOTAL_PERSPECTIVE_VORTEX) ) && STARVATION_SPECIFIC_NOCORPSE(mdat))
		return FALSE;
