	...
	int	dest;
{
	...
	if((!accessible(x, y) && !is_pool(x, y)) ||
	   (x == u.ux && y == u.uy)) {
		...
		if ((mdat == &mons[PM_GECKO] || mdat == &mons[PM_GIANT_GECKO] || mdat == &mons[PM_FLYING_GECKO]) && !rn2(40) && timebasedlowerchance() ) otmp = mksobj_at(EUCALYPTUS_LEAF, x, y, TRUE, FALSE);
