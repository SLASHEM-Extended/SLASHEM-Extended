	...
	int	dest;
{
	...
	if((!accessible(x, y) && !is_waterypool(x, y) && !is_watertunnel(x,y)) ||
	   (x == u.ux && y == u.uy)) {
		...
		if (!rn2(120) && !splittinggremlin(mdat) && !ishardmoder && !splittinglavagremlin(mdat) && timebasedlowerchance() && !(YouDoNotGetDeathDrops || (mtmp->data->msound == MS_FLUIDATOR) || (mtmp->data->msound == MS_BULLETATOR) ) && (rn2(100) > u.usefulitemchance) ) otmp = mksobj_at(SCR_STANDARD_ID, x, y, TRUE, FALSE, FALSE);
		...
		if (uarmg && uarmg->oartifact == ART_SCROLLSCROLLSCROLL) {
			...
			if (!rn2(120) && !splittinggremlin(mdat) && !splittinglavagremlin(mdat) && timebasedlowerchance() && !(YouDoNotGetDeathDrops || (mtmp->data->msound == MS_FLUIDATOR) || (mtmp->data->msound == MS_BULLETATOR) ) && (rn2(100) > u.usefulitemchance) ) otmp = mksobj_at(SCR_STANDARD_ID, x, y, TRUE, FALSE, FALSE);
