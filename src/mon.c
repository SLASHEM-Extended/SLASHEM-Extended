	...
	switch(mndx) {
		...
		if (!rn2(mtmp->mrevived ? 20 : 3)) {
		    obj = mksobj_at(GRAY_DRAGON_SCALES +
				    monsndx(mdat)-PM_GRAY_DRAGON, x, y, FALSE, TRUE);
		    obj->spe = 0;
		...
		(void) mksobj_at(UNICORN_HORN, x, y, TRUE, TRUE);
		goto default_1;
	    case PM_LONG_WORM:
		(void) mksobj_at(WORM_TOOTH, x, y, TRUE, TRUE);
		goto default_1;
	    case PM_KILLER_TRIPE_RATION:            
		(void) mksobj_at(TRIPE_RATION, x, y, TRUE, TRUE);
		newsym(x, y);
		return (struct obj *)0;
	    case PM_KILLER_FOOD_RATION:
		(void) mksobj_at(FOOD_RATION, x, y, TRUE, TRUE);
		newsym(x, y);
		return (struct obj *)0;
	    case PM_VAMPIRE:
	    case PM_VAMPIRE_LORD:
		...
		num = d(2,6);
		while (num--)
			obj = mksobj_at(IRON_CHAIN, x, y, TRUE, TRUE);
		mtmp->mnamelth = 0;
		break;
	    case PM_GLASS_GOLEM:
		num = d(2,4);   /* very low chance of creating all glass gems */
		while (num--)
			obj = mksobj_at((LAST_GEM + rnd(9)), x, y, TRUE, TRUE);
		mtmp->mnamelth = 0;
		break;
	    case PM_CLAY_GOLEM:
		obj = mksobj_at(ROCK, x, y, FALSE, TRUE);
		...
		num = d(2,4);
		while(num--) {
			obj = mksobj_at(QUARTERSTAFF, x, y, TRUE, TRUE);
		...
		num = d(2,4);
		while(num--)
			obj = mksobj_at(LEATHER_ARMOR, x, y, TRUE, TRUE);
		mtmp->mnamelth = 0;
		break;
		case PM_WAX_GOLEM:
		num = d(2,4);
		while (num--)
			obj = mksobj_at(WAX_CANDLE, x, y, TRUE, TRUE);
		mtmp->mnamelth = 0;
		break;
		...
		num = d(2,3);
		while (num--)
			obj = mksobj_at(SCR_BLANK_PAPER, x, y, TRUE, TRUE);
		mtmp->mnamelth = 0;
		break;
	    default_1:
	    default:
	...
	register struct monst *mtmp;
{
	...
	for (otmp = level.objects[mtmp->mx][mtmp->my];
		...
		/* KMH -- Don't eat undigestable/choking objects */
		    ...
		    /* Left behind a pile? */
		    if(rnd(25) < 3) (void) mksobj_at(ROCK, mtmp->mx, mtmp->my, TRUE, TRUE);
...
struct monst *mtmp;
{
	...
	if (stop_timer(UNPOLY_MON, (genericptr_t) mtmp)) {
		mtmp->mhp = mtmp->mhpmax;
		pline("But wait...");
		if (newcham(mtmp, &mons[mtmp->oldmonnm], cansee(mtmp->mx,mtmp->my))) {
#if 0
			if (cansee(mtmp->mx,mtmp->my)) {
				pline("%s becomes %s!", buf, an(mtmp->data->mname));
			}
#endif
...
register struct monst *mdef;
{
	...
	if ((int)mdef->data->msize > MZ_TINY ||
     ...
     (obj->otyp == STATUE && mons[obj->corpsenm].msize >= mdef->data->msize) ||
		...
		otmp = mksobj_at(ROCK, x, y, TRUE, TRUE);
	...
	int     dest;
{
	...
	if(mdat == &mons[PM_MAIL_DAEMON]) {
		stackobj(mksobj_at(SCR_MAIL, x, y, FALSE, TRUE));
		redisp = TRUE;
	}
#endif
...
/* transform_msg prints the "Foo turns into bar." if true.  Needed because newcham
