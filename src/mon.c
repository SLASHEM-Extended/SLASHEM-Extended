	...
	int	dest;
{
	...
	if((!accessible(x, y) && !is_waterypool(x, y) && !is_watertunnel(x,y)) ||
	   (x == u.ux && y == u.uy)) {
		...
		if (!rn2(Race_if(PM_ROHIRRIM) ? 100 : 250) && !splittinggremlin(mdat) && !splittinglavagremlin(mdat) && timebasedlowerchance() && !(u.uprops[NO_DROPS_EFFECT].extrinsic || NoDropsEffect || have_droplessstone() || (mtmp->data->msound == MS_FLUIDATOR) ) && (rn2(100) > u.usefulitemchance) ) otmp = mksobj_at(SCR_PHASE_DOOR, x, y, TRUE, FALSE, FALSE);
