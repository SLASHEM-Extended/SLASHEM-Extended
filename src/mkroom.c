...
struct mkroom *sroom;
{
	...
	switch(type) {
		...
		case TENSHALL:
			...
			if (!rn2(10)) {u.specialtensionmonster = rnd(363); /* monstercolor function! */
				if (!rn2(4)) u.specialtensionmonsterB = rnd(363);
			}
			break;
	    case DRAGONLAIR:
	...
	for(sx = sroom->lx; sx <= sroom->hx; sx++)
		...
		if ( (rnd(100) <= moreorless) && (type != EMPTYNEST) ) mon = makemon(
		    ...
		    (type == BEEHIVE) ? (sx == tx && sy == ty ? (((depthuz < 5) && (level_difficulty() < (3 + rn2(3)))) ? &mons[PM_BIG_BEE] : &mons[PM_QUEEN_BEE]) : beehivemon()) :
		    ...
		    (type == MIGOHIVE) ? (sx == tx && sy == ty ? (((depthuz < 10) && (level_difficulty() < (5 + rn2(5)))) ? &mons[PM_SUDO_MIGO] : &mons[PM_MIGO_QUEEN]) : migohivemon()) :
...
migohivemon()
{
	...
	if ((depthuz < 10) && (level_difficulty() < (5 + rn2(5)))) return (&mons[PM_LITTLE_MIGO]);
...
squadmon()		/* return soldier types. */
{
	...
	if ((depthuz < 8) && (level_difficulty() < (4 + rn2(2)))) {
	...
	if ((depthuz < 8) && (level_difficulty() < (4 + rn2(2)))) mndx = PM_UNARMORED_SOLDIER;
gotone:
...
doomsquadmon()
{
	...
	if ((depthuz < 8) && (level_difficulty() < (4 + rn2(3)))) return(&mons[PM_STUNTED_ZOMBIEMAN]);
