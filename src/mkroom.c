...
struct mkroom *sroom;
{
	...
	switch(type) {
		...
		case TENSHALL:
			...
			if (!rn2(10)) {u.specialtensionmonster = rnd(425); /* monstercolor function! */
				if (!rn2(4)) u.specialtensionmonsterB = rnd(425);
			}
			break;
	    case DRAGONLAIR:
	...
	for(sx = sroom->lx; sx <= sroom->hx; sx++)
		...
		if ( ((rnd(100) <= moreorless) || (rn2(5) && sx == tx && sy == ty)) && (type != EMPTYNEST) ) mon = makemon(
			...
			(type == CASINOROOM) ? (level_difficulty() > 80 ? &mons[PM_ELITE_CROUPIER] : level_difficulty() > 40 ? &mons[PM_MASTER_CROUPIER] : &mons[PM_CROUPIER] )  :
...
beehivemon()
{
	...
	if (i > 108) return((level_difficulty() > 40) ? &mons[PM_NEUROBEE] : &mons[PM_KILLER_BEE]);
	else if (i > 107) return((level_difficulty() > 15) ? &mons[PM_VORACIOUS_FORCE_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 106) return((level_difficulty() > 15) ? &mons[PM_THWARTER_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 104) return((level_difficulty() > 15) ? &mons[PM_VORACIOUS_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 103) return((level_difficulty() > 9) ? &mons[PM_HIGHBEE] : &mons[PM_KILLER_BEE]);
	else if (i > 102) return(((level_difficulty() > 9) && !rn2(50)) ? &mons[PM_BAN_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 101) return((level_difficulty() > 6) ? (rn2(5) ? &mons[PM_ZOMBEE] : &mons[PM_SPIKE_ZOMBEE]) : &mons[PM_KILLER_BEE]);
	else if (i > 100) return((level_difficulty() > 6) ? &mons[PM_FUMBLEBEE] : &mons[PM_KILLER_BEE]);
	else if (i > 92) return((level_difficulty() > 6) ? &mons[PM_WING_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 84) return((level_difficulty() > 6) ? &mons[PM_TWIN_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 83) return((level_difficulty() > 6) ? &mons[PM_WEREKILLERBEE] : &mons[PM_KILLER_BEE]);
	else if (i > 82) return((level_difficulty() > 5) ? &mons[PM_GIANT_JELLY_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 81) return((level_difficulty() > 5) ? &mons[PM_SPIKE_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 71) return((level_difficulty() > 5) ? &mons[PM_GIANT_KILLER_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 70) return((level_difficulty() > 3) ? &mons[PM_SHARP_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 68) return((level_difficulty() > 3) ? &mons[PM_STUNNING_BEE] : &mons[PM_KILLER_BEE]);
	else if (i > 58) return(&mons[PM_THORN_BEE]);
	else if (i > 57) return(&mons[PM_BOTULISM_BEE]);
	else if (i > 56) return(&mons[PM_INJECTING_BEE]);
	else if (i > 55) return(&mons[PM_HUNTER_BEE]);
	else if (i > 54) return(&mons[PM_BUMBLEBEE]);
	else if (i > 53) return(&mons[PM_WEREBEE]);
	else if (i > 52) return(&mons[PM_HONEY_BEE]);
	else if (i > 51) return(&mons[PM_WAX_BEE]);
	else if (i > 50) return(&mons[PM_JELLY_BEE]);
