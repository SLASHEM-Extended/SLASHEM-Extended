    ...
    register xchar worm_tail;	/* mon is actually a worm tail */
{
	...
	( (Protection_from_shape_changers && !(mon->data == &mons[PM_DEFORMED_FISH] || mon->data == &mons[PM_KEYSTONE_INSPECTOR] || mon->data == &mons[PM_INSPECTOR_SERGEANT] || mon->data == &mons[PM_MAIDENHACK_HORROR] || mon->data == &mons[PM_YASD_HORROR] || mon->data == &mons[PM_GRATING_CHICKEN] || mon->data == &mons[PM_DEFORMED_LIZARD] || mon->data == &mons[PM_INSPECTOR_LIEUTENANT] || mon->data == &mons[PM_INSPECTOR_KAPTAIN] || mon->data == &mons[PM_INSPECTOR_KOMMISSIONER] || mon->data == &mons[PM_INSPECTOR_KCHIEF] || mon->data == &mons[PM_INSPECTOR_KATCHER] || mon->data == &mons[PM_OLOG_HAI_MIMIC] || mon->data == &mons[PM_OLOG_HAI_PERMAMIMIC] || mon->data == &mons[PM_VESTY] || mon->data == &mons[PM_CAMO_RUBBER] || mon->data == &mons[PM_WIPER_RUBBER] || mon->data == &mons[PM_DRONING_UFO_PART] || mon->data == &mons[PM_GNOSIS_SEPHIRAH] || mon->data == &mons[PM_HIDDEN_MOLD] || mon->data == &mons[PM_SECLUDED_MOLD] || mon->data == &mons[PM_HIDDEN_FUNGUS] || mon->data == &mons[PM_SECLUDED_FUNGUS] || mon->data == &mons[PM_HALLUCINATION_IMAGE] || mon->data == &mons[PM_MARSUPILAMI] || mon->data == &mons[PM_MAELSTROM]) ) || sensemon(mon));
    /*
    ...
    register int x,y;
{
	...
	if (x == u.ux && y == u.uy) {
		...
		: (mon_visible(mon)) || tp_sensemon(mon) || MATCH_WARN_OF_MON(mon) || (Role_if(PM_ACTIVISTOR) && mon->data == &mons[PM_TOPMODEL]) || (Role_if(PM_ACTIVISTOR) && type_is_pname(mon->data) && uwep && is_quest_artifact(uwep) ) || (uamul && uamul->otyp == AMULET_OF_UNDEAD_WARNING && is_undead(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_POISON_WARNING && poisonous(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_OWN_RACE_WARNING && your_race(mon->data) ) || (Role_if(PM_PALADIN) && is_demon(mon->data) ) || (Race_if(PM_VORTEX) && unsolid(mon->data) ) || (Race_if(PM_VORTEX) && nolimbs(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_COVETOUS_WARNING && is_covetous(mon->data) ) || (Stunnopathy && Stunned && always_hostile(mon->data) && (mon)->mhp % 4 != 0) || (Numbopathy && Numbed && avoid_player(mon->data) ) || (Sickopathy && Sick && extra_nasty(mon->data) ) || (Freezopathy && Frozen && mon->data->mcolor == CLR_WHITE ) || (Burnopathy && Burned && infravision(mon->data) ) || (Race_if(PM_RODNEYAN) && mon_has_amulet(mon)) || (Race_if(PM_RODNEYAN) && mon_has_special(mon)) || (Race_if(PM_LEVITATOR) && is_flyer(mon->data) ) );
	...
	if (x == u.ux && y == u.uy) {
		...
		&& ((see_it = (tp_sensemon(mon) || MATCH_WARN_OF_MON(mon) || (Role_if(PM_ACTIVISTOR) && mon->data == &mons[PM_TOPMODEL]) || (Role_if(PM_ACTIVISTOR) && type_is_pname(mon->data) && uwep && is_quest_artifact(uwep) ) 	|| (uamul && uamul->otyp == AMULET_OF_UNDEAD_WARNING && is_undead(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_POISON_WARNING && poisonous(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_OWN_RACE_WARNING && your_race(mon->data) ) || (Role_if(PM_PALADIN) && is_demon(mon->data) ) || (Race_if(PM_VORTEX) && unsolid(mon->data) ) || (Race_if(PM_VORTEX) && nolimbs(mon->data) ) || (uamul && uamul->otyp == AMULET_OF_COVETOUS_WARNING && is_covetous(mon->data) ) || (Stunnopathy && Stunned && always_hostile(mon->data) && (mon)->mhp % 4 != 0) || (Numbopathy && Numbed && avoid_player(mon->data) ) || (Sickopathy && Sick && extra_nasty(mon->data) ) || (Freezopathy && Frozen && mon->data->mcolor == CLR_WHITE ) || (Burnopathy && Burned && infravision(mon->data) ) || (Race_if(PM_RODNEYAN) && mon_has_amulet(mon)) || (Race_if(PM_RODNEYAN) && mon_has_special(mon)) || (Race_if(PM_LEVITATOR) && is_flyer(mon->data) )	    		|| (see_with_infrared(mon) && mon_visible(mon))))
		    || Detect_monsters)
		&& !is_worm_tail(mon)) {
