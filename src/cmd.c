...
int final;	/* 0 => still in progress; 1 => over, survived; 2 => dead */
{
	...
	if (Fast && !SpeedBug) you_are(Very_fast ? "very fast" : "fast");
	if (Fast && SpeedBug) you_are(Very_fast ? "very slow" : "slow");
	if (!Fast && Very_fast && !SpeedBug) you_are("very fast");
	if (!Fast && Very_fast && SpeedBug) you_are("very slow");
...
minimal_enlightenment()
{
	...
	*statline = '\0';
	...
	if (!DisplayLoss) {
		...
		Sprintf(eos(statline), "You are %s, a %s %s %s%s%s%s%s%s%s%s%s%s%s%s%s %s.", plname, align_str(u.ualign.type), (flags.female ? "female" : "male"), (flags.hybridangbander ? "angbander " : ""), (flags.hybridaquarian ? "aquarian " : ""), (flags.hybridcurser ? "curser " : ""), (flags.hybridhaxor ? "haxor " : ""), (flags.hybridhomicider ? "homicider " : ""), (flags.hybridsuxxor ? "suxxor " : ""), (flags.hybridwarper ? "warper " : ""), (flags.hybridrandomizer ? "randomizer " : ""), (flags.hybridnullrace ? "null " : ""), (flags.hybridmazewalker ? "mazewalker " : ""), (flags.hybridsoviet ? "soviet " : ""), (flags.hybridxrace ? "x-race " : ""), urace.adj, (flags.female && urole.name.f) ? urole.name.f : urole.name.m);
