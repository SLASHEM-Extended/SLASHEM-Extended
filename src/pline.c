...
impossible VA_DECL(const char *, s)
	...
	pline("Program in disorder. Please inform Amy about this bug.");
	pline("The admins can be contacted on the #em.slashem.me IRC channel (Libera).");
...
hybrid_strcode()
{
    ...
    *string = '\0';
	...
	if (flags.hybridhardmoder) sprintf(eos(string), "Har");
	if (flags.hybridstunfish) sprintf(eos(string), "Stu");
	if (flags.hybridkillfiller) sprintf(eos(string), "Kil");
	if (flags.hybridbadstatter) sprintf(eos(string), "Bad");
	if (flags.hybriddroughter) sprintf(eos(string), "Dgh");
...
register struct monst *mtmp;
{
	...
	if (mtmp->ispriest || mtmp->data == &mons[PM_ALIGNED_PRIEST] || mtmp->data == &mons[PM_ELITE_PRIEST] || mtmp->data == &mons[PM_MASTER_PRIEST] || mtmp->data == &mons[PM_EXPERIENCED_PRIEST] || mtmp->data == &mons[PM_EXCEPTIONAL_PRIEST]
...
register struct monst *mtmp;
{
	...
	if (mtmp->ispriest || mtmp->data == &mons[PM_ALIGNED_PRIEST] || mtmp->data == &mons[PM_MASTER_PRIEST] || mtmp->data == &mons[PM_EXPERIENCED_PRIEST] || mtmp->data == &mons[PM_EXCEPTIONAL_PRIEST] || mtmp->data == &mons[PM_ELITE_PRIEST]
				|| mtmp->data == &mons[PM_ANGEL])
		alignment = EPRI(mtmp)->shralign;
	else
		alignment = mtmp->data->maligntyp;
