...
register struct obj *obj;
{
	...
	case FOOD_CLASS:
	    ...
	    if (mon->data == &mons[PM_GHOUL] || mon->data == &mons[PM_GHAST]) {
	       if (obj->otyp == CORPSE && obj->corpsenm != PM_ACID_BLOB &&
		   ((obj->age + 5*(10+rn2(20))) <= monstermoves)) return DOGFOOD;
	       return TABU;
	    }
	    ...
	    if (is_vampire(mon->data)) {
	    	if (obj->otyp == CORPSE && has_blood(&mons[obj->corpsenm]) && 
	    	    ((obj->age + 5) >= monstermoves) && !obj->oeaten) return DOGFOOD;
	    	return TABU;
	    }
            /* otherwise */
	    if (!carni && !herbi)
		...
		case CORPSE:
		    ...
		    if (mons[obj->corpsenm].mlet == mon->data->mlet) {
                        ...
                        if (has_edog && (moves > EDOG(mon)->hungrytime + 500))
